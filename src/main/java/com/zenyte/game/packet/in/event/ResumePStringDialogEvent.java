...
public class ResumePStringDialogEvent implements ClientProtEvent {
    ...
    public void handle(Player player) {
        ...
        final java.lang.Object input = player.getTemporaryAttributes().get("interfaceInput");
        if (input instanceof StringDialogue) {
            final com.zenyte.plugins.dialogue.StringDialogue dialogue = (StringDialogue) input;
            dialogue.execute(player, string);
        }
    }

    public ResumePStringDialogEvent(final String string) {
        this.string = string;
    }
}
