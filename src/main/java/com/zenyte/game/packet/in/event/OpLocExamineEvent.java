...
public class OpLocExamineEvent implements ClientProtEvent {
    ...
    public void handle(Player player) {
        final com.zenyte.game.util.Examine examine = ObjectExamineLoader.DEFINITIONS.get(id);
        if (examine == null) {
            return;
        }
        final mgi.types.config.ObjectDefinitions def = ObjectDefinitions.get(id);
    ...
    public OpLocExamineEvent(final int id) {
        this.id = id;
    }
}
