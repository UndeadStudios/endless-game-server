...
public class ApeAtollArea extends Area implements ContainerPlugin {
	...
	public void enter(final Player player) {
		...
		final com.zenyte.game.world.region.area.apeatoll.Greegree greegree = Greegree.MAPPED_VALUES.get(player.getEquipment().getId(EquipmentSlot.WEAPON));
		if (greegree == null) {
			return;
		}
	...
	public void leave(final Player player, boolean logout) {
		...
		if (player.getTemporaryAttributes().remove("greegree") != null) {
			...
			final com.zenyte.game.item.Item item = equipment.getItem(EquipmentSlot.WEAPON);
	...
	public void onContainerModification(final Player player, final Container container, final Item previousItem, final Item currentItem) {
		...
		if (currentItem != null) {
			final com.zenyte.game.world.region.area.apeatoll.Greegree greegree = Greegree.MAPPED_VALUES.get(currentItem.getId());
			if (greegree != null) {
				greegree.transform(player);
			}
		}
