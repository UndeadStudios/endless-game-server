...
public enum Greegree {
	...
	public final void transform(final Player player) {
		...
		CharacterLoop.forEach(player.getLocation(), 15, NPC.class, npc -> {
			...
			final com.zenyte.game.world.entity.npc.NPCCombat combat = npc.getCombat();
	...
	static {
		for (final com.zenyte.game.world.region.area.apeatoll.Greegree greegree : VALUES) {
			MAPPED_VALUES.put(greegree.itemId, greegree);
		}
	}

	private Greegree(final int itemId, final int npcId) {
		this.itemId = itemId;
		this.npcId = npcId;
	}
