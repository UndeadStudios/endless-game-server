...
public class ApeAtollDungeonArea extends ApeAtollArea implements CycleProcessPlugin {
    ...
    public static void onFloorItemSpawn(final FloorItemSpawnEvent event) {
        if (event.getItem().getId() == 3187) {
            final com.zenyte.game.world.flooritem.FloorItem bones = event.getItem();
            ...
            WorldTasksManager.schedule(() -> {
                ...
                final com.zenyte.game.world.entity.Location tile = bones.getLocation();
                final java.util.List<com.zenyte.game.world.entity.npc.NPC> list = CharacterLoop.find(tile, 15, NPC.class, n -> !n.isDead() && n.getId() == 5237);
                if (list.size() >= 25) {
                    return;
                }
    ...
    private void mapSpikes() {
        final it.unimi.dsi.fastutil.ints.IntArrayList regions = new IntArrayList(new int[] {10894, 11150});
        regions.forEach((java.util.function.IntConsumer) id -> {
            final com.zenyte.game.world.region.Region region = World.getRegion(id, true);
            final it.unimi.dsi.fastutil.objects.ObjectCollection<com.zenyte.game.world.object.WorldObject> objects = region.getObjects().values();
    ...
    public void process() {
        ...
        for (final com.zenyte.game.world.entity.player.Player player : players) {
            ...
            final com.zenyte.game.world.object.WorldObject trap = traps.get(hash);
            if (trap != null) {
                sendSpikes(player, trap);
            } else if (hash == firstStalagmiteTrap || hash == secondStalagmiteTrap) {
                final com.zenyte.game.world.object.WorldObject object = World.getObjectWithType(hash == firstStalagmiteTrap ? firstTrapObjectLocation : secondTrapObjectLocation, 10);
                if (object != null) {
                    if (!object.isLocked()) {
