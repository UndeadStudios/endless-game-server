...
public class GreegreeItem extends ItemPlugin implements EquipPlugin {
	...
	public boolean handle(final Player player, final Item item, final int slotId, final int equipmentSlot) {
		final com.zenyte.game.world.region.area.apeatoll.Greegree greegree = Greegree.MAPPED_VALUES.get(item.getId());
		if (greegree == null) {
			return false;
		}
	...
	@Override
	public int[] getItems() {
		final it.unimi.dsi.fastutil.ints.IntArrayList list = new IntArrayList(Greegree.VALUES.length);
		for (final com.zenyte.game.world.region.area.apeatoll.Greegree greegree : Greegree.VALUES) {
