...
public class MapBuilder {
    ...
    private static DynamicRegion getDynamicRegion(final int regionId) {
        synchronized (MapBuilder.$LOCK) {
            final com.zenyte.game.world.region.Region region = World.regions.get(regionId);
            if (region instanceof DynamicRegion) {
                return (DynamicRegion) region;
            }
            final com.zenyte.game.world.region.DynamicRegion newRegion = new DynamicRegion(regionId);
    ...
    private static void destroyRegion(@NotNull final Region region) {
        ...
        CharacterLoop.forEachChunk(region.getId(), chunk -> {
            ...
            final java.util.List<com.zenyte.game.world.entity.npc.NPC> npcs = chunk.getNPCs();
            if (npcs.isEmpty()) return;
            //To prevent concurrent modifications, we construct our own list of the entities and iterate it.
            final java.util.ArrayList<com.zenyte.game.world.entity.npc.NPC> list = new ArrayList<NPC>(npcs);
            for (NPC next : list) {
                ...
                if (next instanceof Follower) {
                    ...
                    final com.zenyte.game.content.skills.hunter.npc.HunterDummyNPC dummy = (HunterDummyNPC) next;
                    final java.lang.ref.WeakReference<com.zenyte.game.content.skills.hunter.object.HunterTrap> trapWeakReference = dummy.getTrap();
                    if (trapWeakReference != null) {
                        final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = trapWeakReference.get();
                        if (trap != null) {
                            trap.remove();
                            continue;
                        }
    ...
    private static void copyChunk(final AllocatedArea allocated, final int fromChunkX, final int fromChunkY, final int fromPlane, final int toChunkX, final int toChunkY, final int toPlane, final int rotation) throws OutOfBoundaryException {
        final com.zenyte.game.world.region.DynamicRegion toRegion = getDynamicRegion(((toChunkX >> 3) << 8) + (toChunkY >> 3));
    ...
    private static void copyMap(final AllocatedArea allocated, final int fromChunkX, final int fromChunkY, final int toChunkX, final int toChunkY, final int width, final int height, final int... planes) throws OutOfBoundaryException {
        for (int xOffset = 0; xOffset < width; xOffset++) {
            for (int yOffset = 0; yOffset < height; yOffset++) {
                ...
                final com.zenyte.game.world.region.DynamicRegion toRegion = getDynamicRegion(((toThisChunkX >> 3) << 8) + (toThisChunkY >> 3));
