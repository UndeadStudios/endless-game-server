...
public class LumbridgeSwampCavesArea extends DarkArea implements PartialMovementPlugin {
	...
	public void process() {
		...
		for (final com.zenyte.game.world.entity.player.Player player : players) {
			final java.util.Map<java.lang.Object, java.lang.Object> attributes = player.getTemporaryAttributes();
			final java.lang.Object flaring = attributes.get("lightsource flaring");
			if (flaring != null) {
				final int flareTimer = player.getNumericTemporaryAttribute("lightsource flare count").intValue();
				if (flareTimer == 5) {
					final com.zenyte.game.world.entity.Location position = new Location(player.getNumericTemporaryAttribute("lightsource flare hash").intValue());
					if (player.getLocation().withinDistance(position, 3)) {
						...
						final java.lang.Object name = attributes.get("lightsource name");
			...
			if (Utils.random(1000) == 0) {
				final com.zenyte.plugins.item.LightSourceItem.LightSource[] lightsources = LightSource.getLitLightSources(player, LightSourceItem.NONE);
			...
			if (Utils.random(10) == 0 && flaring == null) {
				final com.zenyte.plugins.item.LightSourceItem.LightSource[] lightsources = LightSource.getLitLightSources(player, LightSourceItem.FLAMMABLE);
				if (lightsources.length == 0) {
					continue;
				}
				final com.zenyte.game.world.entity.Location location = player.getLocation();
				for (final com.zenyte.game.world.entity.Location flammableLocation : FLAMMABLE_LOCATIONS) {
					if (location.withinDistance(flammableLocation, 3)) {
						final java.lang.String name = lightsources[Utils.random(lightsources.length - 1)].toString().toLowerCase().replaceAll("_", " ");
	...
	public boolean processMovement(Player player, int x, int y) {
		final com.zenyte.game.world.entity.Location tile = WALL_BEASTS.get(y | x << 14 | player.getPlane() << 28);
		if (tile == null) {
			return true;
		}
