...
public class SouthernDesertArea extends DesertArea implements CycleProcessPlugin {
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SouthernDesertArea.class);
	...
	public void enter(final Player player) {
		final java.lang.Object time = player.getTemporaryAttributes().getOrDefault("desertHeatCounter", 0);
	...
	public void process() {
		...
		loop:
		for (final com.zenyte.game.world.entity.player.Player player : desertPlayers) {
			try {
				if (player.isNulled()) {
					continue;
				}
				...
				final java.lang.Object time = attributes.getOrDefault("desertHeatCounter", 0);
				if (!(time instanceof Integer)) {
					return;
				}
