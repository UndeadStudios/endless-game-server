...
public class Region {
    ...
    public void addMultiZone(final int chunkXInRegion, final int chunkYInRegion, final int chunkZ, final int copiedChunkHash) {
        ...
        final java.util.List<Area> polygons = allPolygons.get(copiedChunkZ);
        for (int i = polygons.size() - 1; i >= 0; i--) {
            ...
            if (!area.isEmpty()) {
                ...
                final com.zenyte.game.world.region.Chunk chunk = World.getChunk(Chunk.getChunkHash(regionChunkX + chunkXInRegion, regionChunkY + chunkYInRegion, chunkZ));
                final Area repositionedArea = getRepositionedArea(area, xOffset, yOffset);
                final java.util.List<EfficientArea> efficientAreas = MultiwayArea.addDynamicMultiArea(repositionedArea, chunkZ);
    ...
    private Area getRepositionedArea(final Area area, final int xOffset, final int yOffset) {
        final java.util.ArrayList<int[]> list = new ArrayList<int[]>();
        final java.awt.geom.PathIterator it = area.getPathIterator(null);
        while (!it.isDone()) {
        ...
        final com.zenyte.game.world.region.RSPolygon rspolygon = new RSPolygon(array);
        final Polygon polygon = rspolygon.getPolygon();
        return new Area(polygon);
    }

    void removeMultiZone(final int chunkX, final int chunkY, final int z) {
        final com.zenyte.game.world.region.Chunk chunk = World.getChunk(Chunk.getChunkHash(chunkX, chunkY, z));
        final java.util.List<com.zenyte.game.world.region.Chunk.RSArea> multiAreas = chunk.getMultiPolygons();
        if (multiAreas != null) {
            for (int i = multiAreas.size() - 1; i >= 0; i--) {
                final com.zenyte.game.world.region.Chunk.RSArea area = multiAreas.get(i);
    ...
    public final FloorItem getFloorItem(final int id, final Location tile, final Player player) {
        final com.zenyte.game.world.region.Chunk chunk = this.getChunk(tile.getX() & 63, tile.getY() & 63, tile.getPlane());
        final java.util.Set<com.zenyte.game.world.flooritem.FloorItem> floorItems = chunk.getFloorItems();
        if (floorItems == null) {
            return null;
        }
    ...
    public void load() {
        ...
        loadStage = 1;
        try {
            final java.util.Set<com.zenyte.game.world.flooritem.GlobalItem> set = GlobalItem.getGlobalItems(regionId);
            if (set != null) {
                for (final com.zenyte.game.world.flooritem.GlobalItem item : set) {
                    item.spawn();
                }
            }
            loadRegionMap();
    ...
    public void spawnObject(final WorldObject object, final int plane, final int localX, final int localY, final boolean mapObject, final boolean alterClipping) {
        ...
        final com.zenyte.game.world.region.Chunk chunk = this.getChunk(localX, localY, plane);
        final it.unimi.dsi.fastutil.shorts.Short2ObjectMap<com.zenyte.game.world.object.WorldObject> originalObjects = chunk.getOriginalObjects();
        final it.unimi.dsi.fastutil.shorts.Short2ObjectMap<com.zenyte.game.world.object.WorldObject> spawnedObjects = chunk.getSpawnedObjects();
        originalObjects.remove(hash);
        final com.zenyte.game.world.object.WorldObject original = objects.get(hash);
        final com.zenyte.game.world.object.WorldObject spawned = spawnedObjects.remove(hash);
    ...
    public void removeObject(final WorldObject object, final int plane, final int localX, final int localY) {
        ...
        final it.unimi.dsi.fastutil.shorts.Short2ObjectMap<com.zenyte.game.world.object.WorldObject> spawnedObjects = chunk.getSpawnedObjects();
        final com.zenyte.game.world.object.WorldObject spawned = spawnedObjects.remove(hash);
        final com.zenyte.game.world.object.WorldObject current = objects.get(hash);
        if (spawned != null) {
            unclip(spawned, localX, localY);
        }
        if (current != null) {
    ...
    private final WorldObject getSpawnedObjectWithSlot(final int plane, final int x, final int y, final int slot) {
        final com.zenyte.game.world.region.Chunk chunk = this.getChunk(x, y, plane);
        final it.unimi.dsi.fastutil.shorts.Short2ObjectMap<com.zenyte.game.world.object.WorldObject> spawnedObjects = chunk.getSpawnedObjects();
    ...
    public void loadRegionMap() {
        try {
            ...
            final mgi.utilities.ByteBuffer mapBuffer = mapGroup == null ? null : mapGroup.findFileByID(0).getData();
    ...
    public boolean containsSpawnedObject(final WorldObject object) {
        final com.zenyte.game.world.region.Chunk chunk = this.getChunk(object.getX() & 63, object.getY() & 63, object.getPlane());
        final it.unimi.dsi.fastutil.shorts.Short2ObjectMap<com.zenyte.game.world.object.WorldObject> spawnedObjects = chunk.getSpawnedObjects();
        if (spawnedObjects == null) {
            return false;
        }
    ...
    public WorldObject getObjectWithId(final int plane, final int x, final int y, final int id) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (object.getId() == id) {
                final com.zenyte.game.world.region.Chunk chunk = this.getChunk(x, y, plane);
                final it.unimi.dsi.fastutil.shorts.Short2ObjectMap<com.zenyte.game.world.object.WorldObject> removedObjects = chunk.getOriginalObjects();
