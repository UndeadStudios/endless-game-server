...
public class FillContainer extends Action {
            ...
            if (combo == FillCombinations.FILL_WATER) {
                if (item.getName().startsWith("Watering can")) {
                    if (object.getName().toLowerCase().contains("well")) {
                        player.getDialogueManager().start(new PlainChat(player, "If I drop my watering can down there I don\'t think I\'m likely to get it back."));
                        return;
                    }
                }
                if (item.getId() == 1925) {
    ...
    private boolean check() {
        if (player.isDead() || player.isFinished() || !checkObject()) return false;
        if (!player.getInventory().containsItem(item) || combo == null) return false;
        if (combo == FillCombinations.FLOUR_BIN && player.getNumericAttribute("flourbin").intValue() == 0) return false;
        return !combo.isAutofill() || count == 0;
    }


    public enum FillCombinations {
        FILL_WATER("", Animation.GRAB, new Integer[][] {{229, 1923, 1925, 1935, 20800, 5331, 5333, 5334, 5335, 5336, 5337, 5338, 5339, 1831, 1829, 1827, 1825}, {227, 1921, 1929, 1937, 20801, 5340, 5340, 5340, 5340, 5340, 5340, 5340, 5340, 1823, 1823, 1823, 1823}}, "well", "fountain", "sink", "geyser", "water barrel", "waterpump", "pump and drain", "tap"), FLOUR_BIN("", new Animation(832), new Integer[][] {{1931}, {1933}}, "flour bin"), FILL_MILK("You milk the cow.", new Animation(2305), new Integer[][] {{1925}, {1927}}, "dairy cow"), SPADE_COKE("You take a spadeful of coke.", BlastFurnaceArea.TAKE_COKE, new Integer[][] {{952}, {6448}}, false, "coke");
