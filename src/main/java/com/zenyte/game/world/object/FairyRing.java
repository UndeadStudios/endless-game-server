...
public enum FairyRing {
	...
	static {
		final com.zenyte.game.world.object.FairyRing[] entries = values();
	...
	FairyRing(final Location tile, final String code, final String tag, final String name) {
		final mgi.types.config.enums.StringEnum e = Enums.FAIRY_RING_CODES;
		assert code.length() == 3;
		final java.util.OptionalInt optionalComponentHash = e.getKey(code.charAt(0) + " " + code.charAt(1) + " " + code.charAt(2));
	...
	public static final List<String> getFavourites(final Player player) {
		final java.lang.Object obj = player.getAttributes().get("fairyRingFavourites");
	...
	public static final void onLogin(final LoginEvent event) {
		...
		final java.util.OptionalInt bitValue = Enums.FAIRY_RING_VARBIT_CODES.getKey(381 << 16 | component);
	...
	public static final void handle(final Player player, final WorldObject object, final FairyRing ring) {
		...
		if (ring != null) {
			final java.util.OptionalInt bitValue = Enums.FAIRY_RING_VARBIT_CODES.getKey(381 << 16 | ring.getComponent());
