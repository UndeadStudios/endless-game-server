...
public abstract class Cutscene {
	...
	public final boolean process() {
		...
		if (duration >= length + 3) {
			...
			if (getMinimapState() != MinimapState.ENABLED) player.getPacketDispatcher().sendMinimapState(MinimapState.ENABLED);
		} else if (duration == 0) {
			if (getMinimapState() != MinimapState.ENABLED) player.getPacketDispatcher().sendMinimapState(getMinimapState());
		}
	...
	public void addActions(final int time, final Runnable... action) {
		...
		if (actions.containsKey(time)) {
			...
			for (int i = 0; i < existingActions.length; i++) actionSet[i] = existingActions[i];
			for (int i = 0; i < action.length; i++) actionSet[i + existingActions.length] = action[i];
