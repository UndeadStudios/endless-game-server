...
public enum ChallengeCategory {
    SKILLING(player -> {
        ...
        final com.zenyte.game.world.entity.player.dailychallenge.ChallengeDifficulty difficulty = level >= 75 ? ELITE : level <= 74 && level >= 45 ? HARD : level <= 44 && level >= 20 ? MEDIUM : EASY;
        return new ChallengeDetails(difficulty, skill);
    }), COMBAT(player -> {
        final int combatLevel = player.getCombatLevel();
        final com.zenyte.game.world.entity.player.dailychallenge.ChallengeDifficulty difficulty = combatLevel >= 3 && combatLevel <= 40 ? EASY : combatLevel >= 41 && combatLevel <= 75 ? MEDIUM : combatLevel >= 76 && combatLevel <= 100 ? HARD : ELITE;
    ...
    private ChallengeCategory(final DetailsDetermination determination) {
