...
public class SuperiorNPC extends NPC {
    ...
    private void executeOnDeathPlugins(final Player killer) {
        final java.util.List<com.zenyte.game.world.entity.npc.drop.matrix.DropProcessor> processors = DropProcessorLoader.get(getId());
        if (processors != null) {
    ...
    public void drop(final Location tile) {
        ...
        final java.util.List<com.zenyte.game.world.entity.npc.drop.matrix.DropProcessor> processors = DropProcessorLoader.get(root.getId());
        final com.zenyte.game.world.entity.npc.drop.matrix.NPCDrops.DropTable drops = NPCDrops.getTable(root.getId());
        for (int i = 0; i < 3; i++) {
            root.onDrop(killer);
            if (processors != null) {
                for (final com.zenyte.game.world.entity.npc.drop.matrix.DropProcessor processor : processors) {
        ...
        final double probability = 1.0F / (200.0F - (Math.pow(req + 55.0F, 2) / 125.0F));
