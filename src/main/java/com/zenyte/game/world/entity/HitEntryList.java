...
public class HitEntryList implements Iterable<HitEntry> {
    ...
    public boolean add(HitEntry hitEntry) {
        final com.zenyte.game.world.entity.HitEntry l = last;
    ...
    private void remove(HitEntry entry) {
        final com.zenyte.game.world.entity.HitEntry next = entry.getNext();
        final com.zenyte.game.world.entity.HitEntry prev = entry.getPrevious();
        if (prev == null) {
            first = next;
        } else {
            prev.setNext(next);
