...
public enum StandardRareDropTable {
    ...
    static {
        int weight = 0;
        for (final com.zenyte.game.world.entity.npc.drop.StandardRareDropTable value : values) {
            weight += value.weight;
        }
        TOTAL_WEIGHT = weight;
    }
    ...
    public static final boolean roll(@NotNull final NPC npc) {
        final com.zenyte.game.world.entity.npc.drop.StandardRareDropTable.SRDTNPC srdtnpc = SRDTNPC.map.get(npc.getDefinitions().getName().toLowerCase());
        if (srdtnpc != null) {
    ...
    public enum SRDTNPC {
        ...
        static {
            for (final com.zenyte.game.world.entity.npc.drop.StandardRareDropTable.SRDTNPC value : values) {
    ...
    private StandardRareDropTable(final int weight, final ImmutableItem item) {
        this.weight = weight;
        this.item = item;
    }
