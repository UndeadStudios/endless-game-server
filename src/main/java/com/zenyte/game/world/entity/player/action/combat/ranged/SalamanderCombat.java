...
public final class SalamanderCombat extends RangedCombat {
	...
	public int processWithDelay() {
		...
		final com.zenyte.game.world.entity.player.action.combat.ranged.SalamanderCombat.Tar tar = Tar.map.get(ammunition);
		assert tar != null : "No tar found for ammunition " + ammunition;
		resetFlag();
		final int style = player.getCombatDefinitions().getStyle();
		final com.zenyte.game.world.entity.player.action.combat.ranged.SalamanderCombat.StyleDefinition tarStyle = tar.styles[Math.min(style, tar.styles.length - 1)];
	...
	private static final int getSalamanderStrength(final Player player) {
