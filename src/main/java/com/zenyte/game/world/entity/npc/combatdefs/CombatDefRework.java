...
public class CombatDefRework {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(CombatDefRework.class);
    ...
    public static void launch() {
        ...
        NPCCombatDefinitionsLoader.DEFINITIONS.forEach((id, def) -> {
            final com.zenyte.game.world.entity.npc.OldNPCCombatDefinitions d = new OldNPCCombatDefinitions(def);
            d.setBonuses();
            final com.zenyte.game.world.entity.npc.combatdefs.MonsterCombatDefinition defs = new MonsterCombatDefinition();
            defs.construct(d);
        ...
        map.forEach((name, cbmap) -> {
            cbmap.forEach((level, osrsdef) -> {
                for (int i = 0; i < Utils.getIndiceSize(Indice.NPC_DEFINITIONS); i++) {
                    final mgi.types.config.npcs.NPCDefinitions npcDefinitions = NPCDefinitions.get(i);
        ...
        healthDefinitions.forEach((name_cb, hp) -> {
            final java.lang.String[] split = name_cb.split("_");
            final java.lang.String name = split[0];
            final java.lang.Integer level = Integer.valueOf(split[1]);
            for (int i = 0; i < Utils.getIndiceSize(Indice.NPC_DEFINITIONS); i++) {
                final mgi.types.config.npcs.NPCDefinitions npcDefinitions = NPCDefinitions.get(i);
                if (npcDefinitions == null || !npcDefinitions.getName().equals(name) || level != npcDefinitions.getCombatLevel()) continue;
                final com.zenyte.game.world.entity.npc.combatdefs.MonsterCombatDefinition defs = updatedMap.get(i);
        ...
        updatedMap.forEach((id, def) -> {
            ...
            if (def.getSpawnDefinitions() == null) {
                final com.zenyte.game.world.entity.npc.combatdefs.MonsterCombatDefinition.SpawnDefinitions spawnDefs = new MonsterCombatDefinition.SpawnDefinitions();
    ...
    public static void load() {
        try {
            ...
            for (final com.zenyte.game.world.entity.npc.combatdefs.MonsterExamineDefinition def : definitions) {
    ...
    public static void populate() {
        receivedMap.forEach((name, def) -> {
            def.getDefinitions().forEach((level, statistics) -> {
                final com.zenyte.game.world.entity.npc.combatdefs.CombatDefRework.OSRSDef defs = new OSRSDef();
                defs.set(statistics);
        ...
        map.forEach((name, cbmap) -> {
            cbmap.forEach((level, osrsdef) -> {
                for (int i = 0; i < Utils.getIndiceSize(Indice.NPC_DEFINITIONS); i++) {
                    final mgi.types.config.npcs.NPCDefinitions npcDefinitions = NPCDefinitions.get(i);
                    if (npcDefinitions == null || !npcDefinitions.getName().equals(name) || level != npcDefinitions.getCombatLevel()) continue;
                    final com.zenyte.game.world.entity.npc.combatdefs.NPCCombatDefinitions d = NPCCDLoader.get(i);
                    if (d != null) {
    ...
    static final class OSRSDef {
        ...
        private void set(final String statistics) {
            final java.lang.String escapedString = statistics.replaceAll("<col=ffb000>", "").replaceAll("</col>", "").replaceAll(" : ", ": ");
            final java.lang.String[] tabs = escapedString.split("[|]");
        ...
        private void setOtherAttributes(final String attributes) {
            final java.lang.String[] splitStats = attributes.split("<br>");
            for (final java.lang.String split : splitStats) {
        ...
        private void setDefensiveStats(final String stats) {
            final java.lang.String[] splitStats = stats.split("<br>");
            for (final java.lang.String split : splitStats) {
        ...
        private void setStats(final String stats) {
            ...
            for (final java.lang.String split : splitStats) {
                ...
                if (split.startsWith("Combat level: ")) {
                    ...
                    final java.lang.String value = split.replace("Main attack style: ", "");
                    switch (value) {
                    case "Crush": 
