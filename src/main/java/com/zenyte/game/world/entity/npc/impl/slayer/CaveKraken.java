...
public class CaveKraken extends NPC implements CombatScript, Spawnable {
	...
	static {
		for (final com.zenyte.game.world.region.RSPolygon polygon : polygons) {
			final java.awt.geom.Rectangle2D box = polygon.getPolygon().getBounds2D();
	...
	public void processMovement() {
		if (getFaceEntity() >= 0) {
			...
			if (target != null) {
				setDirection(Utils.getFaceDirection(target.getLocation().getCoordFaceX(target.getSize()) - getX(), target.getLocation().getCoordFaceY(target.getSize()) - getY()));
			}
		}
	...
	public void handleIngoingHit(final Hit hit) {
		...
		final com.zenyte.game.world.entity.player.Player target = (Player) hit.getSource();
		if (id == 493) {
