...
public enum GemDropTable {
    ...
    static {
        int weight = 0;
        for (final com.zenyte.game.world.entity.npc.drop.GemDropTable value : values) {
            weight += value.weight;
        }
        TOTAL_WEIGHT = weight;
    }
    ...
    public static final Optional<Item> get(@NotNull final Player player) {
        final com.zenyte.game.item.Item ring = player.getRing();
        ...
        for (final com.zenyte.game.world.entity.npc.drop.GemDropTable value : values) {
            if ((currentRoll += value.weight) >= roll) {
                if (value == TALISMAN) {
                    final com.zenyte.game.world.entity.Location loc = player.getLocation();
    ...
    public static final boolean roll(@NotNull final NPC npc) {
        final com.zenyte.game.world.entity.npc.drop.GemDropTable.GDTNPC gdtnpc = GDTNPC.map.get(npc.getDefinitions().getName().toLowerCase());
        if (gdtnpc != null) {
    ...
    public enum GDTNPC {
        ...
        static {
            for (final com.zenyte.game.world.entity.npc.drop.GemDropTable.GDTNPC value : values) {
                map.put(value.name.toLowerCase(), value);
            }
        }

        private GDTNPC(final String name, final int roll, final int maxRoll) {
            this.name = name;
            this.roll = roll;
    ...
    private GemDropTable(final int weight, final Item item) {
        this.weight = weight;
        this.item = item;
    }
