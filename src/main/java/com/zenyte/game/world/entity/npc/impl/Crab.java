...
public final class Crab extends NPC implements Spawnable {
	...
	protected void updateCombatDefinitions() {
		...
		if (def == null) {
			final com.zenyte.game.world.entity.npc.combatdefs.NPCCombatDefinitions cachedDefs = NPCCDLoader.get(isRocks() ? rocks2AliveMap.get(getId()) : getId());
	...
	static {
		...
		for (final it.unimi.dsi.fastutil.ints.Int2IntMap.Entry entry : alive2RocksMap.int2IntEntrySet()) {
