...
public interface CombatScript {
	...
	default void processPoison(final NPC npc, final Entity target) {
		final com.zenyte.game.world.entity.npc.combatdefs.NPCCombatDefinitions defs = npc.getCombatDefinitions();
	...
	default void attackSound() {
		try {
			final com.zenyte.game.world.entity.SoundEffect sound = ((NPC) this).getCombatDefinitions().getAttackDefinitions().getStartSound();
	...
	default Hit executeMeleeHit(final Entity target, final int max) {
		final com.zenyte.game.world.entity.masks.Hit hit = melee(target, max);
		delayHit(0, target, hit);
		return hit;
	}

	default void executeMeleeHit(final Entity target, final HitType type, final int max) {
		final com.zenyte.game.world.entity.npc.NPC npc = (NPC) this;
		final com.zenyte.game.world.entity.masks.Hit hit = new Hit(npc, getRandomMaxHit(npc, max, MELEE, target), type);
		delayHit(0, target, hit);
	}
	...
	default void useSpell(final CombatSpell spell, final Entity target, final int maxHit) {
		...
		int delay = 1;
		int clientDelay = 30;
		final com.zenyte.game.world.entity.masks.Graphics castGfx = spell.getCastGfx();
		if (castGfx != null) {
		...
		final com.zenyte.game.world.entity.HitEntry hitEntry = new HitEntry(npc, delay, magic(target, maxHit));
	...
	default void playAttackSound(final Entity target) {
		if (target instanceof Player) {
			final com.zenyte.game.world.entity.npc.NPC npc = (NPC) this;
			final com.zenyte.game.world.entity.npc.combatdefs.AttackDefinitions attDefs = npc.getCombatDefinitions().getAttackDefinitions();
			final com.zenyte.game.world.entity.SoundEffect sound = attDefs.getStartSound();
			if (sound != null) {
