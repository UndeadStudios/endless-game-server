...
public class LizardmanShaman extends NPC implements Spawnable, CombatScript {
    ...
    private final void spawnFollowers() {
        ...
        final com.zenyte.game.world.entity.Location baseSpawnLocation = getBaseSpawnLocation(player);
        final java.util.List<com.zenyte.game.world.entity.Location> spawnTiles = getSpawnLocations(baseSpawnLocation);
        final boolean isTemple = getId() == 8565;
        //Spawn the purple followers.
        for (final com.zenyte.game.world.entity.Location spawnTile : spawnTiles) {
    ...
    private final Location getBaseSpawnLocation(@NotNull final Player target) {
        ...
        final com.zenyte.game.world.entity.Location tile = new Location(target.getLocation());
        //Try and set the location away from underneath the Player, assuming it is still in an accessible location.
        for (int i = 0; i < 50; i++) {
            final com.zenyte.game.world.entity.Location potentialTile = new Location(target.getX() + Utils.random(-5, 5), target.getY() + Utils.random(-5, 5), target.getPlane());
    ...
    private final List<Location> getSpawnLocations(@NotNull final Location tile) {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.Location> list = new ObjectArrayList<Location>(3);
        //Add the base location itself.
        list.add(new Location(tile));
        final com.zenyte.game.world.entity.Location garbage = new Location(0);
    ...
    private final void jump(@NotNull final Entity target, @NotNull final Location tile) {
        ...
        WorldTasksManager.schedule(() -> {
            ...
            setLocation(tile);
            setAnimation(land);
            for (final com.zenyte.game.world.entity.player.Player p : CharacterLoop.find(tile, 1, Player.class, player -> !player.isDead() && (isMultiArea() || player == target))) {
    ...
    private final void shootPoison(@NotNull final Entity target) {
        setAnimation(attack);
        final com.zenyte.game.world.entity.Location tt = new Location(target.getLocation().getX(), target.getLocation().getY(), target.getLocation().getPlane());
        ...
        final com.zenyte.game.world.entity.Location startTile = getFaceLocation(target);
        final int delay = World.sendProjectile(startTile.matches(target) ? getLocation() : startTile, tt, poisonProjectile);
        WorldTasksManager.schedule(() -> {
            if (!isDead()) {
                for (final com.zenyte.game.world.entity.player.Player p : CharacterLoop.find(tt, 1, Player.class, player -> !player.isDead() && (isMultiArea() || player == target))) {
    ...
    public int attack(final Entity target) {
        ...
        if (style == 5) {
            final com.zenyte.game.world.entity.Location targetTile = target.getLocation().transform(-1, -1, 0);
    ...
    public static final float protectionModifier(final Player player) {
        ...
        final com.zenyte.game.item.Item helmet = player.getHelmet();
