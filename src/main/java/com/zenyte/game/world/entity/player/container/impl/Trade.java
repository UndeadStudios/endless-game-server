...
public class Trade {
	...
	public void addItem(final int slot, final int amount) {
		...
		if (!player.getPrivilege().eligibleTo(Privilege.ADMINISTRATOR) && !this.partner.getPrivilege().eligibleTo(Privilege.ADMINISTRATOR) && !item.isTradable()) {
	...
	public void removeItem(final int slot, final int amount) {
		...
		partner.getTrade().refreshMessages(1);
		cancelAccepted();
		for (final java.lang.Integer i : container.getModifiedSlots()) {
	...
	public void accept(final int stage) {
		if (!isTrading()) {
			...
			if (stage == 1) {
				for (final com.zenyte.game.item.Item item : partner.getTrade().getContainer().getItems().values()) {
	...
	public void closeTrade(final TradeStatus stage) {
		...
		final com.zenyte.game.world.entity.player.Player partner = this.partner;
		reset();
		...
		if (!stage.equals(SUCCESS)) {
			...
			for (final com.zenyte.game.item.Item item : partner.getTrade().container.getItems().values()) {
				if (item == null) {
					continue;
				}
			...
			final it.unimi.dsi.fastutil.objects.ObjectCollection<com.zenyte.game.item.Item> givenValues = container.getItems().values();
			final it.unimi.dsi.fastutil.objects.ObjectCollection<com.zenyte.game.item.Item> receivedValues = partner.getTrade().container.getItems().values();
			...
			// api trade logging
			if (Constants.WORLD_PROFILE.getApi().isEnabled() && !Constants.WORLD_PROFILE.isPrivate() && !Constants.WORLD_PROFILE.isDevelopment() && !Constants.WORLD_PROFILE.isBeta()) {
				CoresManager.getServiceProvider().submit(() -> new SubmitTradeLog(new TradeLog(player.getUsername(), player.getIP(), given, partner.getUsername(), partner.getIP(), received, Constants.WORLD_PROFILE.getNumber())).execute());
			}
	...
	private int getValue(final Player player) {
		final com.zenyte.game.world.entity.player.container.Container container = player.getIndex() == this.player.getIndex() ? this.container : partner.getTrade().getContainer();
