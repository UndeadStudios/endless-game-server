...
public final class LoyaltyManager {
    ...
    public static final void onInitialization(final InitializationEvent event) {
        ...
        final com.zenyte.game.world.entity.player.loyalty.LoyaltyManager currentLoyalty = player.getLoyaltyManager();
        currentLoyalty.login = Date.from(Instant.now());
        final com.zenyte.game.world.entity.player.Player saved = event.getSavedPlayer();
        final com.zenyte.game.world.entity.player.loyalty.LoyaltyManager loyaltyManager = saved.getLoyaltyManager();
    ...
    public static final void onLogout(final LogoutEvent event) {
        final com.zenyte.game.world.entity.player.Player player = event.getPlayer();
        final com.zenyte.game.world.entity.player.loyalty.LoyaltyManager loyalty = player.getLoyaltyManager();
        ...
        final java.util.Date minimumDate = calendar.getTime();
    ...
    public int getConsecutiveDaysLoggedIn(@NotNull final LocalDate from, @NotNull final LocalDate until, final int requiredDurationPerDay, @NotNull final TimeUnit timeUnit) {
        ...
        for (long epochDay = endDay; epochDay >= startDay; epochDay--) {
            final java.time.LocalDate date = LocalDate.ofEpochDay(epochDay);
    ...
    private long getLoggedInDuration(@NotNull final LocalDate day) {
        final java.util.Date date = Date.from(day.atStartOfDay(ZoneId.systemDefault()).toInstant());
        final java.util.Date endDate = Date.from(day.atStartOfDay(ZoneId.systemDefault()).toInstant());
        endDate.setTime(endDate.getTime() + TimeUnit.DAYS.toMillis(1));
        long totalMilliseconds = 0;
        for (final com.zenyte.game.world.entity.player.loyalty.LoyaltySession session : Objects.requireNonNull(this.sessions)) {
            final java.util.Date login = session.getLogin();
            final java.util.Date logout = session.getLogout();
    ...
    public void informSession(final int count) {
        ...
        final java.lang.String time = hours == 0 ? remainingMinutes + " minutes" : (hours + (hours == 1 ? " hour " : " hours ") + (remainingMinutes != 0 ? "and " : "") + (remainingMinutes == 0 ? "" : (remainingMinutes + " minutes")));
