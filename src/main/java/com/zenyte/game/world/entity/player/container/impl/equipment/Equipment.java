...
public class Equipment extends ContainerWrapper {
	private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(Equipment.class);
	...
	public final boolean wear(final int slotId) {
		...
		final com.zenyte.game.content.achievementdiary.DiaryReward reward = DiaryReward.get(item.getId());
	...
	private void equipmentPlugin(final Item old, final Item newItem) {
		if (old != null) {
			final com.zenyte.plugins.equipment.equip.EquipPlugin inventoryPlugin = EquipPluginLoader.PLUGINS.get(old.getId());
			if (inventoryPlugin != null) {
				inventoryPlugin.onUnequip(player, container, old);
			}
		}
		if (newItem != null) {
			final com.zenyte.plugins.equipment.equip.EquipPlugin inventoryPlugin = EquipPluginLoader.PLUGINS.get(newItem.getId());
			if (inventoryPlugin != null) {
