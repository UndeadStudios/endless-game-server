...
public class TeleportScrollPlugin extends ItemPlugin {
    ...
    public void handle() {
        bind("Read", (player, item, slotId) -> {
            ...
            final com.zenyte.game.world.entity.player.teleportsystem.TeleportScroll scroll = Objects.requireNonNull(TeleportScroll.map.get(id));
            final java.lang.String name = scroll.getName().replaceAll(" scroll", "");
    ...
    private final void readScroll(final Player player, final Item item, final int slotId) {
        ...
        final int id = item.getId();
        final com.zenyte.game.world.entity.player.teleportsystem.TeleportScroll scroll = Objects.requireNonNull(TeleportScroll.map.get(id));
        final java.lang.String name = scroll.getName().replaceAll(" teleport scroll", "");
        player.lock(5);
