...
public final class WalkStep {
	...
	public static boolean find(final Entity src, final Entity dest, int maxStepsCount, final boolean calculate) {
		...
		final com.zenyte.game.world.entity.Location nextPos = dest.getLocation();
	...
	public static final boolean calcFollow(final Entity entity, final Position target, final int maxStepsCount, final boolean calculate, final boolean intelligent, final boolean checkEntities) {
		if (intelligent) {
			final com.zenyte.game.world.entity.pathfinding.RouteResult steps = RouteFinder.findRoute(entity.getX(), entity.getY(), entity.getPlane(), entity.getSize(), target instanceof WorldObject ? new ObjectStrategy((WorldObject) target) : target instanceof Location ? new TileStrategy((Location) target) : new EntityStrategy((Entity) target), true);
