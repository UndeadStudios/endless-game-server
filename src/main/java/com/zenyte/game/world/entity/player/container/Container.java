...
public class Container {
	...
	public void set(final int slot, final Item item) {
		final com.zenyte.game.item.Item oldItem = item == null ? items.remove(slot) : items.put(slot, item);
		if (oldItem != null) {
		...
		if (player != null) {
			...
			if (area instanceof ContainerPlugin) {
				final com.zenyte.game.world.region.area.plugins.ContainerPlugin plugin = (ContainerPlugin) area;
	...
	public int getSlotOf(final int id) {
		for (int i = 0; i < getContainerSize(); i++) {
			final com.zenyte.game.item.Item item = get(i);
	...
	public Item getAny(final int... items) {
		for (int itemId : items) {
			final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> item = getFirst(itemId);
			if (item != null) {
	...
	public void shift() {
		...
		final it.unimi.dsi.fastutil.ints.Int2ObjectOpenHashMap<com.zenyte.game.item.Item> shiftedMap = new Int2ObjectOpenHashMap<Item>(itemsSize);
		int index = 0;
	...
	public MultiResult add(final Item... items) {
		final java.util.ArrayList<com.zenyte.game.world.entity.player.container.ContainerResult> results = new ArrayList<ContainerResult>(items.length);
		for (int i = 0; i < items.length; i++) {
			final com.zenyte.game.item.Item item = items[i];
			results.add(this.add(item));
		}
		final com.zenyte.game.world.entity.player.container.MultiResult multiresult = new MultiResult(ContainerState.ADD, results.toArray(new ContainerResult[results.size()]));
		return multiresult;
	}
	...
	public MultiResult remove(final Item... items) {
		...
		final com.zenyte.game.world.entity.player.container.MultiResult multiresult = new MultiResult(ContainerState.REMOVE, results.toArray(new ContainerResult[results.size()]));
		return multiresult;
	}
	...
	private void recalculateWeight() {
		weight = 0;
		for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> entry : this.items.int2ObjectEntrySet()) {
