...
public class SceneSynchronization {
    ...
    public static final void updateScopeInScene(@NotNull final Player player) {
        ...
        final it.unimi.dsi.fastutil.ints.IntOpenHashSet chunksInScope = player.getChunksInScope();
        ...
        for (int x = startX; x <= endX; x++) {
            for (int y = startY; y <= endY; y++) {
                ...
                final com.zenyte.game.world.region.Chunk chunk = World.getChunk(Chunk.getChunkHash(chunkX, chunkY, plane));
                ...
                if (!originalObjects.isEmpty()) {
                    final it.unimi.dsi.fastutil.objects.ObjectCollection<com.zenyte.game.world.object.WorldObject> objects = originalObjects.values();
                    for (final com.zenyte.game.world.object.WorldObject removedObject : objects) {
                        player.sendZoneUpdate(removedObject.getX(), removedObject.getY(), new LocDel(removedObject));
                    }
                }
                if (!spawnedObjects.isEmpty()) {
                    final it.unimi.dsi.fastutil.objects.ObjectCollection<com.zenyte.game.world.object.WorldObject> objects = spawnedObjects.values();
                    for (final com.zenyte.game.world.object.WorldObject spawnedObject : objects) {
                        player.sendZoneUpdate(spawnedObject.getX(), spawnedObject.getY(), new LocAdd(spawnedObject));
                    }
                }
                if (!floorItems.isEmpty()) {
                    for (final com.zenyte.game.world.flooritem.FloorItem item : floorItems) {
                        if (!item.isVisibleTo(player) || !player.isFloorItemDisplayed(item)) {
                            continue;
                        }
                        final com.zenyte.game.world.entity.Location location = item.getLocation();
