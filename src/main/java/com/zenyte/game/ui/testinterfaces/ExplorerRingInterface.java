...
public class ExplorerRingInterface extends Interface {
    ...
    private static final ItemSpell LOW_ALCHEMY_SPELL = Magic.getSpell(Spellbook.NORMAL, "low level alchemy", LowLevelAlchemy.class);
    private static final ItemSpell HIGH_ALCHEMY_SPELL = Magic.getSpell(Spellbook.NORMAL, "high level alchemy", HighLevelAlchemy.class);
    ...
    protected void build() {
        ...
        bind("Inventory", ((player, slotId, itemId, option) -> {
            ...
            final com.zenyte.game.content.skills.magic.spells.ItemSpell spell = player.getVarManager().getBitValue(HIGH_ALCHEMY_VARBIT) == 1 ? HIGH_ALCHEMY_SPELL : LOW_ALCHEMY_SPELL;
            if (spell == null) {
                return;
            }
