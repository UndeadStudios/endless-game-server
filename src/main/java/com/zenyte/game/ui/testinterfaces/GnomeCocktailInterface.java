...
public class GnomeCocktailInterface extends Interface {
    ...
    protected void build() {
        ...
        bind("Create", player -> {
            final com.zenyte.game.content.skills.cooking.GnomeCocktail cocktail = GnomeCocktail.getBySlot((Integer) player.getAttributes().get(SELECTED_COCKTAIL_SLOT));
            if (player.getInventory().containsItems(cocktail.getIngredients()) && player.getSkills().getLevel(Skills.COOKING) >= cocktail.getLevel()) {
                final CocktailShaker shaker = CocktailShaker.SHAKERS.get(cocktail.getShaker());
                final java.lang.String heat = (shaker.equals(CocktailShaker.CHOCOLATE_SATURDAY) || shaker.equals(CocktailShaker.DRUNK_DRAGON)) ? ", (heat it)," : "";
                String query = "";
