...
public class TeleportInterface extends Interface {
    ...
    protected void build() {
        bind("View Category", (player, slotId, itemId, option) -> {
            final com.zenyte.game.world.entity.player.teleportsystem.TeleportManager manager = player.getTeleportManager();
            final com.zenyte.game.world.entity.player.teleportsystem.TeleportCategory selectedCategory = TeleportCategory.get(slotId / 2);
        ...
        bind("Teleport to/Toggle favorite", (player, slotId, itemId, option) -> {
            final com.zenyte.game.world.entity.player.teleportsystem.TeleportManager manager = player.getTeleportManager();
            final com.zenyte.game.world.entity.player.teleportsystem.TeleportCategory category = manager.getLastCategory();
            if (category == null) {
                return;
            }
            ...
            if (slotId % 4 == 0) {
                ...
                final com.zenyte.game.world.entity.player.teleportsystem.PortalTeleport teleport = sorted.get(slot);
                if (teleport == null) {
                    return;
                }
        ...
        bind("Favorite teleport to", (player, slotId, itemId, option) -> {
            ...
            final com.zenyte.game.world.entity.player.teleportsystem.PortalTeleport teleport = manager.getFavoriteTeleports().get(slotId / 2);
            if (teleport == null) {
                return;
            }
            manager.teleportTo(teleport);
