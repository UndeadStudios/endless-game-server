...
public class DiangoItemRetrievalService extends Interface {
    ...
    private boolean refresh(@NotNull final Player player) {
        final java.util.List<java.lang.Integer> trackedHolidayItems = player.getTrackedHolidayItems();
        final java.util.BitSet set = new BitSet(trackedHolidayItems.size());
        for (final it.unimi.dsi.fastutil.ints.Int2IntMap.Entry entry : Enums.DIANGO_ITEM_RETRIEVAL.getValues().int2IntEntrySet()) {
    ...
    protected void build() {
        bind("Interact with item", (player, slotId, itemId, option) -> {
            ...
            final java.util.List<java.lang.Integer> trackedItems = player.getTrackedHolidayItems();
