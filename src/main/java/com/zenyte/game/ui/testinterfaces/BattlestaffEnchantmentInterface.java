...
public class BattlestaffEnchantmentInterface extends Interface {
    ...
    private static final void enchant(@NotNull final Player player, @NotNull final Battlestaff battlestaff) {
        ...
        for (int i = 0; i < 28; i++) {
            ...
            for (final com.zenyte.game.item.ItemOnItemAction.ItemPair pair : battlestaff.itemPairs) {
        ...
        final java.lang.String name = battlestaff.toString().toLowerCase().replace("_", " ");
        player.getInterfaceHandler().closeInterface(GameInterface.BATTLESTAFF_ENCHANTMENT);
        final int cost = DiaryReward.KANDARIN_HEADGEAR4.eligibleFor(player) ? 20000 : DiaryReward.KANDARIN_HEADGEAR3.eligibleFor(player) ? 30000 : 40000;
        if (slot == -1) {
