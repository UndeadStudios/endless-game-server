...
public class GrandExchangeOffersViewerInterface extends Interface {
    ...
    protected void build() {
        ...
        bind("Sort by quantity", player -> {
            ...
            player.getPacketDispatcher().sendClientScript(10808, getInterface().getId() << 16 | getComponent("Sort by quantity"), 0, player.getBooleanAttribute("ge_offers_viewer_quantity_sort") ? 1 : 0);
            final java.lang.Object obj = player.getTemporaryAttributes().get("ge_offers_selected_exchangetype");
            if (!(obj instanceof ExchangeType)) {
                return;
            }
            final com.zenyte.game.content.grandexchange.ExchangeType type = (ExchangeType) obj;
            final int itemId = player.getNumericTemporaryAttribute("ge_offers_selected_item").intValue();
        ...
        bind("Offer Entry", (player, slotId, itemId, option) -> {
            final java.lang.Object obj = player.getTemporaryAttributes().get("ge_offers_result");
            if (!(obj instanceof ArrayList)) {
                return;
            }
            final java.util.ArrayList<com.zenyte.game.content.grandexchange.ExchangeOffer> results = (ArrayList<ExchangeOffer>) obj;
            final int index = slotId / 9;
            final com.zenyte.game.content.grandexchange.ExchangeOffer offer = results.get(index);
            if (offer == null) {
                return;
            }
            ...
            if (offer.getType().equals(ExchangeType.BUYING)) {
                final mgi.types.config.items.ItemDefinitions def = ItemDefinitions.getOrThrow(id);
    ...
    public static final void search(final Player player, final int itemId, final ExchangeType exchangeType, SortType sortType) {
        final java.util.ArrayList<com.zenyte.game.content.grandexchange.ExchangeOffer> results = new ArrayList<ExchangeOffer>();
        ...
        for (final java.util.Map.Entry<java.lang.String, it.unimi.dsi.fastutil.ints.Int2ObjectOpenHashMap<com.zenyte.game.content.grandexchange.ExchangeOffer>> group : GrandExchangeHandler.getOffers().entrySet()) {
            final it.unimi.dsi.fastutil.ints.Int2ObjectOpenHashMap<com.zenyte.game.content.grandexchange.ExchangeOffer> offers = group.getValue();
            for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.content.grandexchange.ExchangeOffer> entry : offers.int2ObjectEntrySet()) {
                final com.zenyte.game.content.grandexchange.ExchangeOffer offer = entry.getValue();
        ...
        if (sortType == null) {
            final java.lang.Object obj = player.getTemporaryAttributes().get("ge_offers_selected_sorttype");
            if (obj instanceof SortType) {
                final com.zenyte.game.ui.testinterfaces.GrandExchangeOffersViewerInterface.SortType savedSortType = (SortType) obj;
        ...
        if (size > 0) {
            ...
            for (int index = 0; index < size; index++) {
                final com.zenyte.game.content.grandexchange.ExchangeOffer result = results.get(index);
