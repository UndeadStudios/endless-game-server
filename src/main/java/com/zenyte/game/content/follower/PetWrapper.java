...
public class PetWrapper {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PetWrapper.class);
    ...
    static {
        for (final com.zenyte.game.content.follower.impl.BossPet pet : BossPet.VALUES) {
            PETS_BY_ITEM_ID.put(pet.getItemId(), pet);
            PETS_BY_PET_ID.put(pet.getPetId(), pet);
        }
        for (final com.zenyte.game.content.follower.impl.SkillingPet pet : SkillingPet.VALUES) {
            PETS_BY_ITEM_ID.put(pet.getItemId(), pet);
            PETS_BY_PET_ID.put(pet.getPetId(), pet);
        }
    ...
    public static void startDialogue(final Player player, final NPC npc, final Class<? extends Dialogue> dialogue) {
        try {
            if (dialogue != null) {
                final com.zenyte.game.world.entity.player.dialogue.Dialogue d = dialogue.getDeclaredConstructor(invocationClasses).newInstance(player, npc);
