...
public class PetProcessor extends DropProcessor {
    ...
    public void onDeath(final NPC npc, final Player killer) {
        final com.zenyte.game.content.follower.impl.BossPet pet = BossPet.getByBossNPC(npc.getId());
        if (pet == null) {
            return;
        }
    ...
    @Override
    public int[] ids() {
        final it.unimi.dsi.fastutil.ints.IntSet set = BossPet.BOSS_PETS_BY_BOSS_NPC_ID.keySet();
