...
public enum Stage {
    ...
    public void sendVarbit(final Player player) {
        if (varbitValue != -1) {
            ...
            final com.zenyte.game.world.entity.ImmutableLocation position = mappedPositions.get(currentValue);
            if (position != null) {
                player.getPacketDispatcher().sendGraphics(poof, position);
            }
            final com.zenyte.game.world.entity.ImmutableLocation newPosition = mappedPositions.get(varbitValue);
            if (newPosition != null) {
    ...
    private Stage(final int varbitValue) {
