...
public class Incubator extends NPCPlugin {
    ...
    public void handle() {
        bind("Look-at", new OptionHandler() {
            ...
            public void click(final Player player, final NPC npc, final NPCOption option) {
                final com.zenyte.game.world.object.WorldObject object = World.getObjectWithType(incubatorLocation, 10);
    ...
    public enum State {
        ...
        private State(final int id, final int baseId, final int varbitId, final int varbitValue) {
            this.id = id;
            this.baseId = baseId;
    ...
    public static class ItemOnIncubator implements ItemOnNPCAction {
        ...
        public void handleItemOnNPCAction(Player player, Item item, int slot, NPC npc) {
            ...
            final com.zenyte.game.content.event.easter2020.plugin.npc.Incubator.State state = State.getCurrentState(player);
            switch (state) {
            case BROKEN: 
