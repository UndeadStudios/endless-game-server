...
public class HalloweenSteppingStone implements ObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (option.equalsIgnoreCase("Cross")) {
            final com.zenyte.game.util.Direction dir = player.getX() < object.getX() && player.getY() < object.getY() ? Direction.NORTH_EAST : player.getX() < object.getX() && player.getY() > object.getY() ? Direction.SOUTH_EAST : player.getX() > object.getX() && player.getY() < object.getY() ? Direction.NORTH_WEST : player.getX() > object.getX() && player.getY() > object.getY() ? Direction.SOUTH_WEST : player.getX() < object.getX() ? Direction.EAST : player.getX() > object.getX() ? Direction.WEST : player.getY() > object.getY() ? Direction.SOUTH : Direction.NORTH;
            final com.zenyte.game.world.entity.Location destination = player.getLocation().transform(dir, 1);
