...
public class ChristmasConstants {
    ...
    public static final void refreshAllVarbits(@NotNull final Player player) {
        final com.zenyte.game.content.event.christmas2019.PersonalSnowImp.Imp imp = ChristmasUtils.getImp(player);
        if (imp != null) {
        ...
        if (AChristmasWarble.progressedAtLeast(player, AChristmasWarble.ChristmasWarbleProgress.GHOST_OF_CHRISTMAS_PRESENT)) {
            ...
            final java.lang.String characters = ChristmasUtils.getFrozenGuestOrder(player);
            for (final char c : characters.toCharArray()) {
                final com.zenyte.game.content.event.christmas2019.cutscenes.FrozenGuest guest = Utils.findMatching(FrozenGuest.getValues(), g -> g.getConstant() == c);
