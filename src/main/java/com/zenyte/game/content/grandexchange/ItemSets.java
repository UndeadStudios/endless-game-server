...
public class ItemSets {
    public static List<Item> getItems(final int set) {
        final java.util.ArrayList<com.zenyte.game.item.Item> items = new ArrayList<Item>();
        final int enumId = Enums.ITEM_SETS.getValue(set).orElseThrow(Enums.exception());
        final mgi.types.config.enums.IntEnum specificEnum = EnumDefinitions.getIntEnum(enumId);
    ...
    public static void unpack(final Player player, final int set) {
        final java.util.List<com.zenyte.game.item.Item> items = getItems(set);
        if (player.getInventory().getFreeSlots() < items.size()) {
            player.sendMessage("Not enough space in your inventory");
            return;
        }
        final com.zenyte.game.world.entity.player.container.ContainerResult result = player.getInventory().deleteItem(set, 1);
