...
public class Decantation implements ItemOnItemAction {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        if (from.getId() == vial || to.getId() == vial || from.getId() == gourdVial || to.getId() == gourdVial || from.getId() == ItemId.EMPTY_CUP || to.getId() == ItemId.EMPTY_CUP) {
            final com.zenyte.game.item.Item vial = from.getId() == Decantation.vial || from.getId() == gourdVial || from.getId() == ItemId.EMPTY_CUP ? from : to;
            final com.zenyte.game.item.Item potion = vial == from ? to : from;
            final com.zenyte.game.content.consumables.Consumable consumable = Consumable.consumables.get(potion.getId());
            if (!(consumable instanceof Drinkable)) return;
            final com.zenyte.game.content.consumables.Drinkable drink = (Drinkable) consumable;
            final int doses = drink.getDoses(potion.getId());
            final com.zenyte.game.item.Item vialReplacement = drink.getItem((int) Math.ceil(doses / 2.0F));
            final com.zenyte.game.item.Item potionReplacement = drink.getItem(doses / 2);
        ...
        final com.zenyte.game.content.consumables.Consumable fromConsumable = Consumable.consumables.get(from.getId());
        if (!(fromConsumable instanceof Drinkable)) return;
        final com.zenyte.game.content.consumables.Consumable toConsumable = Consumable.consumables.get(to.getId());
        if (!(toConsumable instanceof Drinkable)) return;
        final com.zenyte.game.content.consumables.Drinkable fromDrink = (Drinkable) fromConsumable;
        final com.zenyte.game.content.consumables.Drinkable toDrink = (Drinkable) toConsumable;
        ...
        final com.zenyte.game.world.entity.player.container.impl.Inventory inventory = player.getInventory();
        final com.zenyte.game.item.Item fromReplacement = fromDrink.getItem(decantedFromDoses);
    ...
    public ItemPair[] getMatchingPairs() {
        ...
        final java.util.ArrayList<com.zenyte.game.content.consumables.Drinkable> decantablePotions = new ArrayList<Drinkable>(1500);
        ...
        for (final com.zenyte.game.content.consumables.Drinkable drinkable : decantablePotions) {
