...
public interface Consumable {
    ...
    static void initialize() {
        for (final com.zenyte.game.content.consumables.edibles.Food food : Food.values) {
            consumables.put(food.getId(), food);
            Consumable.food.put(food.getId(), food);
        }
        for (final com.zenyte.game.content.consumables.drinks.Ale ale : Ale.values) {
            consumables.put(ale.getId(), ale);
        }
        for (final com.zenyte.game.content.consumables.drinks.Drink drink : Drink.values) {
            for (final int id : drink.getIds()) {
                consumables.put(id, drink);
            }
        }
        for (final com.zenyte.game.content.consumables.drinks.Potion drink : Potion.values) {
            for (final int id : drink.getIds()) {
                consumables.put(id, drink);
            }
        }
        for (final com.zenyte.game.content.consumables.drinks.BarbarianMix drink : BarbarianMix.values) {
            for (final int id : drink.getIds()) {
                consumables.put(id, drink);
            }
        }
        for (final com.zenyte.game.content.consumables.drinks.GourdPotion drink : GourdPotion.values) {
            for (final int id : drink.getIds()) {
                consumables.put(id, drink);
                gourdDrinks.put(id, drink);
            }
        }
        for (final com.zenyte.game.content.consumables.drinks.KegAle drink : KegAle.values) {
            for (final int id : drink.getIds()) {
                consumables.put(id, drink);
            }
        }
