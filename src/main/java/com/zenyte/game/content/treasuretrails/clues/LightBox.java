...
public final class LightBox {
	...
	private void construct() {
		final it.unimi.dsi.fastutil.ints.IntArrayList availableBits = new IntArrayList(25);
		...
		final it.unimi.dsi.fastutil.ints.IntOpenHashSet uniqueBits = new IntOpenHashSet(8);
		/*
         ...
         */
		final it.unimi.dsi.fastutil.ints.Int2IntOpenHashMap usedBits = new Int2IntOpenHashMap(17);
	...
	private void associate(@NotNull final Int2IntOpenHashMap usedBits) {
		final it.unimi.dsi.fastutil.objects.ObjectIterator<it.unimi.dsi.fastutil.ints.Int2IntMap.Entry> it = usedBits.int2IntEntrySet().fastIterator();
	...
	private void refresh() {
		...
		if (longBit.length == 0) {
			final com.zenyte.game.item.Item item = findLightboxItem();
			if (item == null) {
				return;
			}
			item.setCharges(2);
	...
	private Item findLightboxItem() {
		...
		final it.unimi.dsi.fastutil.ints.Int2ObjectSortedMap.FastSortedEntrySet<com.zenyte.game.item.Item> entrySet = player.getInventory().getContainer().getItems().int2ObjectEntrySet();
		final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> item = Utils.findMatching(entrySet, entry -> entry.getValue().getId() == lightBox);
	...
	public void press(final int slot) {
		...
		final com.zenyte.game.content.treasuretrails.clues.LightBox.LightButton button = lightButtons.get(slot);
		if (button == null) {
			return;
		}
