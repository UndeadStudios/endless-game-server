...
public class StrangeCasket implements ObjectAction {
    ...
    private static final void search(@NotNull final Player player) {
        ...
        final com.zenyte.game.item.Item item = Utils.findMatching(player.getInventory().getContainer().getItems().values(), it -> it.getId() == ItemId.MIMIC && it.getNumericAttribute("The Mimic slain").intValue() == 0);
        if (item == null) {
            player.getDialogueManager().start(new PlainChat(player, "You find nothing in the strange casket."));
            return;
        }
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                final com.zenyte.game.world.entity.player.dialogue.NPCMessage message = npc(1, "The Mimic has... found you!<br>Will you fight now?", key);
