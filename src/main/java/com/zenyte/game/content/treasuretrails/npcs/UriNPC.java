...
public class UriNPC extends TreasureGuardian {
    ...
    public static final void spawnUri(@NotNull final Player player, @NotNull final EmoteRequest clue) {
        final java.util.Optional<com.zenyte.game.world.entity.Location> square = Utils.findEmptySquare(player.getLocation(), 2, 1, Optional.of(tile -> !tile.matches(player.getLocation()) && !ProjectileUtils.isProjectileClipped(null, null, player.getLocation(), tile, true) && clue.getPolygon().contains(tile)));
        final com.zenyte.game.world.entity.Location tile = square.orElse(player.getLocation());
        final com.zenyte.game.content.treasuretrails.npcs.UriNPC uri = new UriNPC(player, tile, clue);
    ...
    public void onFinish(final Entity source) {
        final com.zenyte.game.world.entity.Location tile = new Location(getLocation());
        ...
        World.getPlayer(username).ifPresent(player -> {
            ...
            final java.lang.Object attr = player.getTemporaryAttributes().get("spawned uri npc");
            if (attr == UriNPC.this) {
    ...
    public static final Optional<UriNPC> findUri(@NotNull final Player player) {
        ...
        if (attr instanceof UriNPC) {
            final com.zenyte.game.content.treasuretrails.npcs.UriNPC uri = (UriNPC) attr;
    ...
    public void setTalkative(final boolean talkative) {
