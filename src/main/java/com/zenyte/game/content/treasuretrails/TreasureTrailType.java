...
public enum TreasureTrailType {
    ...
    TreasureTrailType(final Class<? extends Enum<? extends Clue>> e) {
        ...
        int weight;
        for (final com.zenyte.game.content.treasuretrails.ClueLevel level : ClueLevel.values) {
            weight = 0;
            for (final java.lang.Enum<? extends com.zenyte.game.content.treasuretrails.clues.Clue> constant : constants) {
                final com.zenyte.game.content.treasuretrails.clues.Clue clue = ((Clue) constant);
                final com.zenyte.game.content.treasuretrails.ClueLevel constantLevel = clue.level();
    ...
    static {
        ...
        final it.unimi.dsi.fastutil.objects.Object2ObjectOpenHashMap<java.lang.String, com.zenyte.game.content.treasuretrails.clues.Clue> labelledClues = new Object2ObjectOpenHashMap<String, Clue>();
        for (final com.zenyte.game.content.treasuretrails.TreasureTrailType value : values) {
            for (final it.unimi.dsi.fastutil.objects.Object2IntMap.Entry<com.zenyte.game.content.treasuretrails.ClueLevel> weightMap : value.weights.object2IntEntrySet()) {
                final com.zenyte.game.content.treasuretrails.ClueLevel level = weightMap.getKey();
                totalWeights.put(level, totalWeights.getInt(level) + weightMap.getIntValue());
                final java.util.Set<com.zenyte.game.content.treasuretrails.clues.Clue> values = value.clues.get(level);
                if (values != null) {
                    for (final com.zenyte.game.content.treasuretrails.clues.Clue clue : values) {
                        final java.lang.String name = clue.getEnumName();
    ...
    public static final Clue random(@NotNull final ClueLevel level) {
        final java.util.EnumMap<com.zenyte.game.content.treasuretrails.TreasureTrailType, java.util.Set<com.zenyte.game.content.treasuretrails.clues.Clue>> typeMap = allClues.get(level);
        final com.zenyte.game.content.treasuretrails.TreasureTrailType randomEntry = Utils.getRandomCollectionElement(typeMap.keySet());
        final java.util.Set<com.zenyte.game.content.treasuretrails.clues.Clue> entries = typeMap.get(randomEntry);
