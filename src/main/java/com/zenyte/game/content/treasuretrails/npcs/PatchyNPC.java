...
public class PatchyNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handleItemOnNPCAction(Player player, Item item, int slot, NPC npc) {
        final com.zenyte.game.content.treasuretrails.npcs.PatchyNPC.SewableClothing clothing = Objects.requireNonNull(Utils.findMatching(SewableClothing.values, v -> v.sewnItem == item.getId()));
    ...
    public Object[] getItems() {
        ...
        for (final com.zenyte.game.content.treasuretrails.npcs.PatchyNPC.SewableClothing value : SewableClothing.values) {
    ...
    private enum SewableClothing {
        WHITE_BANDANA_EYEPATCH(ItemId.RIGHT_EYE_PATCH, ItemId.PIRATE_BANDANA, ItemId.BANDANA_EYEPATCH), RED_BANDANA_EYEPATCH(ItemId.RIGHT_EYE_PATCH, ItemId.PIRATE_BANDANA_7124, ItemId.BANDANA_EYEPATCH_8925), BLUE_BANDANA_EYEPATCH(ItemId.RIGHT_EYE_PATCH, ItemId.PIRATE_BANDANA_7130, ItemId.BANDANA_EYEPATCH_8926), BROWN_BANDANA_EYEPATCH(ItemId.RIGHT_EYE_PATCH, ItemId.PIRATE_BANDANA_7136, ItemId.BANDANA_EYEPATCH_8927), HAT_EYEPATCH(ItemId.RIGHT_EYE_PATCH, ItemId.PIRATES_HAT, ItemId.HAT_EYEPATCH), CRABCLAW_HOOK(ItemId.CRAB_CLAW, ItemId.PIRATES_HOOK, ItemId.CRABCLAW_HOOK), CAVALIER_AND_MASK(ItemId.BLACK_CAVALIER, ItemId.HIGHWAYMAN_MASK, ItemId.CAVALIER_MASK_11280), BERET_MASK(ItemId.MIME_MASK, ItemId.BLACK_BERET, ItemId.BERET_MASK_11282), PARTYHAT_AND_SPECS(ItemId.BLUE_PARTYHAT, ItemId.SAGACIOUS_SPECTACLES, ItemId.PARTYHAT__SPECS), TOP_HAT_AND_MONOCLE(ItemId.TOP_HAT, ItemId.MONOCLE, ItemId.TOP_HAT__MONOCLE), PIRATE_HAT_AND_PATCH(ItemId.BIG_PIRATE_HAT, ItemId.RIGHT_EYE_PATCH, ItemId.PIRATE_HAT__PATCH);
        ...
        private SewableClothing(final int baseItem, final int attachmentItem, final int sewnItem) {
    ...
    private final void sew(@NotNull final Player player, @NotNull final SewableClothing piece) {
        ...
        if (inventory.getAmountOf(ItemId.COINS_995) < CONNECT_COST || !inventory.containsItem(piece.baseItem, 1) || !inventory.containsItem(piece.attachmentItem, 1)) {
            final java.lang.String baseName = ItemDefinitions.getOrThrow(piece.baseItem).getName().toLowerCase();
            final java.lang.String attachmentName = ItemDefinitions.getOrThrow(piece.attachmentItem).getName().toLowerCase();
            final java.lang.String completeName = ItemDefinitions.getOrThrow(piece.sewnItem).getName().toLowerCase();
