    ...
    private enum Transformation {
        ...
        static {
            ...
            for (int i = 0; i < values.length; i++) {
                options[i] = WordUtils.capitalize(values[i].toString().replace("_", " ").toLowerCase(), '\000');
            }
        }

        private Transformation(final int npcId) {
            this.npcId = npcId;
        }
    }
        ...
        bind("Wear", (player, item, container, slotId) -> {
            ...
            player.getDialogueManager().start(new OptionsMenuD(player, "What would you like to be?", Transformation.options) {
                ...
                public void handleClick(int slotId) {
                    final com.zenyte.game.content.treasuretrails.plugins.RingOf3rdAge.Transformation formation = Transformation.values[slotId];
                    player.getInterfaceHandler().closeInterfaces();
