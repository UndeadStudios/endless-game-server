...
public final class PuzzleBox {
    ...
    private int emptySlot() {
        final it.unimi.dsi.fastutil.ints.IntAVLTreeSet slots = currentPuzzle.getAvailableSlots();
        assert slots.size() == 1;
    ...
    public void shiftPuzzle(final int slot) {
        ...
        final com.zenyte.game.item.Item movedTile = currentPuzzle.get(slot);
    ...
    public void checkCompletion() {
        ...
        final mgi.types.config.enums.IntEnum enumList = getEnum();
        for (int i = 0; i < 24; i++) {
            final java.util.OptionalInt optionalValue = enumList.getValue(i);
            final com.zenyte.game.item.Item currentPiece = currentPuzzle.get(i);
            if (!optionalValue.isPresent() || currentPiece == null || (currentPiece.getId() != optionalValue.getAsInt())) {
                return;
            }
        }
        final com.zenyte.game.item.Item item = findPuzzleItem();
        if (item == null) {
            return;
        }
        item.setCharges(1);
    ...
    private Item findPuzzleItem() {
        ...
        final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> item = Utils.findMatching(entrySet, entry -> entry.getValue().getId() == puzzleBox);
    ...
    private void shuffle() {
        final it.unimi.dsi.fastutil.ints.IntArrayList availableMoves = new IntArrayList(4);
