    ...
    public void handle() {
        bind("Open", (player, item, container, slotId) -> {
            ...
            if (item.getNumericAttribute("The Mimic slain").intValue() == 1) {
                ...
                final com.zenyte.game.content.treasuretrails.ClueLevel tier = Objects.requireNonNull(ClueReward.getTier(originalCasketId));
                ...
                player.getInventory().deleteItem(slotId, item);
                final com.zenyte.game.content.treasuretrails.rewards.ClueRewardTable rewards = Objects.requireNonNull(ClueReward.getTable(originalCasketId));
                final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.item.Item> loot = new ObjectArrayList<Item>();
                if (isRingLoot) {
                    loot.add(new Item(ItemId.RING_OF_3RD_AGE));
                }
                final int rate = 228 + ((6 - remainingRolls) * 22);
                if (Utils.random(rate - 1) == 0) {
                    final com.zenyte.game.item.ImmutableItem randomBonusLoot = thirdAgeLoot.get(Utils.random(thirdAgeLoot.size() - 1));
                    final com.zenyte.game.item.Item element = new Item(randomBonusLoot.getId());
                    loot.add(element);
                    ...
                    if (Utils.random(1) == 0) {
                        final com.zenyte.game.item.ImmutableItem randomBonusLoot = bonusLoot.get(Utils.random(bonusLoot.size() - 1));
                        final com.zenyte.game.item.Item element = new Item(randomBonusLoot.getId(), Utils.random(1) == 0 ? randomBonusLoot.getMinAmount() : randomBonusLoot.getMaxAmount());
                        loot.add(element);
                        player.sendMessage("The Mimic adds a little extra reward for you: " + Colour.RED.wrap(element.getAmount() + " x " + element.getName()));
                    }
                }
