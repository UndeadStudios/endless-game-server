...
public class StrangeDevice extends ItemPlugin {
    ...
    public void handle() {
        bind("Feel", (player, item, container, slotId) -> {
            final java.util.Optional<com.zenyte.game.content.treasuretrails.HotColdResult> result = TreasureTrail.getHotColdResult(player, item.getId() == ItemId.STRANGE_DEVICE_23183);
            if (!result.isPresent()) {
                player.getDialogueManager().start(new ItemChat(player, item, "The strange device is inactive."));
                return;
            }
            final com.zenyte.game.content.treasuretrails.HotColdResult res = result.get();
            final com.zenyte.game.content.treasuretrails.DeviceTemperature temperature = res.getTemperature();
            final com.zenyte.game.item.Item clueItem = res.getClue();
            ...
            final java.lang.Object lastDistance = clueItem.getAttribute("Hot Cold Distance");
