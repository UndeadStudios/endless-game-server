...
public abstract class ClueProcessor extends DropProcessor {
    ...
    public void attach() {
        for (final int i : allIds) {
            ...
            final java.util.List<com.zenyte.game.content.treasuretrails.npcs.drops.PredicatedClueDrop> clueNPCs = map().get(definitions.getName().toLowerCase());
            final PredicatedClueDrop clueNPC = Utils.findMatching(clueNPCs, n -> {
    ...
    public void onDeath(final NPC npc, final Player killer) {
        final mgi.types.config.npcs.NPCDefinitions definitions = npc.getDefinitions();
        final java.util.List<com.zenyte.game.content.treasuretrails.npcs.drops.PredicatedClueDrop> list = map().get(definitions.getLowercaseName());
        if (list == null || list.isEmpty()) {
            return;
        }
        final com.zenyte.game.content.treasuretrails.npcs.drops.PredicatedClueDrop constant = Utils.findMatching(list, n -> {
            final Predicate<NPCDefinitions> predicate = n.getPredicate();
            return predicate == null || predicate.test(definitions);
        });
        if (constant == null) {
        ...
        final int itemId = itemId();
        final com.zenyte.game.item.Item ring = killer.getEquipment().getItem(EquipmentSlot.RING);
        final boolean isImbuedRingOfWealth = ring != null && ring.getName().startsWith("Ring of wealth (i");
        final boolean boosted = itemId != ClueItem.BEGINNER.getScrollBox() && isImbuedRingOfWealth && WildernessArea.isWithinWilderness(npc.getX(), npc.getY());
        ...
        if (Utils.randomDouble() < 1.0F / rate) {
    ...
    public int[] ids() {
        ...
        for (int i = 0; i < Utils.getIndiceSize(Indice.NPC_DEFINITIONS); i++) {
            ...
            final java.util.List<com.zenyte.game.content.treasuretrails.npcs.drops.PredicatedClueDrop> clueNPCs = map().get(name);
            if (clueNPCs == null) {
                continue;
            }
            for (final com.zenyte.game.content.treasuretrails.npcs.drops.PredicatedClueDrop npc : clueNPCs) {
                final java.util.function.Predicate<mgi.types.config.npcs.NPCDefinitions> predicate = npc.getPredicate();
