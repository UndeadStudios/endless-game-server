...
public class StashUnitInterface extends Interface {
    ...
    private final void iterateEnums(@NotNull final IntEnum intEnum, @NotNull final Stash stash, @NotNull final IntConsumer consumer) {
        for (final it.unimi.dsi.fastutil.ints.Int2IntMap.Entry entry : intEnum.getValues().int2IntEntrySet()) {
            final int objectId = entry.getIntKey();
            final int coordGrid = entry.getIntValue();
            final com.zenyte.game.content.treasuretrails.stash.StashUnit unit = StashUnit.getMap().get(objectId);
            if (unit == null) {
    ...
    public void open(Player player) {
        ...
        final mgi.types.config.enums.IntEnum containerEnum = Enums.STASH_UNIT_BUILD_STAGES_CONTAINER;
        final com.zenyte.game.world.entity.player.container.Container container = new Container(ContainerPolicy.NEVER_STACK, ContainerType.STASH_UNIT_BUILD_STAGES, Optional.empty());
        ...
        final mgi.types.config.enums.IntEnum varsEnum = Enums.STASH_UNIT_BUILD_STAGES_VARS;
