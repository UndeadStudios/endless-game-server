...
public class FaloTheBardNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handleItemOnNPCAction(final Player player, final Item item, final int slot, final NPC npc) {
        ...
        if (list == null) {
            final java.util.Optional<com.zenyte.game.content.treasuretrails.ClueEntry> clue = TreasureTrail.findFaloClue(player);
            if (!clue.isPresent()) {
                player.sendFilteredMessage("Nothing interesting happens.");
                return;
            }
            final com.zenyte.game.content.treasuretrails.ClueEntry scroll = clue.get();
            final com.zenyte.game.content.treasuretrails.challenges.FaloTheBardChallenge challenge = (FaloTheBardChallenge) scroll.getChallenge();
            player.getDialogueManager().start(new Dialogue(player, npc) {
                @Override
                public void buildDialogue() {
                    item(item, "You show your object to Falo.");
                    final com.zenyte.game.content.treasuretrails.clues.emote.ItemRequirement[] requirements = challenge.getTask().getItemRequirements();
    ...
    private void talk(@NotNull final Player player, @NotNull final NPC npc) {
        player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                final com.zenyte.game.content.treasuretrails.challenges.FaloTheBardChallenge challenge = (FaloTheBardChallenge) scroll.getChallenge();
                final com.zenyte.game.item.Item clueItem = scroll.getItem();
