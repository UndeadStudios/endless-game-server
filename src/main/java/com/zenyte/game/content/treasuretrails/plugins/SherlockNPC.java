...
public class SherlockNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            @Override
            public void buildDialogue() {
                final java.util.Optional<com.zenyte.game.content.treasuretrails.ClueEntry> entry = TreasureTrail.findSherlockClue(player);
                if (!entry.isPresent()) {
                    npc("Good day, " + player.getName() + ".");
                    return;
                }
                final com.zenyte.game.item.Item item = entry.get().getItem();
                final java.util.List<java.lang.String> list = TreasureTrail.getCluesList(item);
                assert list != null;
                assert list.size() >= 1;
                final java.lang.String constantName = list.get(0);
