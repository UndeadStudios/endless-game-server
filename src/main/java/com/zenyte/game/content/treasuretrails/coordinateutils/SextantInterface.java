...
public class SextantInterface extends Interface {
    ...
    public void open(Player player) {
        ...
        final com.zenyte.game.content.treasuretrails.coordinateutils.SextantPosition sextantPosition = new SextantPosition(framePosition, armPosition);
    ...
    private final void refreshSextantPosition(@NotNull final Player player) {
        final com.zenyte.game.content.treasuretrails.coordinateutils.SextantPosition sextantPosition = getSextantPosition(player);
    ...
    private final void sendCoordinates(@NotNull final Player player) {
        ...
        final java.lang.String longitudeMessage = longitudeDegrees + " degree" + (longitudeDegrees == 1 ? "" : "s") + " " + longitudeMinutes + " minute" + (longitudeMinutes == 1 ? "" : "s") + " " + (south ? "south" : "north");
        final java.lang.String latitudeMessage = latitudeDegrees + " degree" + (latitudeDegrees == 1 ? "" : "s") + " " + latitudeMinutes + " minute" + (latitudeMinutes == 1 ? "" : "s") + " " + (west ? "west" : "east");
    ...
    private final void move(@NotNull final Player player, @MagicConstant(intValues = {-1, 1}) final int horizontalOffset, @MagicConstant(intValues = {-1, 0, 1}) final int verticalOffset) {
        final com.zenyte.game.content.treasuretrails.coordinateutils.SextantPosition currentSextantPosition = getSextantPosition(player);
        ...
        final com.zenyte.game.content.treasuretrails.coordinateutils.SextantPosition newSextantPosition = new SextantPosition(framePosition, horizontalConditionalOffset == 0 ? armPosition : (Math.max(framePosition, Math.min(SextantPosition.ARM_POSITIONS_COUNT - (SextantPosition.FRAME_POSITIONS_COUNT - 2 - framePosition), armPosition))));
