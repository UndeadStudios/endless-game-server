...
public abstract class ClueRewardTable {
    ...
    public void calculate() {
        ...
        defineTables();
        for (final com.zenyte.game.item.ImmutableItem reward : rewards) {
            totalWeight += (int) reward.getRate();
        }
        for (final com.zenyte.game.item.ImmutableItem reward : entranaRewards) {
    ...
    protected void appendTable(final int rarity, final List<ImmutableItem> table) {
        ...
        for (final com.zenyte.game.item.ImmutableItem item : table) {
    ...
    public final List<Item> roll(final int min, final int max, final boolean entrana) {
        assert min <= max;
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.item.Item> loot = new ObjectArrayList<Item>(max);
        final int count = Utils.random(min, max);
        for (int i = 0; i < count; i++) {
            final int roll = Utils.random(entrana ? totalEntranaWeight : totalWeight);
            int current = 0;
            for (final com.zenyte.game.item.ImmutableItem item : (entrana ? entranaRewards : rewards)) {
