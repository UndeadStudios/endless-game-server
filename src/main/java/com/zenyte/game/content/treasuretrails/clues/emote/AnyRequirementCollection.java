...
public class AnyRequirementCollection implements ItemRequirement {
    ...
    public IntArrayList getFulfillingItemsIndexes(Item[] items) {
        for (final com.zenyte.game.content.treasuretrails.clues.emote.ItemRequirement requirement : requirements) {
            final it.unimi.dsi.fastutil.ints.IntArrayList indexes = requirement.getFulfillingItemsIndexes(items);
            if (indexes != null) {
                return indexes;
            }
        }
    ...
    public String toString() {
        return "AnyRequirementCollection(name=" + this.name + ", requirements=" + java.util.Arrays.deepToString(this.requirements) + ")";
    }
}
