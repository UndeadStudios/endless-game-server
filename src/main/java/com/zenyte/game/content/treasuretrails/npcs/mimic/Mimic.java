...
final class Mimic extends NPC implements CombatScript {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(Mimic.class);
    ...
    private List<Location> findLocations(@NotNull final Location tile) {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.Location> list = new ObjectArrayList<Location>();
        int tryCount = 500;
        list.add(tile);
        while (list.size() < 6) {
            if (--tryCount <= 0) {
            ...
            final com.zenyte.game.world.entity.Location location = new Location(tile, 3);
    ...
    private void clearThirdAgeMonsters() {
        for (final com.zenyte.game.content.treasuretrails.npcs.mimic.ThirdAgeMonster monster : thirdAgeMonsters) {
    ...
    private void iterateCollidingPlayers() {
        for (final com.zenyte.game.world.entity.player.Player player : instance.getPlayers()) {
    ...
    public int attack(@NotNull Entity target) {
        ...
        if (attackType == 2) {
            ...
            WorldTasksManager.schedule(() -> {
                final java.util.List<com.zenyte.game.world.entity.Location> tiles = findLocations(new Location(target.getLocation()));
                for (int i = tiles.size() - 1; i >= 0; i--) {
                    final com.zenyte.game.content.treasuretrails.npcs.mimic.Mimic.Candy candy = Candy.immutableCandyList.get(i);
