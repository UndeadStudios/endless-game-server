	...
	private static final class ConfigurationD extends Dialogue {
		...
		public void buildDialogue() {
			options("Select a jar to display.",
					...
					"Next selection.")
			.onOptionOne(() -> { handleJar(12007); finish(); })
			.onOptionTwo(() -> { handleJar(12885); finish(); })
			.onOptionThree(() -> { handleJar(12936); finish(); })
			.onOptionFour(() -> { handleJar(13245); finish(); })
			...
			options(5, "Select a jar to display.",
					...
					"Previous selection.")
			.onOptionOne(() -> { handleJar(13277); finish(); })
			.onOptionTwo(() -> { handleJar(19701); finish(); })
			.onOptionThree(() -> { handleJar(21745); finish(); })
			.onOptionFour(() -> { handleJar(22106); finish(); })
