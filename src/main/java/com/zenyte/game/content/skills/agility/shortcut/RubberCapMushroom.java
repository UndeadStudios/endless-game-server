    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        final Runnable runnable = () -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                options("You might hurt yourself if you do that", new DialogueOption("I don\'t care, I\'m doing it.", () -> handle(player, object, 0)), new DialogueOption("I\'d better not in that case"));
            }
        });
