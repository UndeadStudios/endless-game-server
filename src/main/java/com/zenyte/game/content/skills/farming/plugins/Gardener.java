...
public class Gardener extends NPCPlugin {
    ...
    public void handle() {
        bindOptions(option -> option.startsWith("Pay"), (player, npc, option) -> {
            final com.zenyte.game.content.skills.farming.FarmingPatch patch = FarmingPatch.getPatchByGardener(npc.getId(), option.getId()).orElseThrow(RuntimeException::new);
            final com.zenyte.game.content.skills.farming.FarmingSpot spot = player.getFarming().create(patch);
        ...
        bind("Talk-to", (player, npc) -> {
            ...
            final java.util.Set<com.zenyte.game.content.skills.farming.FarmingPatch> set = FarmingPatch.getPatchSetByGardeners(npc.getId()).orElseThrow(RuntimeException::new);
            assert !set.isEmpty();
            //If multiple patches tied to the gardener, it is guaranteed to be an allotment patch.
            final boolean multiple = set.size() > 1;
            final com.zenyte.game.content.skills.farming.FarmingPatch patch = set.stream().findFirst().orElseThrow(RuntimeException::new);
            final com.zenyte.game.content.skills.farming.FarmingSpot spot = player.getFarming().create(patch);
            final com.zenyte.game.content.skills.farming.PatchState state = spot.getState();
            ...
            player.getDialogueManager().start(new Dialogue(player, npc) {
                ...
                private void buildWatchOverOptions() {
                    final com.zenyte.game.world.entity.player.dialogue.PlayerMessage message = player(5, "Would you look after my crops for me?");
                    if (multiple) {
                        ...
                        for (final com.zenyte.game.content.skills.farming.FarmingPatch patch : set) {
    ...
    private void startPaymentDialogue(final Player player, final NPC npc, final FarmingPatch patch, final boolean quickPay) {
        ...
        player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                final com.zenyte.game.item.Item[] payment = product.getPayment();
                ...
                for (final com.zenyte.game.item.Item item : payment) {
                    index++;
                    if (item == null) continue;
                    final java.lang.String prefix = length > 1 && index == (length - 1) ? " and " : ", ";
                ...
                final java.lang.String pay = builder.delete(builder.length() - 2, builder.length()).toString();
