...
public class SeedVault {
    ...
    public static final void onInit(final InitializationEvent event) {
        ...
        final com.zenyte.game.content.skills.farming.seedvault.SeedVault savedVault = savedPlayer.getSeedVault();
    ...
    public void deposit(final Container fromContainer, final int fromSlot, final int amount, boolean notifyOnFailure) {
        ...
        final com.zenyte.game.item.Item item = fromContainer.get(fromSlot);
        if (!canDeposit(item)) {
            if (notifyOnFailure) {
