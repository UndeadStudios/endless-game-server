...
public final class MonsterExamine extends Interface implements NPCSpell {
		...
		boolean pathfind() {
			...
			if (player.isProjectileClipped(target, false) || !withinRange(target, target.getSize()) || (target.hasWalkSteps() && !withinRange(target.getNextPosition(target.isRun() ? 2 : 1), target.getSize()))) {
				...
				if (player.getWalkSteps().size() >= 2) {
					...
					final com.zenyte.game.world.entity.Location nextPos = player.getNextPosition(1);
	...
	private static final String getStats(final NPC npc) {
		final com.zenyte.game.world.entity.npc.combatdefs.NPCCombatDefinitions combat = npc.getCombatDefinitions();
		final com.zenyte.game.world.entity.npc.combatdefs.StatDefinitions statDefs = combat.getStatDefinitions();
	...
	public void open(Player player) {
		final java.lang.Object attr = player.getTemporaryAttributes().get("monster examine npc");
		if (!(attr instanceof NPC)) {
			throw new RuntimeException("Opening monster examine illegally.");
		}
		final com.zenyte.game.world.entity.npc.NPC npc = (NPC) attr;
