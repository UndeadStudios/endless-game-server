		...
		player.getDialogueManager().start(new Dialogue(player) {
			...
			public void buildDialogue() {
				options("What would you like to take?",
						...
						"A pot of flour.",
						"Next selection.")
				.onOptionOne(() -> { addItem(player, 0); finish(); })
				.onOptionTwo(() -> { addItem(player, 1); finish(); })
				.onOptionThree(() -> { addItem(player, 2); finish(); })
				.onOptionFour(() -> { addItem(player, 3); finish(); })
				...
				options(5, "What would you like to take?",
						...
						"Cheese.",
						"Previous selection.")
				.onOptionOne(() -> { addItem(player, 4); finish(); })
				.onOptionTwo(() -> { addItem(player, 5); finish(); })
				.onOptionThree(() -> { addItem(player, 6); finish(); })
				.onOptionFour(() -> { addItem(player, 7); finish(); })
