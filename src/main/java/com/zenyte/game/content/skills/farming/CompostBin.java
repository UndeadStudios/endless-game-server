...
public class CompostBin {
    ...
    static {
        for (final com.zenyte.game.content.skills.farming.FarmingProduct product : FarmingProduct.supercompostableProducts) {
        ...
        for (final com.zenyte.game.content.skills.farming.FarmingProduct prod : FarmingProduct.values()) {
            final com.zenyte.game.item.Item product = prod.getProduct();
            if (product != null) {
        ...
        for (final com.zenyte.game.content.skills.farming.FarmingProduct ignored : ignoredProducts) {
            final com.zenyte.game.item.Item product = ignored.getProduct();
            if (product != null) {
    ...
    public int add(@NotNull final Item item) {
        final com.zenyte.game.item.Item it = new Item(item.getId(), 1);
        int count;
    ...
    private boolean isRottenTomatoes() {
        for (final com.zenyte.game.item.Item item : contents) {
            if (item.getId() != 1982) {
                return false;
            }
        }
    ...
    public CompostBin(final boolean isBig) {
        this.isBig = isBig;
    }
    ...
    public static IntArrayList getCompostableItems() {
        return CompostBin.compostableItems;
    }

    public static IntArrayList getSupercompostableItems() {
        return CompostBin.supercompostableItems;
    }

    public void setType(final CompostBinType type) {
        this.type = type;
    }
}
