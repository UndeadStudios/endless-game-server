...
public class BirdhouseObjectPlugin implements ItemOnObjectAction, ObjectAction {
    ...
    public void handleItemOnObjectAction(Player player, Item item, int slot, WorldObject object) {
        final com.zenyte.game.content.skills.hunter.Hunter hunter = player.getHunter();
        final java.util.Optional<com.zenyte.game.content.skills.hunter.object.Birdhouse> existingBirdhouse = hunter.findBirdhouse(object.getId());
    ...
    private final void build(@NotNull final Player player, @NotNull final Item item, @NotNull final OptionalInt slot, final WorldObject object) {
        ...
        final com.zenyte.game.content.skills.hunter.Hunter hunter = player.getHunter();
        final com.zenyte.game.content.skills.hunter.node.BirdHouseType type = BirdHouseType.findThroughBirdhouse(item.getId()).orElseThrow(RuntimeException::new);
        final com.zenyte.game.content.skills.hunter.BirdHousePosition position = BirdHousePosition.findPosition(object.getId()).orElseThrow(RuntimeException::new);
        ...
        final com.zenyte.game.content.skills.hunter.object.Birdhouse birdhouse = new Birdhouse(player).setState(BirdHouseState.EMPTY).setType(type).setPosition(position).setSeedsFilled(0);
    ...
    public Object[] getItems() {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<java.lang.Object> list = new ObjectArrayList<>();
        for (final com.zenyte.game.content.skills.hunter.node.BirdHouseType birdhouse : BirdHouseType.getValues()) {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        if (option.equals("Build")) {
            final java.util.Optional<com.zenyte.game.content.skills.hunter.node.BirdHouseType> bestBirdhouse = findBestBirdhouse(player);
        ...
        final com.zenyte.game.content.skills.hunter.object.Birdhouse birdhouse = player.getHunter().findBirdhouse(object.getId()).orElseThrow(RuntimeException::new);
        switch (option) {
        case "Seeds": 
    ...
    private final void dismantle(@NotNull final Player player, @NotNull final Birdhouse birdhouse) {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.item.Item> list = new ObjectArrayList<Item>();
        list.add(new Item(ItemId.CLOCKWORK));
        if (progress == 1) {
            final com.zenyte.game.content.skills.hunter.node.BirdHouseType type = birdhouse.getType();
            ...
            if (Utils.random(399) == 0) {
                final java.util.OptionalInt nestConstant = ClueItem.pseudoRandomNestConstant();
                if (nestConstant.isPresent()) {
                    final com.zenyte.game.item.Item skillingItem = new Item(nestConstant.getAsInt());
                    list.add(skillingItem);
                    final java.lang.String name = skillingItem.getName().toLowerCase();
                    final java.lang.String itemTypeName = name.substring(0, name.indexOf(" ("));
                    final java.lang.String clueTypeName = name.substring(name.indexOf(" (") + 2, name.length() - 1);
                    final java.lang.String prefix = Utils.getAOrAn(clueTypeName);
    ...
    private final void checkSeeds(@NotNull final Player player, @NotNull final Birdhouse birdhouse) {
        ...
        final com.zenyte.game.content.skills.hunter.node.BirdHouseState state = birdhouse.getState();
        switch (state) {
        case EMPTY: 
        case IN_PROGRESS: 
            final long timeRequired = TimeUnit.MINUTES.toMillis(birdhouse.getLengthInMinutes());
    ...
    private final Optional<BirdHouseType> findBestBirdhouse(@NotNull final Player player) {
        ...
        for (int i = 0; i < 28; i++) {
            ...
            final java.util.Optional<com.zenyte.game.content.skills.hunter.node.BirdHouseType> birdhouse = BirdHouseType.findThroughBirdhouse(item.getId());
            if (!birdhouse.isPresent()) {
                continue;
            }
            final com.zenyte.game.content.skills.hunter.node.BirdHouseType type = birdhouse.get();
