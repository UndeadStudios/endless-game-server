...
public class RollingBlock {
    ...
    private enum RollingBlockObject {
        ...
        private RollingBlockObject(final ImmutableLocation swTile) {
            this.swTile = swTile;
        }
    }
    ...
    public static final void roll(@NotNull final Player player, final int x, final int y) {
        ...
        for (final com.zenyte.game.content.skills.agility.pyramid.area.RollingBlock.RollingBlockObject block : RollingBlockObject.values) {
            final com.zenyte.game.world.entity.ImmutableLocation tile = block.swTile;
            ...
            final com.zenyte.game.world.object.WorldObject stoneObject = World.getObjectWithType(tile, 10);
            player.lock();
    ...
    private static void success(final Player player, final WorldObject object) {
        ...
        final com.zenyte.game.world.entity.masks.ForceMovement forceMovement = new ForceMovement(player.getLocation().transform(object.getFaceDirection(), 2), 30, object.getFaceDirection().getDirection());
    ...
    private static void failure(final Player player, final WorldObject blockObject, final boolean reverse) {
        ...
        final com.zenyte.game.world.entity.masks.ForceMovement forceMovement = createSlipMovement(player.getLocation(), blockObject);
        player.setForceMovement(forceMovement);
        WorldTasksManager.schedule(() -> {
    ...
    private static ForceMovement createSlipMovement(final Location location, final WorldObject object) {
        final com.zenyte.game.util.Direction dir = object.getRotation() == 1 ? Direction.WEST : object.getRotation() == 0 ? Direction.SOUTH : object.getRotation() == 2 ? Direction.NORTH : Direction.EAST;
        final com.zenyte.game.world.entity.Location destination = location.transform(dir, 2).transform(0, 0, -1);
