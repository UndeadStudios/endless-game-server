...
public final class Saturating extends Action {
    ...
    public boolean start() {
        ...
        if (compost.isPresent()) {
            final com.zenyte.game.content.skills.farming.PatchFlag flag = getCompostType(item);
            assert flag != null;
    ...
    public int processWithDelay() {
        final com.zenyte.game.content.skills.farming.PatchFlag flag = getCompostType(item);
        assert flag != null;
        player.getSkills().addXp(Skills.FARMING, flag == PatchFlag.COMPOST ? 18 : flag == PatchFlag.SUPERCOMPOST ? 26 : 36);
        if (item.getId() == 22994 || item.getId() == 22997) {
