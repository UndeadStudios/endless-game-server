...
public final class Humidify implements DefaultSpell, NPCSpell {
	...
	private final boolean spellEffect(final Player player) {
		...
		for (int i = 0; i < 28; i++) {
			...
			final java.lang.Integer response = VESSELS.get(item.getId());
			if (response == null) {
				continue;
			}
			final com.zenyte.game.item.Item it = new Item(response);
			item.setId(it.getId());
	...
	public boolean spellEffect(final Player player, final NPC npc) {
		...
		final com.zenyte.game.world.Projectile projectile = new Projectile(1669, 0, 0, 0, 0, 60, 0, 0);
