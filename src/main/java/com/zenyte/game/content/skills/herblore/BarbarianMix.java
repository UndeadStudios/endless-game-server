...
public class BarbarianMix implements ItemOnItemAction {
	...
	private static enum Mix {
		...
		static {
			for (final com.zenyte.game.content.skills.herblore.BarbarianMix.Mix mix : VALUES) {
		...
		private Mix(final int basePotion, final int produce, final int level, final float experience) {
	...
	public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
		final com.zenyte.game.item.Item eggs = from.getId() == ROE || from.getId() == CAVIAR ? from : to;
		final com.zenyte.game.item.Item potion = eggs == from ? to : from;
		final com.zenyte.game.content.skills.herblore.BarbarianMix.Mix mix = Mix.MAPPED_VALUES.get(potion.getId());
	...
	public ItemPair[] getMatchingPairs() {
		final java.util.ArrayList<com.zenyte.game.item.ItemOnItemAction.ItemPair> list = new ArrayList<ItemPair>(Mix.VALUES.length + 6);
		for (final com.zenyte.game.content.skills.herblore.BarbarianMix.Mix mix : Mix.VALUES) {
			if (mix.level <= 24) {
