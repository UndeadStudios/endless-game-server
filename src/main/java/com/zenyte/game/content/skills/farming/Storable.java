...
public enum Storable {
    ...
    SECATEURS(11, 100, 1438, 7409, 5329) {
        ...
        void _refresh(@NotNull final Player player, final Item item) {
            ...
            manager.sendBit(1438, amount & 1);
        }
    },
    WATERING_CAN(12, 1, -1, Enums.FARMING_WATERING_CANS.getValues().values().toIntArray()) {
        @Override
        void _refresh(@NotNull final Player player, final Item item) {
            player.getVarManager().sendBit(1439, item.getAmount() == 0 ? 0 : Enums.FARMING_WATERING_CANS.getKey(item.getId()).orElseThrow(Enums.exception()));
        }
    },
    GARDENING_TROWEL(13, 100, 1440, 5325) {
        ...
        void _refresh(@NotNull final Player player, final Item item) {
            ...
            manager.sendBit(1440, amount & 1);
        }
    },
    PLANT_CURE(14, 1000, 6268, 6036), BOTTOMLESS_BUCKET(15, 1, -1, 22997, 22994) {
        @Override
        void _refresh(@NotNull final Player player, final Item item) {
            final VarManager manager = player.getVarManager();
    ...
    EMPTY_BUCKET(16, 1000, -1, 1925) {
        ...
        void _refresh(@NotNull final Player player, final Item item) {
            ...
            manager.sendBit(4731, (amount & 255) / (int) Math.pow(2, 5));
            manager.sendBit(1441, amount & 31);
        }
    },
    NORMAL_COMPOST(17, 1000, -1, 6032) {
        ...
        void _refresh(@NotNull final Player player, final Item item) {
            ...
            manager.sendBit(1442, amount & 255);
        }
    },
    SUPER_COMPOST(18, 1000, -1, 6034) {
        ...
        void _refresh(@NotNull final Player player, final Item item) {
            ...
            manager.sendBit(1443, amount & 255);
        }
    },
