...
public class CatchImplingAction extends Action {
    ...
    private boolean initiateCombat(final Player player) {
        ...
        final com.zenyte.game.content.skills.hunter.npc.ImplingNPC target = npc;
    ...
    protected boolean isWithinAttackDistance() {
        final com.zenyte.game.content.skills.hunter.npc.ImplingNPC target = npc;
        final com.zenyte.game.world.entity.Location nextTile = target.getLocation();
        final com.zenyte.game.world.entity.Location nextPosition = player.getNextPosition(player.isRun() ? 2 : 1);
        ...
        if ((player.isFrozen() || player.isStunned()) && (Utils.collides(nextPosition.getX(), nextPosition.getY(), player.getSize(), nextLocation.getX(), nextLocation.getY(), target.getSize()) || !withinRange(target, target.getSize()))) {
            return false;
        }
    ...
    private boolean attemptCatch() {
        ...
        WorldTasksManager.schedule(() -> {
            ...
            if (success) {
                ...
                final java.lang.String key = (isPuro ? Impling.PURO_IMPLING_TRACKER_ATTRIBUTE_KEY : Impling.SURFACE_IMPLING_TRACKER_ATTRIBUTE_KEY) + impling.getNpcId();
    ...
    public CatchImplingAction(final Impling impling, final ImplingNPC npc) {
