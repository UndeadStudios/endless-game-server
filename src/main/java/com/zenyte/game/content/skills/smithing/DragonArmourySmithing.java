...
public class DragonArmourySmithing extends Action {
	...
	public int processWithDelay() {
		...
		final com.zenyte.game.item.Item product = item.equals("Kiteshield") ? DRAGON_KITESHIELD : DRAGON_PLATEBODY;
		player.setAnimation(Smithing.ANIMATION);
		player.getSkills().addXp(Skills.SMITHING, experience);
		for (final com.zenyte.game.item.Item item : MATERIALS[item.equals("Kiteshield") ? 0 : 1]) {
	...
	public DragonArmourySmithing(final String item, final int amount) {
		this.item = item;
		this.amount = amount;
	}
}
