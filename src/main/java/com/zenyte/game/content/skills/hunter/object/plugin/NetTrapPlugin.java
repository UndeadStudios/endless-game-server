...
public class NetTrapPlugin implements ObjectAction, ItemOnObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        ...
        case "Check": 
            {
                ...
                final com.zenyte.game.content.skills.hunter.node.TrapType type = TrapType.NET_TRAP_SWAMP_LIZARD;
                final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = HunterUtils.findTrap(type, object, null).orElseThrow(RuntimeException::new);
        ...
        case "Dismantle": 
            {
                final com.zenyte.game.content.skills.hunter.node.TrapType type = TrapType.NET_TRAP_SWAMP_LIZARD;
                final java.util.Optional<com.zenyte.game.content.skills.hunter.object.HunterTrap> failedTrap = HunterUtils.findTrap(type, object, null);
                ...
                final com.zenyte.game.content.skills.hunter.node.NetTrapPair pair = HunterUtils.findNetTrapPair(object);
                final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = HunterUtils.findTrap(type, pair.getNet(), null).orElseThrow(RuntimeException::new);
    ...
    public Object[] getObjects() {
        final it.unimi.dsi.fastutil.objects.ObjectOpenHashSet<java.lang.Object> set = new ObjectOpenHashSet<>();
        for (final com.zenyte.game.content.skills.hunter.node.NetTrap trap : NetTrap.getValues()) {
            set.add(trap.getYoungTree());
            set.add(trap.getBentYoungTree());
            final com.zenyte.game.content.skills.hunter.node.TrapType type = trap.getType();
            set.add(type.getObjectId());
            set.add(type.getCollapsedObjectId());
            set.add(type.getCollapsedAnimatedObjectId());
        }
        final com.zenyte.game.content.skills.hunter.node.TrapPrey[] prey = new TrapPrey[] {TrapPrey.SWAMP_LIZARD, TrapPrey.ORANGE_SALAMANDER, TrapPrey.RED_SALAMANDER, TrapPrey.BLACK_SALAMANDER};
        for (final com.zenyte.game.content.skills.hunter.node.TrapPrey preyType : prey) {
            final com.zenyte.game.content.skills.hunter.node.PreyObject objectType = Objects.requireNonNull(preyType.getObject());
