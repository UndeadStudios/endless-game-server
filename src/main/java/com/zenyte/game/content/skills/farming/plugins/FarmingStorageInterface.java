...
public class FarmingStorageInterface extends Interface {
    ...
    private static final void remove(@NotNull final Player player, @NotNull final Storable storable, int option) {
        final com.zenyte.game.content.skills.farming.FarmingStorage storage = player.getFarming().getStorage();
        final com.zenyte.game.content.skills.farming.plugins.FarmingStorageInterface.Option op = storage.getOption(option);
        final com.zenyte.game.item.Item item = storage.getItem(storable);
        switch (op) {
        case EXAMINE: 
            if (item == null) {
    ...
    public enum Option {
        ...
        private Option(final int optionId) {
