...
public class Clearing extends Action {
    ...
    public int processWithDelay() {
        ...
        if ((tree && spot.getProduct().getTreeDefinitions() == TreeDefinitions.FRUIT_TREE) || Utils.random(5) <= 3) {
            ...
            final com.zenyte.game.content.skills.farming.FarmingProduct product = spot.getProduct();
            final com.zenyte.game.content.skills.farming.PatchType type = product.getType();
            ...
            if (type == PatchType.HESPORI_PATCH) {
                if (value == 8) {
                    final com.zenyte.game.item.Item seed = new Item(22881 + ((Utils.random(2) * 2)), Utils.random(1, 2));
                    ...
                    NPCDrops.forEach(NPCDrops.getTable(8583), drop -> {
                        final com.zenyte.game.item.Item item = new Item(drop.getItemId(), Utils.random(drop.getMinAmount(), drop.getMaxAmount()));
    ...
    private final int getRootsAmount() {
        final int level = player.getSkills().getLevelForXp(Skills.FARMING);
