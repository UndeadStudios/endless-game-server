	...
	private static final class TopiarySelectionD extends Dialogue {
		...
		public void buildDialogue() {
			options("Select a type.",
					...
					"Cerberus topiary.",
					"Next selection.")
			.onOptionOne(() -> { clipTopiary(0); finish(); })
			.onOptionTwo(() -> { clipTopiary(1); finish(); })
			.onOptionThree(() -> { clipTopiary(2); finish(); })
			.onOptionFour(() -> { clipTopiary(3); finish(); })
			...
			options(5, "Select a type.",
					...
					"Vorkath topiary.",
					"Previous selection.")
			.onOptionOne(() -> { clipTopiary(4); finish(); })
			.onOptionTwo(() -> { clipTopiary(5); finish(); })
			.onOptionThree(() -> { clipTopiary(6); finish(); })
