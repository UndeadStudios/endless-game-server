...
public class GuildmasterJaneDialogue extends Dialogue {
    ...
    public void buildDialogue() {
        final com.zenyte.game.content.skills.farming.contract.FarmingContract easyContract = FarmingContract.generateContract(player, FarmingContractDifficulty.EASY);
        final com.zenyte.game.content.skills.farming.contract.FarmingContract mediumContract = FarmingContract.generateContract(player, FarmingContractDifficulty.MEDIUM);
        final com.zenyte.game.content.skills.farming.contract.FarmingContract hardContract = FarmingContract.generateContract(player, FarmingContractDifficulty.HARD);
        ...
        if (dialogueType == JaneDialogueType.FULL) {
            ...
            if (player.getSkills().getLevel(Skills.FARMING) < FarmingContract.MINIMUM_LEVEL) {
                ...
                final com.zenyte.game.world.entity.player.dialogue.Dialogue.DialogueOption secondOption = contractCompleted ? new DialogueOption("I\'ve completed my farming contract.", key(COMPLETED_CONTRACT_KEY)) : new DialogueOption("I\'d like a farming contract.", key(ASK_CONTRACT_KEY));
    ...
    private void askForContract(final int startStage, final FarmingContract easyContract, final FarmingContract mediumContract, final FarmingContract hardContract) {
        final java.lang.String askForContractString = dialogueType == JaneDialogueType.NEW_CONTRACT ? "Yes please." : "I\'d like a farming contract.";
        ...
        if (player.getAttributes().get(FarmingContract.CONTRACT_ATTR) != null) {
            final com.zenyte.game.content.skills.farming.contract.FarmingContract currentContract = FarmingContract.getByName((String) player.getAttributes().get(FarmingContract.CONTRACT_ATTR));
            final java.lang.String productName = getFormattedName(currentContract);
        ...
        npc(EASY_ASSIGNED_CONTRACT_KEY, "Please could you grow " + getFormattedName(easyContract) + " for us? I\'ll reward you once you have " + getCompletionType(easyContract)).executeAction(() -> setKey(OPTIONS_AFTER_CONTRACT_KEY));
        npc(MEDIUM_ASSIGNED_CONTRACT_KEY, "Please could you grow " + getFormattedName(mediumContract) + " for us? I\'ll reward you once you have " + getCompletionType(mediumContract)).executeAction(() -> setKey(OPTIONS_AFTER_CONTRACT_KEY));
        npc(HARD_ASSIGNED_CONTRACT_KEY, "Please could you grow " + getFormattedName(hardContract) + " for us? I\'ll reward you once you have " + getCompletionType(hardContract)).executeAction(() -> setKey(OPTIONS_AFTER_CONTRACT_KEY));
    }
    ...
    private void completedContract(final int startStage) {
        ...
        npc("You\'ll be wanting a reward then.").executeAction(() -> {
            final com.zenyte.game.content.skills.farming.contract.FarmingContract currentContract = FarmingContract.getByName(player.getAttributes().get(FarmingContract.CONTRACT_ATTR).toString());
    ...
    private void anythingEasier(final FarmingContract easyContract, final FarmingContract mediumContract) {
        ...
        player(EASIER_CONTRACT_KEY, "Do you have anything easier?").executeAction(() -> {
            final com.zenyte.game.content.skills.farming.contract.FarmingContractDifficulty currentDifficulty = FarmingContractDifficulty.getByName((String) player.getAttributes().get(FarmingContract.CONTRACT_DIFFICULTY_ATTR));
            if (currentDifficulty == FarmingContractDifficulty.EASY) {
                setKey(NO_EASIER_CONTRACT_KEY);
            } else {
                final com.zenyte.game.content.skills.farming.contract.FarmingContractDifficulty newDifficulty = currentDifficulty.decrease(1);
