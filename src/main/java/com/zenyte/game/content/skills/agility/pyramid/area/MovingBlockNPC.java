...
public class MovingBlockNPC extends NPC {
    ...
    public void onMovement() {
        final com.zenyte.game.content.skills.agility.pyramid.area.AgilityPyramidArea area = GlobalAreaManager.getArea(AgilityPyramidArea.class);
        final java.util.Set<com.zenyte.game.world.entity.player.Player> players = area.getPlayers();
        if (players.isEmpty()) {
            return;
        }
        for (final com.zenyte.game.world.entity.player.Player player : players) {
            ...
            final com.zenyte.game.world.entity.Location destTile = new Location(horizontal ? (respawnTile.getX() + 4) : player.getX(), horizontal ? player.getY() : (respawnTile.getY() + 4), player.getPlane() - 1);
    ...
    void slide(@NotNull final Direction direction) {
        final com.zenyte.game.world.entity.Location destination = getLocation().transform(direction, 2);
    ...
    private void push(final Player player, final Location destination) {
        ...
        final com.zenyte.game.util.Direction backwardsDirection = spawnDirection.getCounterClockwiseDirection(4);
