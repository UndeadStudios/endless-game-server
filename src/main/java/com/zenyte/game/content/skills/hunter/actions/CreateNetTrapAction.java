...
public class CreateNetTrapAction extends ConstructableHunterTrapAction {
    ...
    public boolean start() {
        ...
        final com.zenyte.game.content.skills.hunter.node.TrapType type = trapConstant.getType();
    ...
    public int processWithDelay() {
        ...
        final com.zenyte.game.content.skills.hunter.object.HunterTrap existingPossibleTrap = HunterUtils.findTrap(BOX_TRAP, netTrap, null).orElseGet(() -> HunterUtils.findTrap(BIRD_SNARE, netTrap, null).orElse(null));
        if (existingPossibleTrap != null) {
            final java.lang.Runnable runnable = existingPossibleTrap.getRemovalRunnable();
            if (runnable != null) {
                runnable.run();
            } else {
        ...
        final com.zenyte.game.content.skills.hunter.node.TrapType type = trapConstant.getType();
        final com.zenyte.game.content.skills.hunter.npc.HunterDummyNPC dummy = new HunterDummyNPC(type.getActiveDummyNpcId(), netTrap);
        final com.zenyte.game.world.object.WorldObject bentYoungTree = new WorldObject(trapConstant.getBentYoungTree(), this.youngTree.getType(), this.youngTree.getRotation(), this.youngTree);
        final int rotation = this.youngTree.getRotation();
        final com.zenyte.game.world.object.WorldObject offsetLocation = rotation == 0 || rotation == 1 ? this.youngTree : this.netTrap;
        final com.zenyte.game.world.object.WorldObject collapsedAnimatedTree = new WorldObject(type.getCollapsedAnimatedObjectId(), this.youngTree.getType(), this.youngTree.getRotation(), offsetLocation);
        final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = new HunterTrap(player, dummy, this.youngTree, Collections.singletonList(this.youngTree), Arrays.asList(bentYoungTree, netTrap), Collections.singletonList(collapsedAnimatedTree), type);
        dummy.setTrap(trap);
        final Runnable consumer = () -> {
            ...
            final com.zenyte.game.world.entity.player.Player player = trap.getPlayer().get();
        ...
        final com.zenyte.game.world.object.WorldObject collapsedTree = new WorldObject(type.getCollapsedObjectId(), this.youngTree.getType(), this.youngTree.getRotation(), offsetLocation);
    ...
    public CreateNetTrapAction(final WorldObject youngTree) {
