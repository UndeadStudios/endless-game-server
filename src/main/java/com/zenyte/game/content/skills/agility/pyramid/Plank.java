...
public class Plank implements FailableAgilityPyramidObstacle, IrreversibleObject {
    ...
    private void cross(final Player player, final WorldObject object, final boolean success) {
        final com.zenyte.game.util.Direction walkDirection = object.getFaceDirection().getCounterClockwiseDirection(4);
        final com.zenyte.game.world.entity.Location destination = player.getLocation().transform(walkDirection, 5);
        final com.zenyte.game.world.entity.SoundEffect sound = new SoundEffect(2470, 1, 10, success ? 4 : 2);
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (!success) {
                    final com.zenyte.game.util.Direction fallDirection = object.getFaceDirection().getCounterClockwiseDirection(reverse ? 2 : 6);
                    final com.zenyte.game.world.entity.Location fallDestination = player.getLocation().transform(fallDirection, 2).transform(0, 0, -1);
                    if (ticks == 3) {
