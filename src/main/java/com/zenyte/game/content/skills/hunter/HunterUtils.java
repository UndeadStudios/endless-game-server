...
public class HunterUtils {
    ...
    private static final BiPredicate<Player, HunterDummyNPC> trapOwnershipPredicate = (player, npc) -> {
        final java.lang.ref.WeakReference<com.zenyte.game.content.skills.hunter.object.HunterTrap> associatedTrap = npc.getTrap();
        if (associatedTrap == null) {
            return false;
        }
        final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = associatedTrap.get();
        if (trap == null) {
            return false;
        }
    ...
    public static Optional<PitfallHunterNPC> getTeasedNPC(@NotNull final Player player) {
        final java.lang.Object attribute = player.getTemporaryAttributes().get(teasedNPCAttributeKey);
        if (!(attribute instanceof WeakReference)) {
            return Optional.empty();
        }
        final java.lang.ref.WeakReference<?> reference = (WeakReference<?>) attribute;
        final java.lang.Object object = reference.get();
    ...
    public static final Optional<HunterTrap> findTrap(@NotNull final TrapType trapType, @NotNull final Location location, @Nullable final Predicate<HunterDummyNPC> predicate) {
        ...
        final java.util.List<com.zenyte.game.world.entity.npc.NPC> matchingNPCs = CharacterLoop.find(location, radius, NPC.class, npc -> (npc.getId() == trapType.getActiveDummyNpcId() || npc.getId() == trapType.getInactiveDummyNpcId()) && (predicate == null || npc instanceof HunterDummyNPC && predicate.test((HunterDummyNPC) npc)));
        if (matchingNPCs.isEmpty()) {
            return Optional.empty();
        }
        final com.zenyte.game.world.entity.npc.NPC npc = matchingNPCs.get(0);
        if (!(npc instanceof HunterDummyNPC)) {
            return Optional.empty();
        }
        final java.lang.ref.WeakReference<com.zenyte.game.content.skills.hunter.object.HunterTrap> trapReference = ((HunterDummyNPC) npc).getTrap();
        if (trapReference == null) {
            return Optional.empty();
        }
        final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = trapReference.get();
        return Optional.ofNullable(trap);
    }

    public static OptionalInt findKnifeWithLogs(@NotNull final Player player) {
        final java.util.OptionalInt logs = TreeDefinitions.findLogs(player);
    ...
    public static boolean hasMaxTrapsLaid(@NotNull final Player player) {
        final int maximumLayableTraps = getMaximumLayableTraps(player.getSkills().getLevel(Skills.HUNTER)) + BooleanUtils.toInteger(player.inArea(WildernessArea.class));
    ...
    public static NetTrapPair findNetTrapPair(@NotNull final WorldObject eitherObject) throws IllegalStateException {
        ...
        final java.lang.String name = eitherObject.getName();
        ...
        final com.zenyte.game.util.Direction direction = youngTreeDirections.get(eitherObject.getRotation()).getCounterClockwiseDirection(isNet ? 4 : 0);
        //Build a new location object off of this object and move it one tile in the requested direction.
        final com.zenyte.game.world.entity.Location alternativeObjectCoordinates = eitherObject.transform(direction);
        final com.zenyte.game.world.object.WorldObject alternativeObject = World.getObjectWithType(alternativeObjectCoordinates, eitherObject.getType());
        ...
        final com.zenyte.game.world.object.WorldObject net = isNet ? eitherObject : alternativeObject;
        final com.zenyte.game.world.object.WorldObject tree = isNet ? alternativeObject : eitherObject;
    ...
    public static WorldObject buildNetTrap(@NotNull final WorldObject youngTree, @NotNull final TrapType trapType) {
        ...
        //The direction in which we need to move the tile by one coordinate to find the net's(or tree's if vice versa) physical location.
        final com.zenyte.game.util.Direction direction = youngTreeDirections.get(youngTree.getRotation());
        //Build a new location object off of this object and move it one tile in the requested direction.
        final com.zenyte.game.world.entity.Location location = youngTree.transform(direction);
    ...
    public static final void walkAway(@NotNull final Player player) {
        ...
        for (final com.zenyte.game.util.Direction direction : directions) {
            final com.zenyte.game.world.entity.Location tile = player.getLocation().transform(direction);
    ...
    public static BiPredicate<Player, HunterDummyNPC> getTrapOwnershipPredicate() {
        return HunterUtils.trapOwnershipPredicate;
    }
}
