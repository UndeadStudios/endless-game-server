...
public final class SpellState {
	...
	public boolean check(final boolean inform) {
		...
		for (final com.zenyte.game.item.Item rune : runes) {
			if (rune == null) {
				continue;
			}
			int amount = 0;
			...
			for (final com.zenyte.game.content.skills.magic.resources.RuneResource resource : RESOURCES) {
	...
	public void remove() {
		...
		for (final com.zenyte.game.content.skills.magic.Resource resource : resources) {
	...
	private int checkRuneResource(final RuneResource resource, int id, final int amount) {
		...
		if (resource == STAFF_RESOURCE || resource == TOME_RESOURCE || resource == BRYOPHYTAS_RESOURCE) {
			final com.zenyte.game.content.skills.magic.Resource result = getResource(resource, id, amount);
			if (result != null) {
	...
	private Resource getResource(final RuneResource source, final int id, final int amount) {
		...
		final com.zenyte.game.content.skills.magic.Resource resource = new Resource(id, held, source);
