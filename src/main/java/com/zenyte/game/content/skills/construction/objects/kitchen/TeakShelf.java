		...
		player.getDialogueManager().start(new Dialogue(player) {
			...
			public void buildDialogue() {
				options("What would you like to take?",
						...
						"An empty beer glass.",
						"Next selection.")
				.onOptionOne(() -> { addItem(player, 0, id); finish(); })
				.onOptionTwo(() -> { addItem(player, 1, id); finish(); })
				.onOptionThree(() -> { addItem(player, 2, id); finish(); })
				.onOptionFour(() -> { addItem(player, 3, id); finish(); })
				.onOptionFive(() -> setKey(5));
				options(5, "What would you like to take?", id == 13558 ? SECOND_SHELF : FIRST_SHELF)
				.onOptionOne(() -> { addItem(player, 4, id); finish(); })
				.onOptionTwo(() -> { addItem(player, 5, id); finish(); })
				.onOptionThree(() -> { addItem(player, 6, id); finish(); })
				.onOptionFour(id == 13558 ? () -> { addItem(player, 7, id); finish(); } : () -> setKey(1))
