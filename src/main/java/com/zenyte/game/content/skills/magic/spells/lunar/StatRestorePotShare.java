...
public final class StatRestorePotShare implements ItemSpell {
	...
	public boolean spellEffect(final Player player, final Item item, final int slot) {
		...
		final com.zenyte.game.content.consumables.drinks.Potion potion = (Potion) consumable;
		int count = 0;
		final int doses = potion.getDoses(item.getId());
		final java.util.List<com.zenyte.game.world.entity.player.Player> characters = player.findCharacters(1, Player.class, p2 -> p2 != player && !p2.isDead() && p2.getBooleanSetting(Setting.ACCEPT_AID) && p2.getDuel() == null);
		for (int i = characters.size() - 1; i >= 0; i--) {
			final com.zenyte.game.world.entity.player.Player p2 = characters.get(i);
			if (count >= doses) {
				break;
			}
			count++;
		...
		final com.zenyte.game.item.Item leftover = potion.getItem(dosesLeft);
