...
public class Planting extends Action {
    ...
    public boolean start() {
        ...
        final com.zenyte.game.content.skills.farming.PatchType type = patch.getType();
        ...
        if (patch == FarmingPatch.TROLL_STRONGHOLD_HERB) {
            if (product == FarmingProduct.GOUTWEED) {
                player.getDialogueManager().start(new ItemChat(player, new Item(6311), "You can\'t plant goutweed in this patch."));
                return false;
            }
        }
        if (patch == FarmingPatch.WEISS_HERB_PATCH) {
            if (!OldFirePit.FirePit.WEISS_FIRE.isBuilt(player)) {
                player.getDialogueManager().start(new PlainChat(player, "The crops can\'t grow in areas of extreme cold. Perhaps you should look into warming up the area around first."));
                return false;
            }
        }
    ...
    public int processWithDelay() {
        ...
        final com.zenyte.game.item.Item item = new Item(product.getSeed());
