...
public class SlayerPartnerInterface implements UserInterface {
	...
	public void handleComponentClick(final Player player, final int interfaceId, final int componentId, final int slotId, final int itemId, final int optionId, final String option) {
		...
		final com.zenyte.game.world.entity.player.Player partner = player.getSlayer().getPartner();
		if (partner == null) {
			player.getInterfaceHandler().closeInterface(InterfacePosition.CENTRAL);
			player.sendInputName("Who would you like as your partner?", name -> {
				final com.zenyte.game.world.entity.player.Player target = World.getPlayerByDisplayname(name);
				...
				final com.zenyte.plugins.dialogue.PlainChat chat = new PlainChat(player, "Sending partnership offer...", false);
				final com.zenyte.game.world.entity.player.Player requestingPlayer = player;
				player.getDialogueManager().start(chat);
				target.getDialogueManager().start(new Dialogue(target) {
					@Override
					public void buildDialogue() {
						options("Slayer Partner offer: " + requestingPlayer.getName() + "(" + requestingPlayer.getSkills().getLevelForXp(Skills.SLAYER) + ")", new DialogueOption[] {new DialogueOption("Accept.", () -> {
