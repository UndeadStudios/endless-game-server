...
public final class ItemTrapSetupAction extends Action {
    ...
    public boolean start() {
        ...
        if (floorItem != null) {
            ...
            final com.zenyte.game.world.entity.player.container.ContainerResult result = player.getInventory().deleteItem(slot, item);
            if (result.isFailure()) {
                return false;
            }
    ...
    public int processWithDelay() {
        ...
        World.destroyFloorItem(floorItem);
        final com.zenyte.game.content.skills.hunter.npc.HunterDummyNPC dummy = new HunterDummyNPC(trapType.getActiveDummyNpcId(), location);
        final com.zenyte.game.world.object.WorldObject object = new WorldObject(trapType.getObjectId(), 10, 2, location);
        final com.zenyte.game.world.object.WorldObject collapsedObject = new WorldObject(trapType.getCollapsedObjectId(), 10, 2, location);
        final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = new HunterTrap(player, dummy, location, Collections.emptyList(), Collections.singletonList(object), Collections.singletonList(collapsedObject), trapType);
        dummy.setTrap(trap);
