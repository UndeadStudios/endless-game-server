...
public final class ClimbEmptyStaircaseD extends Dialogue {
	...
	public void buildDialogue() {
		if (upstairs) {
			...
			.onOptionOne(() -> {
				finish();
				final RoomReference ref = new RoomReference(Room.SKILL_HALL_DS, room.getX(), room.getY(), 2, room.getRotation());
			...
			.onOptionTwo(() -> {
				finish();
				final RoomReference ref = new RoomReference(Room.QUEST_HALL_DS, room.getX(), room.getY(), 2, room.getRotation());
				final int slot = getFurnitureSlot(FurnitureSpace.SKILL_HALL_STAIRCASE);
			...
			.onOptionOne(() -> {
				finish();
				final RoomReference ref = new RoomReference(Room.SKILL_HALL, room.getX(), room.getY(), 0, room.getRotation());
			...
			.onOptionTwo(() -> {
				finish();
				final RoomReference ref = new RoomReference(Room.QUEST_HALL, room.getX(), room.getY(), 0, room.getRotation());
				int slot = getFurnitureSlot(FurnitureSpace.SKILL_HALL_STAIRCASE_DS);
				if (slot == -1)
					slot = 0;
			...
			.onOptionThree(() -> {
				finish();
				final RoomReference ref = new RoomReference(Room.DUNGEON_STAIRS_ROOM, room.getX(), room.getY(), 0, room.getRotation());
