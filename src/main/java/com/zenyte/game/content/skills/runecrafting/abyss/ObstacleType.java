...
public enum ObstacleType {
	TENDRILS((player, object, destination) -> {
		...
		final int chance = player.getSkills().getLevel(Skills.WOODCUTTING) + 1;
		AxeDefinitions axe = null;
		for (final com.zenyte.game.content.skills.woodcutting.AxeDefinitions a : AxeDefinitions.VALUES) {
		...
		final com.zenyte.game.content.skills.woodcutting.AxeDefinitions def = axe;
		if (axe == null) {
		...
		final int chance = player.getSkills().getLevel(Skills.MINING) + 1;
		PickaxeDefinitions pickaxe = null;
		for (final java.lang.Integer id : MiningDefinitions.tools.keySet()) {
		...
		final int chance = player.getSkills().getLevel(Skills.THIEVING) + 1;
		...
		final int chance = player.getSkills().getLevel(Skills.AGILITY) + 1;
		...
		final int chance = player.getSkills().getLevel(Skills.FIREMAKING) + 1;
	...
	private ObstacleType(final ObstacleScript script) {
		this.script = script;
	}
