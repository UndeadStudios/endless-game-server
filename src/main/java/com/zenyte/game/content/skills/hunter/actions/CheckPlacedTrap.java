...
public class CheckPlacedTrap extends BuiltHunterTrapAction {
    ...
    public boolean start() {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.item.Item> listOfItemsToBeGiven = new ObjectArrayList<Item>();
        if (type.isNetTrap()) {
        ...
        final com.zenyte.game.content.skills.hunter.node.TrapPrey prey = trap.getPrey();
        if (prey != null) {
            for (final com.zenyte.game.item.ImmutableItem immutableItem : prey.getItems()) {
        ...
        int spaceRequired = 0;
        for (final com.zenyte.game.item.Item item : listOfItemsToBeGiven) {
    ...
    public CheckPlacedTrap(final HunterTrap trap, final boolean reset) {
        this.trap = trap;
        this.reset = reset;
    }
}
