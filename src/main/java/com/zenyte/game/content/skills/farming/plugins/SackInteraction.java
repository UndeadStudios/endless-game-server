...
public class SackInteraction extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    private enum SackType {
        ...
        private SackType(final int baseSackId, final int vegetableId) {
    ...
    public void handle() {
        bind("Fill", (player, item, container, slotId) -> {
            ...
            final com.zenyte.game.content.skills.farming.plugins.SackInteraction.SackType matchingType = Utils.findMatching(SackType.values, value -> value.isOfType(sackId));
        ...
        bind("Remove-one", (player, item, container, slotId) -> {
            ...
            final int sackId = item.getId();
            final com.zenyte.game.content.skills.farming.plugins.SackInteraction.SackType matchingType = Objects.requireNonNull(Utils.findMatching(SackType.values, value -> value.isOfType(sackId)));
            ...
            inventory.refreshAll();
            final java.lang.String vegetableName = ItemDefinitions.getOrThrow(matchingType.vegetableId).getName().toLowerCase();
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        ...
        final com.zenyte.game.item.Item vegetable = fromId == SackType.POTATOES.vegetableId || fromId == SackType.ONIONS.vegetableId || fromId == SackType.CABBAGES.vegetableId ? from : to;
        final com.zenyte.game.item.Item sack = vegetable == from ? to : from;
        final com.zenyte.game.content.skills.farming.plugins.SackInteraction.SackType matchingSack = Utils.findMatching(SackType.values, value -> value.isOfType(sack.getId()));
        final com.zenyte.game.content.skills.farming.plugins.SackInteraction.SackType sackType = Objects.requireNonNull(Utils.findMatching(SackType.values, value -> value.vegetableId == vegetable.getId()));
        final java.lang.String vegetableName = vegetable.getName().toLowerCase();
    ...
    @Override
    public int[] getItems() {
        final it.unimi.dsi.fastutil.ints.IntArrayList intList = new IntArrayList();
        for (final com.zenyte.game.content.skills.farming.plugins.SackInteraction.SackType type : SackType.values) {
            for (int i = 0; i < 10; i++) {
