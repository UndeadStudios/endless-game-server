...
public class RevenantCavePillar implements Shortcut {
	...
	public void startSuccess(Player player, WorldObject object) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (ticks == 0) {
					final com.zenyte.game.world.entity.Location destination = direction == ForceMovement.NORTH ? new Location(player.getX(), player.getY() + 2, player.getPlane()) : direction == ForceMovement.SOUTH ? new Location(player.getX(), player.getY() - 2, player.getPlane()) : direction == ForceMovement.EAST ? new Location(player.getX() + 2, player.getY(), player.getPlane()) : new Location(player.getX() - 2, player.getY(), player.getPlane());
