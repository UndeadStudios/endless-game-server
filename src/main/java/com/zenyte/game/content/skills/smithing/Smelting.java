...
public class Smelting extends Action {
	...
	public static int getAmountOfCoalInCoalBag(final Player player) {
		final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> coalBag = player.getInventory().getContainer().getFirst(CoalBag.ITEM.getId());
	...
	private static void handleDiary(final Player player, final SmeltableBar bar) {
		final com.zenyte.game.content.achievementdiary.AchievementDiaries dairies = player.getAchievementDiaries();
	...
	private boolean isSuccessful(final Player player) {
		...
		if (data.equals(SmeltableBar.IRON_BAR)) {
			final com.zenyte.game.item.Item ringItem = equipment.getItem(EquipmentSlot.RING);
	...
	public int processWithDelay() {
		...
		if (object.getPositionHash() == EDGEVILLE_FURNACE.getPositionHash() && body >= 13104 && body <= 13107) {
			final com.zenyte.game.content.skills.smithing.SmeltableBar limit = body == 13104 ? SmeltableBar.STEEL_BAR : body == 13105 ? SmeltableBar.MITHRIL_BAR : body == 13106 ? SmeltableBar.ADAMANTITE_BAR : SmeltableBar.RUNITE_BAR;
		...
		if (ticks == 0) {
			...
			for (int index = 0; index < items.length; index++) {
				final com.zenyte.game.item.Item material = data.getMaterials()[index];
	...
	public Smelting(final SmeltableBar data, final int amount, final WorldObject object) {
		this.data = data;
		this.amount = amount;
		this.object = object;
	}
