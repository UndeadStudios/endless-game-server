...
public class PitfallHunterNPC extends HunterNPC implements Spawnable {
    ...
    public PitfallHunterNPC(int id, Location tile, Direction facing, int radius) {
        ...
        this.combat = new NPCCombat(this) {
            ...
            public void setTarget(final Entity target) {
                final com.zenyte.game.world.entity.Entity t = this.target;
    ...
    public NPC spawn() {
        ...
        Utils.findEmptySquare(originalTile.transform(Utils.random(10) - 7, Utils.random(10) - 7, 0), 5, getSize(), Optional.empty()).ifPresent(tile -> setRespawnTile(new ImmutableLocation(tile)));
        return super.spawn();
    }
    ...
    private void fallIn(@NotNull final Player player, @NotNull final WorldObject trap) {
        ...
        final com.zenyte.game.util.Direction dir = (rot & 1) == 0 ? (getY() > trap.getY() ? SOUTH : NORTH) : (getX() > trap.getX() ? WEST : EAST);
        setFaceEntity(null);
    ...
    private void jumpOver(@NotNull final Player player, @NotNull final WorldObject trap) {
        ...
        final com.zenyte.game.util.Direction dir = (rot & 1) == 0 ? (py > trap.getY() ? SOUTH : NORTH) : (px > trap.getX() ? WEST : EAST);
        final com.zenyte.game.world.entity.Location destination = new Location(dir == WEST ? (px - 4) : dir == EAST ? (px + 4) : px, dir == SOUTH ? (py - 4) : dir == NORTH ? (py + 4) : py, player.getPlane());
        final com.zenyte.game.content.skills.hunter.npc.PitfallHunterNPC.FlyingCreatureProjectile type = Objects.requireNonNull(Utils.findMatching(FlyingCreatureProjectile.values, value -> value.prey == prey));
        final com.zenyte.game.world.Projectile firstProj = dir == EAST || dir == NORTH ? type.right : type.left;
        final com.zenyte.game.world.Projectile secondProj = dir == EAST || dir == NORTH ? type.left : type.right;
        final com.zenyte.game.world.entity.Location mainProjectileStart = new Location(getLocation());
        final com.zenyte.game.world.entity.Location mainProjectileEnd = new Location(destination);
        final com.zenyte.game.world.entity.Location secondProjectileStart = new Location(mainProjectileStart);
        final com.zenyte.game.world.entity.Location secondProjectileEnd = new Location(mainProjectileEnd);
    ...
    public void follow(@NotNull final Player player, @NotNull final WorldObject leaptTrap) {
        final java.util.Optional<com.zenyte.game.world.entity.Location> optionalDest = calculateDestination(player, leaptTrap);
        if (!optionalDest.isPresent()) {
            return;
        }
        final com.zenyte.game.world.entity.Location dest = optionalDest.get();
        ...
        setRouteEvent(new PitfallTileEvent(this, new TileStrategy(dest), () -> {
            ...
            lock();
            WorldTasksManager.schedule(() -> {
                final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = HunterUtils.findTrap(TrapType.PITFALL, leaptTrap, npc -> HunterUtils.getTrapOwnershipPredicate().test(player, npc)).orElseThrow(RuntimeException::new);
    ...
    private static final class PitfallTileEvent extends NPCTileEvent {
        ...
        public boolean process() {
            ...
            if (!entity.hasWalkSteps()) {
                final com.zenyte.game.world.entity.pathfinding.RouteResult steps = RouteFinder.findRoute(entity, 1, strategy, true);
