...
public enum BossTask implements SlayerTask {
	KREE_ARRA(357, p -> p.getSkills().getLevelForXp(Skills.RANGED) >= 70, "Kree\'arra", false), KRIL_TSUTSAROTH(350.5F, p -> p.getSkills().getLevelForXp(Skills.HITPOINTS) >= 70, "K\'ril Tsutsaroth", false), COMMANDER_ZILYANA(350, p -> p.getSkills().getLevelForXp(Skills.AGILITY) >= 70, "Commander Zilyana", false), GENERAL_GRAARDOR(338, p -> p.getSkills().getLevelForXp(Skills.STRENGTH) >= 70, "General Graardor", false), DAGANNOTH_KINGS(331.5F, p -> true, "Dagannoth Kings", false) {
		@Override
	...
	KING_BLACK_DRAGON(258, p -> true, "King Black Dragon", false), VORKATH(750, p -> true, "Vorkath", false), VETION(312, p -> true, "Vet\'ion reborn", true), CRAZY_ARCHEAOLOGIST(275, p -> true, "Crazy archaeologist", true), ZULRAH(500, p -> true, "Zulrah", false), CERBERUS(690, p -> p.getSkills().getLevelForXp(Skills.SLAYER) >= 91, "Cerberus", false), GIANT_MOLE(215, p -> true, "Giant Mole", false), CALLISTO(312, p -> true, "Callisto", true), CHAOS_ELEMENTAL(250, p -> true, "Chaos elemental", true), SCORPIA(260, p -> true, "Scorpia", true), KRAKEN(255, p -> p.getSkills().getLevelForXp(Skills.SLAYER) >= 87, "Kraken", false), ABYSSAL_SIRE(450, p -> false /* TODO: Implement sire; p.getSkills().getLevelForXp(Skills.SLAYER) >= 85 */, "Abyssal sire", false), KALPHITE_QUEEN(535.5F, p -> true, "Kalphite Queen", false), VENENATIS(388.8F, p -> true, "Venenatis", true), CHAOS_FANATIC(253, p -> true, "Chaos Fanatic", true), THERMONUCLEAR_SMOKE_DEVIL(240, p -> p.getSkills().getLevelForXp(Skills.SLAYER) >= 93, "Thermonuclear smoke devil", false), GROTESQUE_GUARDIANS(1350, p -> p.getSkills().getLevelForXp(Skills.SLAYER) >= 75 && p.getNumericAttribute("brittle-entrance_unlocked").intValue() == 1, "Grotesque Guardians", false) {
		@Override
		public boolean validate(@NotNull final String name, final NPC npc) {
			if (name == null) {
				throw new NullPointerException("name is marked non-null but is null");
	...
	static {
		for (final com.zenyte.game.content.skills.slayer.BossTask value : VALUES) {
