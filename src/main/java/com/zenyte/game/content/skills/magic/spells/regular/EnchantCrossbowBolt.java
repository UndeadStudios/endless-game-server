...
public class EnchantCrossbowBolt implements DefaultSpell {
	...
	public boolean spellEffect(final Player player, final int optionId, final String option) {
		...
		final java.util.List<com.zenyte.game.content.skills.magic.actions.BoltEnchantment.BoltEnchantmentData> list = BoltEnchantmentData.getData(player, true);
		if (list.isEmpty()) {
			player.sendMessage("You have no bolts to enchant.");
			return false;
		}
		final com.zenyte.game.item.Item[] items = new Item[list.size()];
