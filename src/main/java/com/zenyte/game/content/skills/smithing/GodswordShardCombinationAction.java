...
public class GodswordShardCombinationAction extends Action {
	...
	public int processWithDelay() {
		...
		case 9: 
			player.getInventory().deleteItemsIfContains(combination.getMaterials(), () -> {
				player.getDialogueManager().start(new ItemChat(player, combination.getItem(), "Even for an experienced smith it is not an easy task,<br>but eventually it is done."));
	...
	public GodswordShardCombinationAction(final GodswordShardCombination combination) {
