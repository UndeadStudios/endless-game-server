...
public class WildernessDeepDungeonCrevice implements Shortcut {
    ...
    private static Location NORTH_CREVICE = new Location(3046, 10327, 0);
    private static Location SOUTH_CREVICE = new Location(3048, 10335, 0);

    private static Location NORTH_EXIT = new Location(3046, 10326, 0);
    private static Location SOUTH_EXIT = new Location(3048, 10336, 0);

    @Override
    public boolean preconditions(final Player player, final WorldObject object) {
        if (!DiaryReward.WILDERNESS_SWORD2.eligibleFor(player)) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                if(ticks == 0) {
                    ...
                    player.setAttackedByDelay(System.currentTimeMillis());
