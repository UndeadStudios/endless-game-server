...
public class BirdNests {
	...
	public static void main(String[] args) {
		final java.util.HashMap<com.zenyte.game.item.Item, java.lang.Integer> results = new HashMap<Item, Integer>();
		final com.zenyte.game.content.skills.woodcutting.actions.BirdNests.Nests nest = Nests.SEED;
		System.out.println(Arrays.toString(nest.cumulativeProbability));
		for (int i = 0; i < 100000; i++) {
			final com.zenyte.game.item.Item loot = nest.rollLoot();
			final java.lang.Integer count = results.getOrDefault(loot, 0);
			results.put(loot, count + 1);
		}
		System.out.println("Results:");
		for (final java.util.Map.Entry<com.zenyte.game.item.Item, java.lang.Integer> result : results.entrySet()) {
	...
	public enum Nests {
		...
		static {
			for (final com.zenyte.game.content.skills.woodcutting.actions.BirdNests.Nests nest : VALUES) {
		...
		public static Nests rollRandomNest(final boolean eggs) {
			...
			if (typeRoll <= 4) {
				final com.zenyte.game.content.skills.woodcutting.actions.BirdNests.Nests[] eggNests = new Nests[] {Nests.RED_EGG, Nests.GREEN_EGG, Nests.BLUE_EGG};
	...
	private static class NestLoot {
		...
		public NestLoot(final Item item, final int chance) {
			this.item = item;
			this.chance = chance;
		}
	}
