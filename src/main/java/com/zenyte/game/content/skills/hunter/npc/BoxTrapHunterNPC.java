...
public class BoxTrapHunterNPC extends HunterNPC implements Spawnable {
        ...
        if (this.isChinchompa) {
            ...
            this.retreatMechanics = new RetreatMechanics(this) {
                ...
                public boolean process(@NotNull final Entity target) {
                    this.target = target;
                    final com.zenyte.game.world.entity.Location npcLocation = npc.getLocation();
                    if (outOfBounds(npcLocation)) {
                        retreat(npc.getRespawnTile(), target, false);
                        return true;
                    }
                    final com.zenyte.game.world.entity.Location targetLocation = target.getLocation();
    ...
    public void sendDeath() {
        ...
        final com.zenyte.game.world.entity.player.Player player = getMostDamagePlayerCheckIronman();
        onDeath(player);
            ...
            public void run() {
                if (ticks == 0) {
                    ...
                    if (player != null && player.getLocation().getTileDistance(getLocation()) == 1) {
                        final com.zenyte.game.world.entity.Location tile = player.getFaceLocation(BoxTrapHunterNPC.this, 2, 1024);
                        final com.zenyte.game.world.entity.Location destination = new Location(player.getLocation());
                        ...
                        if (!destination.matches(player.getLocation())) {
                            ...
                            final com.zenyte.game.world.entity.masks.ForceMovement fm = new ForceMovement(currentTile, 30, destination, 60, Utils.getFaceDirection(currentTile.getX() - tile.getX(), currentTile.getY() - tile.getY()));
    ...
    private void collapseOrFallIn() {
        Preconditions.checkArgument(trap != null);
        final com.zenyte.game.content.skills.hunter.node.PreyObject preyObject = prey.getObject();
        ...
        if (processing) {
            if (success) {
                ...
                final com.zenyte.game.world.entity.ImmutableLocation tile = trap.getLocation();
        ...
        if (success) {
            ...
            finish();
            setRespawnTask();
            final com.zenyte.game.content.skills.hunter.npc.HunterDummyNPC dummy = trap.getNpc().get();
            if (dummy != null) {
