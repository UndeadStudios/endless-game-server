...
public final class HouseViewer {
	 ...
	 */
	private transient List<Integer> bits = new ArrayList<Integer>();
	...
	public void handleInterface(final int interfaceId, final int componentId, final int slotId, final int itemId, final int option) {
		if (componentId > 5 && componentId < 39) {
			...
			System.out.println("Room selected: " + room.getRoom().toString());
	...
	public void addRoom(final int componentId, final int slot) {
		...
		Room room = Room.getRoomBySlot(slot);
		final int plane = slotId > 170 ? 2 : slotId > 80 ? 1 : 0;
		if (plane == 0) {
			if (!(room == Room.DUNGEON_CORRIDOR || room == Room.DUNGEON_JUNCTION || room == Room.OUBLIETTE || room == Room.DUNGEON_STAIRS_ROOM)) {
				openHouseViewer();
			...
			if (room == Room.SKILL_HALL) {
				room = Room.SKILL_HALL_DS;
			} else if (room == Room.QUEST_HALL) {
				room = Room.QUEST_HALL_DS;
			}
		}
