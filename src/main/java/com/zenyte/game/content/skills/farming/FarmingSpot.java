...
public final class FarmingSpot {
    ...
    public int addCompostableItem(@NotNull final Item item) {
        assert this.getPatch().getType() == PatchType.COMPOST_BIN;
        final com.zenyte.game.content.skills.farming.CompostBin bin = getCompostBin();
        final int amount = bin.add(item);
        final com.zenyte.game.content.skills.farming.CompostBinType type = bin.getType().orElseThrow(RuntimeException::new);
    ...
    public Item removeCompostableItem() {
        ...
        final com.zenyte.game.content.skills.farming.CompostBinType type = bin.getType().orElseThrow(RuntimeException::new);
        bin.removeOne();
        final com.zenyte.game.item.Item product = type.getProduct().getProduct();
        final int count = bin.getAmount();
        if (count == 0) {
            clear();
        } else {
    ...
    public void closeCompostBin() {
        assert this.getPatch().getType() == PatchType.COMPOST_BIN;
        final com.zenyte.game.content.skills.farming.CompostBin bin = this.getCompostBin();
        assert bin.isFull();
    ...
    public void process() {
        ...
        final com.zenyte.game.content.skills.farming.PatchState oldState = getState();
        boolean regeneratingFruit = false;
        while (getTime() < currentTime) {
            final com.zenyte.game.content.skills.farming.FarmingProduct product = getProduct();
            final int value = getValue();
            final com.zenyte.game.content.skills.farming.PatchState state = getState();
            if (product == FarmingProduct.REDWOOD) {
                if (value >= 41) {
                    final com.zenyte.game.content.skills.farming.RedwoodTree redwood = Objects.requireNonNull((RedwoodTree) map.get(FarmingAttribute.REDWOOD));
        ...
        if (!regeneratingFruit) {
            final com.zenyte.game.content.skills.farming.PatchState newState = getState();
    ...
    private final int calculateBushLives() {
        final java.util.Optional<com.zenyte.game.content.skills.farming.PatchFlag> optionalPatchFlag = getCompostFlag();
        if (optionalPatchFlag.isPresent()) {
            final com.zenyte.game.content.skills.farming.PatchFlag flag = optionalPatchFlag.get();
            if (flag == COMPOST) {
                return 5;
            } else if (flag == SUPERCOMPOST) {
                return 6;
    ...
    public void setChoppedDown(final int id) {
        final com.zenyte.game.content.skills.farming.FarmingProduct product = getProduct();
        final com.zenyte.game.content.skills.woodcutting.TreeDefinitions definitions = product.getTreeDefinitions();
        final int time = definitions.getRespawnDelay();
        if (product == FarmingProduct.REDWOOD) {
            final com.zenyte.game.content.skills.farming.RedwoodTree redwood = (RedwoodTree) map.computeIfAbsent(FarmingAttribute.REDWOOD, k -> new RedwoodTree());
    ...
    public void refresh() {
        final com.zenyte.game.content.skills.farming.FarmingPatch patch = getPatch();
    ...
    public void clear() {
        final com.zenyte.game.content.skills.farming.PatchType type = this.getPatch().getType();
    ...
    private boolean disease(final PatchState state) {
        final com.zenyte.game.content.skills.farming.FarmingProduct product = this.getProduct();
        ...
        final com.zenyte.game.content.skills.farming.FarmingPatch patch = this.getPatch();
        ...
        if (this.getPatch().getType() == PatchType.ALLOTMENT) {
            final java.util.Optional<com.zenyte.game.content.skills.farming.FarmingSpot> flowerPatch = player.getFarming().getNearbyFlowerPatch(this);
            if (flowerPatch.isPresent()) {
                final com.zenyte.game.content.skills.farming.FarmingProduct flower = flowerPatch.get().getProduct();
        ...
        final java.util.Optional<com.zenyte.game.content.skills.farming.PatchFlag> compost = this.getCompostFlag();
    ...
    public boolean bearsFruit() {
        ...
        final com.zenyte.game.content.skills.farming.FarmingProduct.Stage misc = this.getProduct().getMiscStage();
    ...
    public boolean isFruitless() {
        ...
        final com.zenyte.game.content.skills.farming.PatchState state = getState();
        final com.zenyte.game.content.skills.farming.FarmingProduct.Stage miscStage = this.getProduct().getMiscStage();
        if (miscStage == null) {
            return true;
        }
    ...
    public Optional<PatchFlag> getCompostFlag() {
        if (!getFlags().isEmpty()) {
            for (final com.zenyte.game.content.skills.farming.PatchFlag flag : compostFlags) {
