...
public final class StaffResource implements RuneResource {
	...
	private static final void populate(final EnumDefinitions e, final int[] array) {
		int index = 0;
		for (final java.lang.Integer value : e.getValues().keySet()) {
			array[index++] = value;
		}
	}

	static {
		final mgi.types.config.enums.EnumDefinitions airStavesEnum = EnumDefinitions.get(988);
		AIR_STAVES = new int[airStavesEnum.getSize()];
		populate(airStavesEnum, AIR_STAVES);
		final mgi.types.config.enums.EnumDefinitions waterStavesEnum = EnumDefinitions.get(989);
		WATER_STAVES = new int[waterStavesEnum.getSize()];
		populate(waterStavesEnum, WATER_STAVES);
		final mgi.types.config.enums.EnumDefinitions earthStavesEnum = EnumDefinitions.get(996);
		EARTH_STAVES = new int[earthStavesEnum.getSize()];
		populate(earthStavesEnum, EARTH_STAVES);
		final mgi.types.config.enums.EnumDefinitions fireStavesEnum = EnumDefinitions.get(997);
