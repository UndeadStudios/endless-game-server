...
public class BowFletching extends Action {
    ...
    public int processWithDelay() {
        ...
        player.getInventory().deleteItemsIfContains(data.getMaterials(), () -> {
            if (data.equals(BowFletchingData.RUNE_CROSSBOW_U)) {
                player.getAchievementDiaries().update(ArdougneDiary.MAKE_RUNE_CROSSBOW, 4);
            } else if (data.equals(BowFletchingData.RUNE_CROSSBOW)) {
                player.getAchievementDiaries().update(ArdougneDiary.MAKE_RUNE_CROSSBOW, 22);
                ...
                player.getAchievementDiaries().update(KandarinDiary.CREATE_YEW_LONGBOW, 4);
    ...
    public BowFletching(final BowFletchingData data, final int amount) {
        this.data = data;
        this.amount = amount;
    }
}
