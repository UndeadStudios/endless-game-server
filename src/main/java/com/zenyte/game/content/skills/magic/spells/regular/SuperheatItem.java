...
public final class SuperheatItem implements ItemSpell {
    ...
    private void superheat(final Player player, final Item item, final int slot) {
        if (player.getTemporaryAttributes().get("performing superheat") == null) {
            ...
            final boolean isIron = item.getId() == 440 && ((player.getInventory().getAmountOf(453) + Smelting.getAmountOfCoalInCoalBag(player) < 2) || player.getSkills().getLevel(Skills.SMITHING) < 30);
            final com.zenyte.game.content.skills.smithing.SmeltableBar data = SmeltableBar.getData(item.getId(), isIron);
            if (data == null) {
                if (item.getId() == 453) {
            ...
            WorldTasksManager.schedule(() -> {
                ...
                if (player.getTemporaryAttributes().get("next superheat item") instanceof Item) {
                    ...
                    final com.zenyte.game.item.Item item1 = (Item) player.getTemporaryAttributes().remove("next superheat item");
