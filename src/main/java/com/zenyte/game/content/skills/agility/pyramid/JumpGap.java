...
public class JumpGap implements FailableAgilityPyramidObstacle, IrreversibleDirection {
    ...
    private void cross(final Player player, final WorldObject object, final boolean success) {
        final com.zenyte.game.util.Direction walkDirection = Direction.getNPCDirection(player.getRoundedDirection());
        final com.zenyte.game.world.entity.Location destination = player.getLocation().transform(walkDirection, success ? 3 : 1);
