...
public class GrapevineTreating extends Action {
    ...
    public boolean start() {
        ...
        if (!player.getInventory().containsItem(FarmingConstants.GARDENING_TROWEL) || !player.getInventory().containsItem(FarmingConstants.SALTPETRE)) {
            player.getDialogueManager().start(new DoubleItemChat(player, FarmingConstants.GARDENING_TROWEL, FarmingConstants.SALTPETRE, "You can treat the patch by digging saltpetre into it with a trowel."));
            return false;
        }
        player.lock(3);
    ...
    public GrapevineTreating(final FarmingSpot spot) {
        this.spot = spot;
    }
}
