...
public class PitfallObject implements ObjectAction, ItemOnObjectAction {
    ...
    public void handleObjectAction(Player player, WorldObject object, String name, int optionId, String option) {
        ...
        case "Jump": 
            ...
            break;
        case "Dismantle": 
            final com.zenyte.game.content.skills.hunter.object.HunterTrap trap = HunterUtils.findTrap(TrapType.PITFALL, object, npc -> HunterUtils.getTrapOwnershipPredicate().test(player, npc)).orElseThrow(RuntimeException::new);
    ...
    public void jump(@NotNull final Player player, @NotNull final WorldObject trap) {
        ...
        final com.zenyte.game.util.Direction dir = (rot & 1) == 0 ? (py > trap.getY() ? Direction.SOUTH : Direction.NORTH) : (px > trap.getX() ? Direction.WEST : Direction.EAST);
        final com.zenyte.game.world.entity.Location destination = new Location(dir == Direction.WEST ? (px - 3) : dir == Direction.EAST ? (px + 3) : px, dir == Direction.SOUTH ? (py - 3) : dir == Direction.NORTH ? (py + 3) : py, player.getPlane());
    ...
    public void handleItemOnObjectAction(Player player, Item item, int slot, WorldObject object) {
        ...
        final com.zenyte.game.content.skills.woodcutting.TreeDefinitions logs = Utils.findMatching(TreeDefinitions.values, value -> value.getLogsId() == item.getId());
        player.getActionManager().setAction(new CreatePitfallAction(object, logs != null ? OptionalInt.of(slot) : OptionalInt.empty()));
    }

    @Override
    public Object[] getItems() {
        final it.unimi.dsi.fastutil.objects.ObjectOpenHashSet<java.lang.Object> list = new ObjectOpenHashSet<>();
        for (final com.zenyte.game.content.skills.woodcutting.TreeDefinitions tree : TreeDefinitions.values) {
