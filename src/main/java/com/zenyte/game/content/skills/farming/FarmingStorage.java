...
public class FarmingStorage {
    ...
    private int getTotalAmount(@NotNull final Storable storable) {
        int count = 0;
        for (final com.zenyte.game.item.Item stored : storage) {
    ...
    public FarmingStorageInterface.Option getOption(final int option) {
        final java.util.ArrayList<com.zenyte.game.content.skills.farming.plugins.FarmingStorageInterface.Option> list = new ArrayList<FarmingStorageInterface.Option>(6);
        ...
        for (final com.zenyte.game.content.skills.farming.plugins.FarmingStorageInterface.Option op : FarmingStorageInterface.Option.values) {
            if (!list.contains(op)) {
                list.add(op);
            }
        }
        final java.util.EnumMap<com.zenyte.game.content.skills.farming.plugins.FarmingStorageInterface.Option, com.zenyte.game.content.skills.farming.plugins.FarmingStorageInterface.Option> map = new EnumMap<FarmingStorageInterface.Option, FarmingStorageInterface.Option>(FarmingStorageInterface.Option.class);
        for (int i = 0; i < FarmingStorageInterface.Option.values.length; i++) {
            final com.zenyte.game.content.skills.farming.plugins.FarmingStorageInterface.Option dynamicOption = list.get(i);
            final com.zenyte.game.content.skills.farming.plugins.FarmingStorageInterface.Option staticOption = FarmingStorageInterface.Option.values[i];
            map.put(staticOption, dynamicOption);
        }
        return Objects.requireNonNull(map.get(Objects.requireNonNull(Utils.findMatching(FarmingStorageInterface.Option.values, i -> i.getOptionId() == option))));
    }

    public void removeItem(final Storable storable, final Item item, final int amount) {
        if (item == null) {
            final java.lang.String itemName = ItemDefinitions.getOrThrow(storable.getBaseId()).getName().toLowerCase();
            final java.lang.String prefix = storable.getItemIds().length > 1 || storable.getMaximumAmount() > 1 ? "any" : Utils.getAOrAn(itemName);
            player.sendMessage("You haven\'t got " + prefix + " " + itemName + " stored in here.");
            return;
        }
        //If requesting banknotes.
        if (amount == Integer.MIN_VALUE) {
            final com.zenyte.game.world.entity.player.container.ContainerResult result = player.getInventory().addItem(new Item(item.getDefinitions().getNotedOrDefault(), item.getAmount(), item.getAttributesCopy()));
    ...
    public void addItem(final Storable storable, final int amount) {
        final java.util.ArrayList<com.zenyte.game.item.Item> storedItems = getStoredItems(storable);
        if (storedItems.isEmpty()) {
            final java.lang.String itemName = ItemDefinitions.getOrThrow(storable.getBaseId()).getName().toLowerCase();
            final java.lang.String anyPrefix = "any";
            final java.lang.String prefix = storable.getItemIds().length > 1 || storable.getMaximumAmount() > 1 ? anyPrefix : Utils.getAOrAn(itemName);
        ...
        if (getTotalAmount(storable) >= maximumAmount) {
            final java.lang.String amountPrefix = maximumAmount == 1 ? "one" : maximumAmount == 100 ? "a hundred" : "a thousand";
            final java.lang.String name = storable.toString().toLowerCase().replaceAll("_", " ");
        ...
        for (final com.zenyte.game.item.Item storedItem : storedItems) {
            final com.zenyte.game.item.Item existingItem = getItem(storable);
            ...
            final java.util.OptionalInt transferrable = getStorableAmount(storable, Math.min(remaining, storedItem.getAmount()));
            ...
            if (storable == Storable.BOTTOMLESS_BUCKET) {
                ...
                for (int i = 0; i < 28; i++) {
                    final com.zenyte.game.item.Item it = inventory.getItem(i);
            ...
            final java.util.Map<java.lang.String, java.lang.Object> attributes = player.getInventory().deleteItem(deletedItem).getItem().getAttributesCopy();
            final com.zenyte.game.item.Item item = new Item(id, transferrableAmount, attributes);
    ...
    private OptionalInt getStorableAmount(final Storable storable, final int requestedAmount) {
        ...
        final com.zenyte.game.item.Item item = getItem(storable);
        if (item == null) {
    ...
    public void refresh() {
        for (final com.zenyte.game.content.skills.farming.Storable storable : Storable.values) {
    ...
    public ArrayList<Item> getStoredItems(final Storable storable) {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectCollection<com.zenyte.game.item.Item> items = player.getInventory().getContainer().getItems().values();
        final java.util.ArrayList<com.zenyte.game.item.Item> list = new ArrayList<Item>();
        for (int i : ids) {
            for (final com.zenyte.game.item.Item item : items) {
    ...
    public void setStorage(final List<Item> storage) {
        this.storage = storage;
    }
