...
public class SlayerMasterAssignmentD extends Dialogue {
    ...
    public void buildDialogue() {
        final com.zenyte.game.content.skills.slayer.SlayerMaster master = SlayerMaster.MAPPED_MASTERS.get(npc.getId());
        if (master == null) {
            return;
        }
        final com.zenyte.game.content.skills.slayer.Slayer slayer = player.getSlayer();
        final com.zenyte.game.content.skills.slayer.Assignment currentTask = slayer.getAssignment();
        final int zukKc = player.getNotificationSettings().getKillcount("tzkal-zuk");
        if (currentTask != null) {
            final java.lang.Class<? extends com.zenyte.game.world.region.Area> clazz = currentTask.getArea();
            if (master.equals(SlayerMaster.KONAR_QUO_MATEN) && clazz != null) {
                final com.zenyte.game.world.region.Area area = GlobalAreaManager.getArea(clazz);
            ...
            final com.zenyte.game.world.entity.player.Player partner = slayer.getPartner();
            ...
            if (task.getTask() instanceof BossTask) {
                npc("You\'re currently assigned to kill " + task.getTask().toString() + ". How many would you like to slay?").executeAction(() -> {
                    ...
                    player.sendInputInt("How many would you like to slay? (3 - 35)", value -> {
                        ...
                        if (partner != null) {
                            final com.zenyte.game.content.skills.slayer.Slayer partnerSlayer = partner.getSlayer();
                            if (partnerSlayer.getAssignment() == null && partner.getSkills().getLevelForXp(Skills.SLAYER) >= task.getTask().getSlayerRequirement() && partnerSlayer.isAssignable(task.getTask(), master)) {
            ...
            if (master.equals(SlayerMaster.KONAR_QUO_MATEN)) {
                final java.lang.Class<? extends com.zenyte.game.world.region.Area> clazz = task.getArea();
                if (clazz != null) {
                    final com.zenyte.game.world.region.Area area = GlobalAreaManager.getArea(clazz);
            ...
            if (task.getTask() instanceof BossTask) {
                ...
                if (task.getTask() == RegularTask.TZHAAR) {
                    ...
                    player(1000, "Yeah, I can beat jad.").executeAction(() -> {
                        final com.zenyte.game.content.skills.slayer.Assignment task = player.getSlayer().getTzTokJadAssignment(master);
                        player.getSlayer().setAssignment(task);
                        /** Replace existing string in the dialogue with new task. */
                        dialogue.put(1001, new NPCMessage(npc.getId(), Expression.DEFAULT, "Your new task is to kill " + task.getAmount() + " " + task.getTask().toString() + "."));
                        dialogue.put(5, new NPCMessage(npc.getId(), Expression.DEFAULT, task.getTask().getTip()));
