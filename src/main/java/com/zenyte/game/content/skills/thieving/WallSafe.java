...
public class WallSafe extends Action {
    ...
    public static final void onServerLaunch(final ServerLaunchEvent event) {
        WorldTasksManager.schedule(() -> {
            for (final it.unimi.dsi.fastutil.ints.Int2LongMap.Entry map : safesMap.int2LongEntrySet()) {
                final int hash = map.getIntKey();
                final long time = map.getLongValue();
                if (time < System.currentTimeMillis()) {
                    final com.zenyte.game.world.object.WorldObject object = World.getObjectWithType(hash, 10);
                    if (object != null) {
    ...
    private enum SafeLoot {
        LOW_COINS(995, 100, 250), MEDIUM_COINS(995, 200, 250), HIGH_COINS(995, 400, 250), SAPPHIRE(1623, 1, 50), EMERALD(1621, 1, 30), RUBY(1619, 1, 15), DIAMOND(1617, 1, 5);
        ...
        static {
            for (final com.zenyte.game.content.skills.thieving.WallSafe.SafeLoot value : values) {
                totalWeight += value.weight;
            }
        }

        private SafeLoot(final int id, final int amount, final int weight) {
            this.id = id;
            this.amount = amount;
            this.weight = weight;
        }
    ...
    public int processWithDelay() {
        ...
        if (Thieving.success(player, hasDisarmed ? 30 : 50)) {
            ...
            WorldTasksManager.schedule(() -> {
                ...
                final com.zenyte.game.world.object.WorldObject cracked = new WorldObject(object);
                cracked.setId(7238);
                ...
                int current = 0;
                for (final com.zenyte.game.content.skills.thieving.WallSafe.SafeLoot loot : SafeLoot.values) {
            ...
            WorldTasksManager.schedule(() -> {
                ...
                final com.zenyte.game.world.object.WorldObject object = World.getObjectWithType(player.getLocation(), 10);
                if (object != null) {
    ...
    public WallSafe(final WorldObject object) {
        this.object = object;
    }
}
