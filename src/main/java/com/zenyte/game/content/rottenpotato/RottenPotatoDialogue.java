...
public class RottenPotatoDialogue extends OptionsMenuD {
    ...
    public void handleClick(int slotId) {
        final com.zenyte.game.content.rottenpotato.handler.RottenPotatoActionHandler action = options.get(slotId);
    ...
    private void handlePlayerAction(final PlayerRottenPotatoActionHandler handler) {
        if (target.isPresent()) {
            final java.lang.Object target = this.target.get();
            ...
            player.sendInputString("Name of player?", name -> {
                final com.zenyte.game.world.entity.player.Player target = World.getPlayerByUsername(name);
                if (target == null) {
                    return;
                }
