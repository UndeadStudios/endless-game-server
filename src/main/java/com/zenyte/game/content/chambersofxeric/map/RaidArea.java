...
public abstract class RaidArea extends DynamicArea implements RoomController, DropPlugin, DeathPlugin, HitProcessPlugin, CannonRestrictionPlugin, TeleportPlugin, LogoutPlugin, LayableObjectPlugin {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RaidArea.class);
    /**
    ...
    protected Location getCrystalTile(final Location location) {
        final mgi.types.config.ObjectDefinitions defs = Objects.requireNonNull(ObjectDefinitions.get(30018));
    ...
    public void refreshStorageUnits(final int storageUnitId) {
        final com.zenyte.game.world.entity.Location[] units = getStorageUnitLocations();
        if (units == null) {
            return;
        }
        ...
        final com.zenyte.game.world.entity.Location tile = getLocation(units[index]);
        final com.zenyte.game.world.object.WorldObject existingObject = World.getObjectWithType(tile, 10);
    ...
    protected int getHighestLevel(final int skill) {
        int level = 1;
        for (final com.zenyte.game.world.entity.player.Player player : raid.getPlayers()) {
    ...
    public Location getObjectLocation(final Location tile, final int sizeX, final int sizeY, final int rotation) {
        final com.zenyte.game.world.entity.Location t = getLocation(tile);
    ...
    private final void updateStage(@NotNull final Player player) {
        ...
        final com.zenyte.game.content.chambersofxeric.map.RaidArea room = type == null ? this : Utils.findMatching(raid.getMap().getRaidChunks(), chunk -> chunk.getToPlane() == plane && (chunk.getType() == RaidRoom.FLOOR_START_UPSTAIRS || chunk.getType() == null));
        if (room == null) {
            return;
        }
        final com.zenyte.game.world.entity.Location boundTile = room.getBoundTile();
        if (boundTile == null) {
            return;
        }
    ...
    public void leave(final Player player, boolean logout) {
        final com.zenyte.game.world.region.Area nextArea = GlobalAreaManager.getArea(player.getLocation());
        if (logout || !(nextArea instanceof RaidArea)) {
            ...
            final org.apache.commons.lang3.mutable.MutableBoolean contained = new MutableBoolean();
            if (raid != null) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (ticks == 1) {
                    ...
                    final java.lang.StringBuilder builder = new StringBuilder(100);
                    final com.zenyte.game.content.chambersofxeric.map.RaidRoom type = getType();
                    final java.lang.Object killingBlow = player.getTemporaryAttributes().get("killing blow hit");
                    final java.lang.String source = killingBlow instanceof NPC ? ((NPC) killingBlow).getDefinitions().getName() : "Unknown source";
    ...
    public void hit(final Player source, final Entity target, final Hit hit, final float modifier) {
        ...
        if (target instanceof JewelledCrab) {
            final com.zenyte.game.content.chambersofxeric.npc.JewelledCrab crab = (JewelledCrab) target;
            hit.setDamage(0);
