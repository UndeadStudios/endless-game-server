...
public class CaptainRimor extends NPCPlugin {
    ...
    private List<String> generateList(@NotNull final Player player) {
        final java.lang.String[] options = RaidPattern.getLayoutCodes();
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<java.lang.String> filteredOptions = new ObjectArrayList<String>();
        final int currentSettings = player.getNumericAttribute("disabled raids layouts").intValue();
        for (int i = 0; i < options.length; i++) {
            final java.lang.String option = options[i];
            final java.lang.String chars = option.split(" - ")[0].replaceAll("[^FSCP]", Strings.EMPTY);
            ...
            final java.lang.String string = builder.toString().trim();
            final java.lang.String firstHalfOfString = string.substring(0, string.length() / 2);
            final java.lang.String secondHalfOfString = string.substring((string.length() / 2) + 1);
    ...
    private void sendInterface(@NotNull final Player player) {
        final java.util.List<java.lang.String> filteredOptions = generateList(player);
        final int currentSettings = player.getNumericAttribute("disabled raids layouts").intValue();
        final int unlockedSlotsCount = player.getNumericAttribute("raids layout count").intValue();
