...
public final class RaidHerblore {
    ...
    public enum RaidPotion {
        ...
        public static final Item[] getPotions(@NotNull final Player player, @NotNull final Item used, @NotNull final Item usedWith) {
            final com.zenyte.game.item.Item vial = used.getId() == 20801 ? used : usedWith.getId() == 20801 ? usedWith : null;
            final com.zenyte.game.item.Item other = vial == null ? null : vial == used ? usedWith : used;
            final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.item.Item> list = new ObjectArrayList<Item>();
            final int level = player.getSkills().getLevel(Skills.HERBLORE);
            for (final com.zenyte.game.content.chambersofxeric.skills.RaidHerblore.RaidPotion[] pots : RaidPotion.groupedValues) {
                RaidPotion potion = null;
                for (final com.zenyte.game.content.chambersofxeric.skills.RaidHerblore.RaidPotion pot : pots) {
                ...
                if (potion != null) {
                    final java.util.ArrayList<com.zenyte.game.item.Item> requiredItems = new ArrayList<Item>();
    ...
    public enum RaidHerb {
        ...
        private RaidHerb(final int level, final int grimy, final int clean, final int xp) {
            this.level = level;
            this.grimy = grimy;
            this.clean = clean;
            this.xp = xp;
    ...
    public static final class RaidHerbCleaningAction extends Action {
        ...
        public static final boolean clean(@NotNull final Player player, final int id, @NotNull final OptionalInt slot) {
            final com.zenyte.game.content.chambersofxeric.skills.RaidHerblore.RaidHerb raidHerb = RaidHerb.map.get(id);
            if (raidHerb == null) {
                return false;
            }
        ...
        public RaidHerbCleaningAction(final int id) {
            this.id = id;
        }
    }
    ...
    public static final class RaidMixDialogue extends SkillDialogue {
        ...
        public void run(final int slotId, final int amount) {
            ...
            final com.zenyte.game.item.Item item = items[slotId];
            if (item == null) {
                return;
            }
            final com.zenyte.game.content.chambersofxeric.skills.RaidHerblore.RaidPotion pot = RaidPotion.map.get(item.getId());
            if (pot == null) {
                return;
            }
            if (amount <= 0) {
    ...
    private static final class RaidPotionAction extends Action {
        ...
        private boolean check() {
            ...
            if (list != null && !list.isEmpty()) {
                ...
                int index = 0;
                for (final java.lang.String missingIngredients : list) {
                ...
                final java.lang.String potName = potion.toString().toLowerCase().replace("_", " ");
        ...
        public int processWithDelay() {
            ...
            final com.zenyte.game.item.Item pot = new Item(potion.potionId, 1);
            ...
            player.sendFilteredMessage("You mix your " + ItemDefinitions.getOrThrow(potion.herb.clean).getName().toLowerCase() + " together with your " + builder.toString() + " into a potion perfectly!");
            if (--amount <= 0) {
                return -1;
            }
            if (!check()) {
