...
public final class LizardmanShamanRoom extends RaidArea implements CycleProcessPlugin {
    ...
    public LizardmanShamanRoom(final RaidRoom type, final Raid raid, final int rotation, final int size, final int regionX, final int regionY, final int chunkX, final int chunkY, final int fromPlane, final int toPlane) {
        ...
        for (int i = 0; i < polyPoints.length; i++) {
            final com.zenyte.game.world.entity.Location tile = getLocation(polyPoints[i][0], polyPoints[i][1], toPlane);
    ...
    public void finish() {
        ...
        WorldTasksManager.schedule(() -> {
            for (final com.zenyte.game.world.entity.npc.NPC spawn : blockingSpawns) {
                if (spawn == null) {
                    continue;
                }
                spawn.sendDeath();
    ...
    public boolean isDead() {
        for (final com.zenyte.game.content.chambersofxeric.npc.LizardmanShaman lizardman : this.shamans) {
            if (!lizardman.isDead()) {
                return false;
            }
        }
     ...
     */
    private void wipe() {
        for (final com.zenyte.game.world.entity.Location tendril : spiritTendrilSpawnLocations[index]) {
            World.removeObject(World.getObjectWithType(getLocation(tendril), 10));
        }
        if (index == 0) {
            for (final com.zenyte.game.world.entity.Location tendril : extraSpiritTendrils) {
                World.removeObject(World.getObjectWithType(getLocation(tendril), 10));
            }
        }
    }
