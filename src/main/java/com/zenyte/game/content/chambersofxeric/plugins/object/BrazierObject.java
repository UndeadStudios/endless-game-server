...
public class BrazierObject implements ObjectAction {
    ...
    private boolean preconditions(@NotNull final Player player, @NotNull final WorldObject object, @NotNull final IceDemonRoom room) {
        ...
        if (object.getId() == 29747) {
            final com.zenyte.game.content.chambersofxeric.npc.IceDemon demon = room.getDemon();
    ...
    private void light(@NotNull final Player player, @NotNull final WorldObject object) {
        player.getRaid().ifPresent(raid -> raid.ifInRoom(player, IceDemonRoom.class, room -> {
            if (object.getId() == 29747) {
                ...
                final com.zenyte.game.content.chambersofxeric.npc.IcefiendNPC icefiend = Utils.findMatching(room.getIcefiends(), i -> i.getLocation().withinDistance(object, 1));
                final int amount = player.getInventory().getAmountOf(20799);
                final com.zenyte.game.content.chambersofxeric.room.IceDemonRoom.Brazier brazier = new IceDemonRoom.Brazier(room, amount * 5, icefiend, 29748, object.getType(), object.getRotation(), new Location(object));
            ...
            final com.zenyte.game.content.chambersofxeric.room.IceDemonRoom.Brazier brazier = Utils.findMatching(room.getBraziers(), b -> b == object);
            if (brazier == null) {
                return;
            }
            final int amount = player.getInventory().getAmountOf(20799);
