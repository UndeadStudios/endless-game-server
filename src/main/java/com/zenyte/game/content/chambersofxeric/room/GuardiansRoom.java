...
public final class GuardiansRoom extends RaidArea implements CycleProcessPlugin {
    ...
    public void finish() {
        ...
        for (final com.zenyte.game.world.entity.Location object : prevented) {
    ...
    private final void pushBack(final Player player, final int index) {
        ...
        final com.zenyte.game.world.entity.Location t = new Location(player.getLocation());
        WorldTasksManager.schedule(() -> {
            ...
            final com.zenyte.game.content.chambersofxeric.npc.RaidGuardianNPC n = npcs.stream().max((n2, n1) -> n1.getLocation().getDistance(player.getLocation()) > n2.getLocation().getDistance(player.getLocation()) ? 1 : -1).orElseThrow(RuntimeException::new);
            n.setFaceLocation(player.getLocation());
            n.setAnimation(RaidGuardianNPC.attack);
            final com.zenyte.game.world.entity.npc.NPCCombat combat = n.getCombat();
            ...
            final com.zenyte.game.content.chambersofxeric.npc.RaidGuardianNPC oppositeNPC = npcs.get(0) == n ? npcs.get(1) : npcs.get(0);
            if (oppositeNPC.isDead() || oppositeNPC.isFinished()) {
                return;
            }
            final com.zenyte.game.world.entity.Location oppositePreventedTile = getLocation(guardianSpawnTiles[this.index][2 + (index == 0 ? 1 : 0)]);
            oppositeNPC.stomp(oppositePreventedTile);
            oppositeNPC.setFaceLocation(player.getLocation());
            final com.zenyte.game.world.entity.npc.NPCCombat oppositeCombat = oppositeNPC.getCombat();
