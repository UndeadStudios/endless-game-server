...
public final class CrystalCluster extends WorldObject {
	...
	public boolean process() {
		...
		case 6: 
			{
				final com.zenyte.game.world.object.WorldObject existingObject = World.getObjectWithType(this, getType());
		...
		case 3: 
			{
				...
				this.setId(30034);
				final java.util.ArrayList<com.zenyte.game.world.entity.Location> tiles = new ArrayList<Location>(8);
				for (int x = -1; x <= 1; x++) {
					for (int y = -1; y <= 1; y++) {
						final com.zenyte.game.world.entity.Location tile = transform(x, y, 0);
				...
				for (Player player : room.getOlm().everyone(GreatOlm.ENTIRE_CHAMBER)) {
					...
					final com.zenyte.game.world.entity.masks.Hit hit = new Hit(room.getOlm(), Utils.random(10, 40), HitType.REGULAR);
					hit.setExecuteIfLocked();
					player.applyHit(hit);
					player.lock(1);
					player.sendMessage("The crystal beneath your feet grows rapidly and shunts you to the side.");
					final com.zenyte.game.world.entity.Location randomTile = tiles.get(Utils.random(tiles.size() - 1));
