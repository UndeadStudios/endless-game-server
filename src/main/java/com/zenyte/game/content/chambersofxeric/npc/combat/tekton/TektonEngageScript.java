		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (stage == 0) {
					...
					final java.util.List<com.zenyte.game.world.entity.Entity> possibleTargets = tekton.getPossibleTargets(EntityType.PLAYER);
					...
					if (tekton.isEnteredCombat()) {
						if (!tekton.isBoostedStats()) {
							final com.zenyte.game.world.entity.npc.combatdefs.NPCCombatDefinitions combat = tekton.getCombatDefinitions();
					...
					if (CombatUtilities.isWithinMeleeDistance(tekton, target)) {
						...
						engage = true;
						final com.zenyte.game.world.object.WorldObject obj = new WorldObject(30023, 10, 0, tekton.getX(), tekton.getY(), tekton.getPlane());
						tekton.setBlock(obj);
