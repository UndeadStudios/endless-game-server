...
public class RaidMap {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(RaidMap.class);
    /**
    ...
    public void construct() throws OutOfSpaceException {
        ...
        final java.awt.Rectangle boundaries = algorithm.boundaries;
    ...
    private void constructBoss() throws OutOfSpaceException {
        final com.zenyte.game.world.region.dynamicregion.AllocatedArea bossArea = MapBuilder.findEmptyChunk(5, 7);
    ...
    public void erase() {
        ...
        for (final com.zenyte.game.content.chambersofxeric.map.RaidArea chunk : this.raidChunks) {
        ...
        for (int z = 3; z >= 2; z--) {
            for (int x = allocatedArea.getChunkX(); x < (allocatedArea.getChunkX() + 16); x++) {
                for (int y = allocatedArea.getChunkY(); y < (allocatedArea.getChunkY() + 8); y++) {
                    final com.zenyte.game.content.chambersofxeric.map.WrapperChunk wrapperChunk = new WrapperChunk(1, 1, 416, 647, x, y, 0, z, Utils.random(3));
    ...
    private void constructRooms(@NotNull final AllocatedArea allocatedArea, @Nullable final MapPalette floorPalette, final int height) {
        ...
        for (final com.zenyte.game.content.chambersofxeric.map.MapChunk room : floorPalette) {
            final com.zenyte.game.content.chambersofxeric.map.RaidArea instance = getChunk(room.room, room.chunkDirection, (room.x * 4) + allocatedArea.getChunkX(), (room.y * 4) + allocatedArea.getChunkY(), room.direction.ordinal(), height);
    ...
    private final void bindTeleportationPositions() {
        ...
        for (int index = 0; index < raidChunks.size(); index++) {
            final com.zenyte.game.content.chambersofxeric.map.RaidArea chunk = raidChunks.get(index);
            ...
            if (type == RaidRoom.RAID_START) {
                ...
                final com.zenyte.game.content.chambersofxeric.map.RaidArea previousRoom = raidChunks.get(index - 1);
                ...
                final com.zenyte.game.content.chambersofxeric.map.RaidArea nextRoom = index == raidChunks.size() - 1 ? boss : raidChunks.get(index + 1);
