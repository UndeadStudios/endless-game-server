...
public final class VespulaRoom extends RaidArea implements PlayerCombatPlugin {
    ...
    public void loadRoom() {
        ...
        final com.zenyte.game.world.entity.Location[] array = portalHitTiles[index];
        for (final com.zenyte.game.world.entity.Location tile : array) {
    ...
    public void load() {
        ...
        loaded = true;
        for (int i = 0; i < 4; i++) {
            final com.zenyte.game.world.entity.Location spawnTile = getLocation(luxGrubs[index][i]).transform(-1, -1, 0);
            final com.zenyte.game.world.entity.Location soldierTile = getLocation(vespineSoldiers[index][i]).transform(-1, -1, 0);
            final com.zenyte.game.content.chambersofxeric.npc.LuxGrub grub = new LuxGrub(raid, this, spawnTile, soldierTile);
            grub.spawn();
            final com.zenyte.game.world.entity.Location faceLocation = getLocation(vespineSoldiers[index][i]);
            grub.setFaceLocation(faceLocation);
            grubs.add(grub);
        }
        for (int i = 0; i < 2; i++) {
            final com.zenyte.game.world.object.WorldObject object = new WorldObject(30070, 10, 0, this.getLocation(boilLocations[index][i]));
            boils.add(object);
        ...
        for (final com.zenyte.game.world.object.WorldObject root : originalMedivaemiaRoots) {
            final com.zenyte.game.world.object.WorldObject newRoot = new WorldObject(root);
            newRoot.setId(30068);
    ...
    public void spawnTendrils() {
        ...
        for (int i = 0; i < 2; i++) {
            final com.zenyte.game.world.object.WorldObject object = new WorldObject(30024, 10, 0, this.getLocation(tendrilLocations[index][i]));
            tendrils.add(object);
            World.spawnObject(object);
        }
    }
    ...
    public void clear() {
        ...
        for (final com.zenyte.game.content.chambersofxeric.npc.LuxGrub grub : this.grubs) {
            grub.kill();
        }
        for (final com.zenyte.game.world.object.WorldObject root : originalMedivaemiaRoots) {
        ...
        WorldTasksManager.schedule(() -> {
            for (final com.zenyte.game.world.object.WorldObject boil : boils) {
                final com.zenyte.game.world.object.WorldObject burstBoil = new WorldObject(boil);
                burstBoil.setId(30071);
                World.spawnObject(burstBoil);
            }
            for (final com.zenyte.game.world.object.WorldObject tendril : tendrils) {
