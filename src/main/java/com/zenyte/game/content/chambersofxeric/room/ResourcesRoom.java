...
public final class ResourcesRoom extends RaidArea implements RoomController, CycleProcessPlugin {
    ...
    public void loadRoom() {
        if (getType() == RaidRoom.RESOURCES_B) {
            final int level = getHighestLevel(Skills.HUNTER);
            final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.content.chambersofxeric.npc.Bat.BatType> bats = new ObjectArrayList<Bat.BatType>();
            final com.zenyte.game.content.chambersofxeric.npc.Bat.BatType bestBat = getBestBat(level);
            ...
            final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.Location> locations = new ObjectArrayList<>(Arrays.asList(ResourcesRoom.bats[index]));
            final int count = Math.min(6, 2 + (raid.getOriginalPlayers().size() >> 3));
            final com.zenyte.game.content.chambersofxeric.npc.Bat.BatType type = bats.get(Utils.random(bats.size() - 1));
            for (int i = 0; i < count; i++) {
                try {
                    final com.zenyte.game.content.chambersofxeric.npc.Bat bat = new Bat(raid, this, type.getId(), getLocation(locations.get(Utils.random(locations.size() - 1))));
                    bat.spawn();
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    private final Bat.BatType getBestBat(final int level) {
        ...
        for (final com.zenyte.game.content.chambersofxeric.npc.Bat.BatType batType : Bat.BatType.values) {
    ...
    public final void riseSnake() {
        ...
        attempts.increment();
        containsSnake = true;
        final com.zenyte.game.content.chambersofxeric.npc.CaveSnake snake = new CaveSnake(raid, this, caveSnakeCenter.transform(-1, -1, 0), fishingSpotPosition);
    ...
    public void process() {
        ...
        if (++ticks >= 50 || attempts.intValue() >= 10) {
            ticks = 0;
            attempts.setValue(0);
            final it.unimi.dsi.fastutil.ints.IntArrayList list = new IntArrayList(IntArray.of(0, 1, 2));
