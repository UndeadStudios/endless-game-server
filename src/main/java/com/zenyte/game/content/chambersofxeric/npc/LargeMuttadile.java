...
public final class LargeMuttadile extends RaidNPC<MuttadileRoom> implements CombatScript {
		...
		if (!unleashed) {
			...
			if (feedCount > 0 && !feeding && !room.getTree().isDead() && !room.getTree().isFinished() && count > 0 && getHitpoints() < getMaxHitpoints() / 2 && Utils.random(2) == 0) {
				...
				if (safespotTicks++ % 3 == 0) {
					...
					resetWalkSteps();
					final com.zenyte.game.world.entity.Location dest = getFaceLocation(target, 15, 1024);
	...
	private void iteratePath(@NotNull final Entity target, final int delay) {
		WorldTasksManager.schedule(new TickTask() {
			...
			public void run() {
				...
				final java.util.List<com.zenyte.game.world.entity.Location> path = Utils.calculateLine(current.getX(), current.getY(), target.getLocation().getX(), target.getLocation().getY(), current.getPlane());
				final int length = ticks == delay ? Integer.MAX_VALUE : tilesPerTick;
				for (int i = 0; i < length; i++) {
					if (i >= path.size()) {
						break;
					}
					final com.zenyte.game.world.entity.Location tile = path.get(i);
	...
	public int attack(final Entity target) {
		...
		if (meleeDistance && Utils.random(1) == 0) {
			...
			for (final com.zenyte.game.world.entity.player.Player player : room.getRaid().getPlayers()) {
