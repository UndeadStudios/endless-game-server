		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (ticks == 1) {
					...
					for (final com.zenyte.game.world.entity.npc.NPC firewall : firewalls) {
						firewall.finish();
					}
	...
	private void checkPlayers(final int hash, final boolean north, final List<Player> everyone) {
		for (final Player player : everyone) {
			if (player.getLocation().getPositionHash() == hash) {
				...
				final com.zenyte.game.world.entity.Location tile = player.getLocation().transform(north ? Direction.NORTH : Direction.SOUTH, 1);
