...
public final class RoomGeneration {
    ...
    public void generate() {
        ...
        final java.util.ArrayList<com.zenyte.game.content.chambersofxeric.map.RaidRoom> combatRooms = new ArrayList<RaidRoom>(bossPattern.random(raidPattern.getCombatRooms()));
        /* A list of scavenger rooms. */
        final java.util.ArrayList<com.zenyte.game.content.chambersofxeric.map.RaidRoom> scavengerRooms = new ArrayList<RaidRoom>(SCAVENGER_ROOMS);
        /* A list of farming/skilling rooms. */
        final java.util.ArrayList<com.zenyte.game.content.chambersofxeric.map.RaidRoom> farmingRooms = new ArrayList<RaidRoom>(FARMING_ROOMS);
        /* A list of puzzle rooms, consisting of ice demon, creature keeper, deathly room and crab room. */
        final java.util.ArrayList<com.zenyte.game.content.chambersofxeric.map.RaidRoom> puzzleRooms = new ArrayList<RaidRoom>(PUZZLE_ROOMS);
        /*
        ...
        final java.util.List<com.zenyte.game.content.chambersofxeric.map.LayoutTypeRoom> pattern = raidPattern.getPattern();
        int count = 0;
        /* Fill the remaining rooms in with random possible options. */
        for (final com.zenyte.game.content.chambersofxeric.map.LayoutTypeRoom room : pattern) {
            final com.zenyte.game.content.chambersofxeric.map.RoomType type = room.getType();
            final com.zenyte.game.util.Direction direction = room.getDirection();
            count++;
            if (type == START) {
            ...
            final java.util.ArrayList<com.zenyte.game.content.chambersofxeric.map.RaidRoom> rooms = type == SCAVENGERS ? scavengerRooms : type == FARMING ? farmingRooms : puzzleRooms;
            if (!rooms.isEmpty()) {
    ...
    private void setNext(final LayoutRoom room) {
        for (int i = 0; i < length; i++) {
            ...
            final com.zenyte.game.content.chambersofxeric.map.LayoutRoom[] floor = bottom ? bottomFloor : topFloor;
