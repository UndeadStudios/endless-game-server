...
public final class Sphere implements OlmCombatScript {
	...
	public void handle(final GreatOlm olm) {
		...
		final it.unimi.dsi.fastutil.ints.IntArrayList typelist = new IntArrayList(Arrays.asList(0, 1, 2));
		...
		while (random-- > 0) {
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					final com.zenyte.game.world.entity.masks.Hit hit = new Hit(olm, Math.min(99, player.getHitpoints() / 2), HitType.REGULAR);
