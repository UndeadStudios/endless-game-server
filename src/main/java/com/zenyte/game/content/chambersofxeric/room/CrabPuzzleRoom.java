...
public final class CrabPuzzleRoom extends RaidArea implements PlayerCombatPlugin {
   ...
   private Location getCrystalTile() {
      final mgi.types.config.ObjectDefinitions defs = Objects.requireNonNull(ObjectDefinitions.get(29753));
   ...
   public void transformCrystal(final int index) {
      final com.zenyte.game.world.object.WorldObject crystal = new WorldObject(crystalId, 10, getRotation(), getCrystalTile());
      if (!crystals[index]) {
         crystals[index] = true;
      ...
      WorldTasksManager.schedule(() -> {
         for (final com.zenyte.game.content.chambersofxeric.npc.JewelledCrab crab : crabs) {
            crab.sendDeath();
         }
      }, 1);
      WorldTasksManager.schedule(() -> {
   ...
   public void onAttack(final Player player, final Entity entity, final String style) {
      if (entity instanceof JewelledCrab) {
         final com.zenyte.game.world.entity.player.Action action = player.getActionManager().getAction();
         WorldTasksManager.schedule(() -> {
