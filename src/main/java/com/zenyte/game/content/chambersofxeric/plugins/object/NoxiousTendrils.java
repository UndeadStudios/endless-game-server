    ...
    public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
        player.getRaid().ifPresent(raid -> raid.ifInRoom(player, MuttadileRoom.class, room -> {
            ...
            if (!room.getTendrilPlayers().add(player.getUsername())) {
                final com.zenyte.game.world.entity.masks.Hit hit = new Hit((int) ((player.getHitpoints() * 0.15F) + (player.getHitpoints() * 0.075F)), HitType.REGULAR);
                hit.setExecuteIfLocked();
                player.applyHit(hit);
            }
            final com.zenyte.game.world.entity.Location destTile = new Location(player.getLocation());
            ...
            final com.zenyte.game.world.object.WorldObject obj = World.getObjectWithType(destTile, 10);
            if (obj != null && obj.getId() == object.getId()) {
                ...
                final com.zenyte.game.world.entity.Location secondaryTile = new Location(destTile);
