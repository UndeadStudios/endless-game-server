...
public class VespineSoldier extends RaidNPC<VespulaRoom> implements CombatScript {
    ...
    public void processNPC() {
        ...
        if (getId() == FLYING) {
            ...
            if (target == null) {
                final com.zenyte.game.world.entity.Entity combatTarget = combat.getTarget();
                if (combatTarget != null && combatTarget.getLocation().withinDistance(getMiddleLocation(), 15)) {
                    this.target = combatTarget;
                } else {
                    final java.util.List<com.zenyte.game.world.entity.Entity> possibleTargets = getPossibleTargets(EntityType.PLAYER);
    ...
    public void updateCombatDefinitions() {
        ...
        final com.zenyte.game.world.entity.npc.combatdefs.NPCCombatDefinitions cachedDefs = NPCCombatDefinitions.clone(getId(), NPCCDLoader.get(getId()));
        final double challengeModeMultiplier = raid.isChallengeMode() ? 1.5 : 1.0;
        final com.zenyte.game.world.entity.npc.combatdefs.StatDefinitions statDefinitions = cachedDefs.getStatDefinitions();
        for (final com.zenyte.game.world.entity.npc.combatdefs.StatType aggressiveStat : aggressiveStats) {
    ...
    public void rise() {
        ...
        final com.zenyte.game.content.chambersofxeric.npc.Vespula vespula = room.getVespula();
        final com.zenyte.game.content.chambersofxeric.npc.AbyssalPortal portal = room.getAbyssalPortal();
        if (!portal.isDead()) {
    ...
    private void explode() {
        final com.zenyte.game.world.entity.Location tile = new Location(getMiddleLocation());
        finish();
    ...
    public void onFinish(final Entity source) {
        ...
        target = null;
        for (final com.zenyte.game.content.chambersofxeric.npc.LuxGrub grub : this.room.getGrubs()) {
            final com.zenyte.game.content.chambersofxeric.npc.VespineSoldier relatedSoldier = grub.getSoldier();
    ...
    public VespineSoldier grantPoints(final boolean grantPoints) {
