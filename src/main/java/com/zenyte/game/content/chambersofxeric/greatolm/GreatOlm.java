...
public final class GreatOlm extends RaidNPC<OlmRoom> implements CombatScript {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(GreatOlm.class);
    /**
    ...
    public GreatOlm(final OlmRoom room, final Location tile) {
        ...
        this.combat = new NPCCombat(this) {
            ...
            public boolean process() {
                ...
                if (combatDelay <= 0) {
                    final java.util.List<com.zenyte.game.world.entity.player.Player> targets = everyone(ENTIRE_CHAMBER);
                    if (targets.isEmpty()) {
                        combatDelay = 4;
                        return true;
                    }
    ...
    public static final void douseFirewall(final Player player, final NPC npc) {
        player.getRaid().ifPresent(raid -> raid.ifInRoom(player.getLocation(), OlmRoom.class, room -> {
            ...
            final com.zenyte.game.content.skills.magic.Spellbook spellbook = player.getCombatDefinitions().getSpellbook();
            ...
            if (spellbook.equals(Spellbook.LUNAR)) {
                final com.zenyte.game.content.skills.magic.spells.NPCSpell humidify = Objects.requireNonNull(humidifySpell);
                final com.zenyte.game.content.skills.magic.SpellState state = new SpellState(player, humidify.getLevel(), humidify.getRunes());
                ...
                for (final com.zenyte.game.world.entity.player.action.combat.magic.CombatSpell spell : waterSpells) {
                    final com.zenyte.game.content.skills.magic.SpellState state = new SpellState(player, spell);
    ...
    public static final void douse(@NotNull final Player player, @NotNull final NPC npc, @NotNull final CombatSpell spell) {
        ...
        state.remove();
        final com.zenyte.game.world.Projectile sp = spell.getProjectile();
        final com.zenyte.game.world.Projectile projectile = new Projectile(sp.getGraphicsId(), sp.getStartHeight(), sp.getEndHeight(), sp.getDelay(), sp.getAngle(), 9, sp.getDistanceOffset(), 0);
        ...
        final com.zenyte.game.world.entity.masks.Animation animation = spell.getAnimation();
        final com.zenyte.game.world.entity.masks.Graphics graphics = spell.getCastGfx();
        if (animation != null) {
        ...
        final com.zenyte.game.world.entity.masks.Graphics gfx = spell.getHitGfx();
        if (gfx != null) {
            World.sendGraphics(new Graphics(gfx.getId(), 60, gfx.getHeight()), new Location(npc.getLocation()));
        }
        final com.zenyte.game.world.entity.SoundEffect hitSound = spell.getHitSound();
        if (hitSound != null) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (restoreStage == 2) {
                    ...
                    final com.zenyte.game.content.chambersofxeric.greatolm.RightClaw rightClaw = room.getRightClaw();
                    WorldTasksManager.schedule(() -> {
                        if (leftClaw != null) {
    ...
    public final List<Player> everyone(final int coverage) {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.player.Player> players = new ObjectArrayList<Player>();
        for (final com.zenyte.game.world.entity.player.Player p : room.getRaid().getPlayers()) {
    ...
    private final void executeDeath() {
        try {
            ...
            for (final com.zenyte.game.world.entity.player.Player player : everyone(ENTIRE_CHAMBER)) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                if (loop == 0) {
                    ...
                    try {
                        final com.zenyte.game.content.chambersofxeric.Raid raid = room.getRaid();
                        ...
                        final com.zenyte.game.content.chambersofxeric.rewards.RaidRewards rewards = room.getRaid().getRewards();
                        rewards.generate();
                        raid.setComplete();
                        final com.zenyte.game.world.object.WorldObject object = World.getObjectWithType(room.getLocation(crystalChestPosition), 10);
                        if (object != null) {
    ...
    private void resetHeadStance() {
        final com.zenyte.game.content.chambersofxeric.greatolm.OlmAnimation latestAnimation = room.getLatestHeadAnimation();
        WorldTasksManager.schedule(() -> {
    ...
    public int attack(final Entity target) {
        ...
        final com.zenyte.game.content.chambersofxeric.greatolm.GreatOlm.OlmAttackType[] array = this.isPenultimatePhase() || this.isFinalStand() ? penultimateAttackSequence : attackSequence;
        if (attackCounter >= (array.length - 1)) {
            this.attackCounter = 0;
        }
        final com.zenyte.game.content.chambersofxeric.greatolm.GreatOlm.OlmAttackType attackType = array[attackCounter];
