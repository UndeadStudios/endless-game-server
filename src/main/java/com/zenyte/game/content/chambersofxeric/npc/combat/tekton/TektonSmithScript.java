...
public final class TektonSmithScript implements TektonScript {
	...
	public void execute(final Tekton tekton) {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (ticks % 4 == 0) {
					final java.util.ArrayList<com.zenyte.game.world.entity.Location> sparks = new ArrayList<>(tekton.getRoom().getSparks());
					final java.util.List<com.zenyte.game.world.entity.player.Player> availablePlayers = tekton.getRoom().getPlayers().stream().filter(p -> !p.getLocation().withinDistance(tekton.getRoom().getSafeTile(), 1)).collect(Collectors.toList());
	...
	private void sendSparks(final Tekton tekton, final Entity target, final List<Location> sparks, final int count) {
		final com.zenyte.game.content.chambersofxeric.room.TektonRoom room = tekton.getRoom();
		final com.zenyte.game.world.entity.Location to = new Location(target.getLocation());
		for (int i = 0; i < count; i++) {
			final com.zenyte.game.world.entity.Location from = sparks.remove(Utils.random(sparks.size() - 1));
