...
public class RaidingPartiesInterface extends Interface {
    ...
    public static final void refresh(@NotNull final Player player) {
        ...
        for (int i = 0; i < 40; i++) {
            ...
            final com.zenyte.game.content.chambersofxeric.party.RaidParty party = advertisedParties.get(i);
            final java.lang.String name = Colour.WHITE.wrap(Utils.formatString(Utils.formatString(party.getPlayer())));
    ...
    protected void build() {
        ...
        bind("Make party", player -> {
            ...
            if (channel.getOwner().equalsIgnoreCase(GameConstants.SERVER_NAME)) {
                player.sendMessage("You cannot create a raiding party out of the main " + GameConstants.SERVER_NAME + " channel.");
                return;
            }
        ...
        bind("View party", (player, slotId, itemId, option) -> {
            final com.zenyte.game.content.chambersofxeric.party.RaidParty party = advertisedParties.get(slotId);
            if (party == null) {
                return;
            }
