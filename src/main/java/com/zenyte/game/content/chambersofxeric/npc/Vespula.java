...
public class Vespula extends RaidNPC<VespulaRoom> implements CombatScript {
    ...
    public void processNPC() {
        ...
        if (id != CRAWLING) {
            ...
            if (room.getAbyssalPortal().isUnderAttack()) {
                if (id == ENRAGED) {
                    for (final com.zenyte.game.world.entity.player.Player player : room.getPlayers()) {
                    ...
                    final java.util.List<com.zenyte.game.world.entity.player.Player> targetsInMeleeDistance = CharacterLoop.find(getLocation(), getSize() + 1, Player.class, target -> CombatUtilities.isWithinMeleeDistance(this, target) && !Utils.collides(getX(), getY(), getSize(), target.getX(), target.getY(), target.getSize()));
                    stingDelay = 2;
                    Location tile;
                    if (!targetsInMeleeDistance.isEmpty()) {
                        tile = targetsInMeleeDistance.get(Utils.random(targetsInMeleeDistance.size() - 1)).getLocation();
                        for (final com.zenyte.game.world.entity.player.Player target : targetsInMeleeDistance) {
            ...
            if (id == NORMAL) {
                ...
                if (this.stinging != null && this.stinging.canBeStung()) {
                    if (!hasWalkSteps()) {
                        final com.zenyte.game.content.chambersofxeric.npc.LuxGrub stinging = this.stinging;
                        this.stinging = null;
                ...
                final java.util.Optional<com.zenyte.game.content.chambersofxeric.npc.LuxGrub> farthestGrub = getFarthestUnstingedGrub();
    ...
    private final Optional<LuxGrub> getFarthestUnstingedGrub() {
        ...
        final java.util.List<com.zenyte.game.content.chambersofxeric.npc.LuxGrub> grubs = room.getGrubs();
        LuxGrub farthestGrub = null;
        final com.zenyte.game.world.entity.Location location = getMiddleLocation();
        for (final com.zenyte.game.content.chambersofxeric.npc.LuxGrub grub : grubs) {
            if (!grub.canBeStung()) {
                continue;
            }
    ...
    public int attack(final Entity target) {
        ...
        final com.zenyte.game.world.Projectile projectile = getId() == CRAWLING ? crawlingAttackProjectile : flyingAttackProjectile;
        ...
        final com.zenyte.game.world.entity.Location destTile = new Location(target.getLocation(), 2);
    ...
    public boolean isSkipStinging() {
        return this.skipStinging;
    }

    public void setSkipStinging(final boolean skipStinging) {
