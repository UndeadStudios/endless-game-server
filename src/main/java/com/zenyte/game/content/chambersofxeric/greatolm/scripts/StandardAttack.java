...
public final class StandardAttack implements OlmCombatScript {
	...
	public void handle(final GreatOlm olm) {
		...
		final com.zenyte.game.world.entity.Location face = olm.getFaceCoordinates();
		final java.util.List<com.zenyte.game.world.entity.player.Player> everyone = olm.everyone(olm.getDirection());
		if (everyone.isEmpty()) {
			return;
		}
		olm.performAttack();
		...
		for (final com.zenyte.game.world.entity.player.Player player : everyone) {
			...
			CombatUtilities.delayHit(olm, rangedProjectile.getTime(face, player.getLocation()), player, new Hit(olm, CombatUtilities.getRandomMaxHit(olm, 33 + (olm.getRoom().getRaid().getOriginalPlayers().size() / 8), ranged ? CombatScript.RANGED : CombatScript.MAGIC, player), ranged ? HitType.RANGED : HitType.MAGIC));
		}
	}
}
