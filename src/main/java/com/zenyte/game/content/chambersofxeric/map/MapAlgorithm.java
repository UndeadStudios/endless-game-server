...
public class MapAlgorithm {
    ...
    private void generatePath(@NotNull final LayoutRoom[] rooms, @NotNull final MapPalette palette, final boolean forceLastRoom) {
        int index = 0;
        final com.zenyte.game.content.chambersofxeric.map.LayoutRoom secondRoom = rooms[1];
        assert secondRoom != null;
        final com.zenyte.game.util.Direction secondRoomEntryPos = secondRoom.getDirection();
        ...
        for (final com.zenyte.game.content.chambersofxeric.map.LayoutRoom room : rooms) {
            final com.zenyte.game.util.Direction dir = index + 1 >= rooms.length ? Direction.SOUTH : rooms[index + 1].getDirection();
    ...
    private void calculateBoundaries() {
        ...
        int maxX = 0;
        int maxY = 0;
        for (final com.zenyte.game.content.chambersofxeric.map.MapPalette floor : Utils.concatenate(topFloorPalette, middleFloorPalette, bottomFloorPalette)) {
            if (floor == null) {
                continue;
            }
            for (final com.zenyte.game.content.chambersofxeric.map.MapChunk room : floor) {
                assert room.x >= minX;
                assert room.y >= minY;
                if (room.x > maxX) {
                    maxX = room.x;
