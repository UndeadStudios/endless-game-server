...
public final class LizardmanShaman extends RaidNPC<LizardmanShamanRoom> implements CombatScript {
	...
	public void processNPC() {
		...
		if (getCombat().getTarget() != null && Utils.random(60) == 0 && lastTime < Utils.currentTimeMillis()) {
			...
			final it.unimi.dsi.fastutil.ints.IntArrayList tiles = new IntArrayList();
			int count = 0;
			Location tile = null;
			while (true) {
				if (++count > 50) {
			...
			if (target instanceof Player) {
				...
				final java.util.List<com.zenyte.game.world.entity.player.Player> players = CharacterLoop.find(getMiddleLocation(), 2, Player.class, player -> !player.isDead());
	...
	public int attack(final Entity target) {
		final com.zenyte.game.content.chambersofxeric.npc.LizardmanShaman npc = this;
					...
					public void run() {
						...
						if (ticks == 0) {
							...
							CharacterLoop.forEach(targetTile, 15, Player.class, p -> {
								final com.zenyte.game.world.entity.player.Action action = p.getActionManager().getAction();
							...
							for (final com.zenyte.game.world.entity.player.Player p : CharacterLoop.find(targetTile, 1, Player.class, player -> !player.isDead() && (npc.isMultiArea() || player == target))) {
		...
		case 4: 
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					...
					for (final com.zenyte.game.world.entity.player.Player p : CharacterLoop.find(tt, 2, Player.class, player -> !player.isDead() && (npc.isMultiArea() || player == target))) {
