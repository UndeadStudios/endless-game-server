...
public final class VasaNistirio extends RaidNPC<VasaNistirioRoom> {
					...
					public void run() {
						...
						for (final Player r : raid.getPlayers()) {
							if (r.getLocation().withinDistance(pTile, 1)) {
								...
								final com.zenyte.game.world.entity.masks.Hit hit = new Hit(VasaNistirio.this, damage, HitType.RANGED);
	...
	private void refreshNextCrystal() {
		final it.unimi.dsi.fastutil.ints.IntArrayList availableCrystals = new IntArrayList(4);
	...
	private void walkToCenter() {
		...
		final com.zenyte.game.world.entity.pathfinding.RouteResult steps = RouteFinder.findRoute(getX(), getY(), getPlane(), getSize(), new TileStrategy(tile.getX(), tile.getY()), true);
	...
	private void sendExplosionAttack() {
		...
		for (final com.zenyte.game.world.entity.player.Player player : nearbyPlayers) {
			player.stun(10);
		}
