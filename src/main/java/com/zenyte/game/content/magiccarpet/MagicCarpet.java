...
public abstract class MagicCarpet {
	...
	public void travel() {
		...
		final com.zenyte.game.world.entity.pathfinding.events.player.TileEvent event = new TileEvent(player, new TileStrategy(getStartNextToCarpetLocation()), () -> {
			final com.zenyte.game.world.entity.Location dest = getStartLocation();
	...
	private void execute() {
		final com.zenyte.game.content.magiccarpet.CarpetMovement[] movements = forward ? getForwardMovement() : getBackwardsMovement();
			...
			public void run() {
				...
				if (movements[index].getSteps() != null) {
					...
					while (true) {
						...
						final com.zenyte.game.world.entity.Location step = movements[tempIndex++].getSteps();
						if (step != null) {
				...
				final com.zenyte.game.world.entity.masks.ForceMovement fm = movements[index].getForceMovement();
				if (fm != null) {
					...
					final com.zenyte.game.world.entity.masks.ForceMovement forceMovement = new ForceMovement(currentTile, 0, fm.getToFirstTile(), fm.getFirstTileTicketDelay(), fm.getDirection());
		...
		WorldTasksManager.schedule(new TickTask() {
			...
			public void run() {
				if (ticks == 0) {
					...
					final com.zenyte.game.world.entity.Location offCarpet = getEndNextToCarpetLocation();
	...
	public MagicCarpet(final Player player, final boolean forward) {
		this.player = player;
