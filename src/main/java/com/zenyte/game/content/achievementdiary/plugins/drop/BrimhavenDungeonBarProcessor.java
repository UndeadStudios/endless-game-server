...
public class BrimhavenDungeonBarProcessor extends DropProcessor {
    ...
    public Item drop(NPC npc, Player killer, Drop drop, Item item) {
        if (killer.inArea("Brimhaven Dungeon") && killer.getBooleanAttribute("noted_bars_metalic_dragons") && DiaryReward.KARAMJA_GLOVES4.eligibleFor(killer)) {//TODO area for brimhaven dungeon
