...
public enum KaramjaDiary implements Diary {
	PICK_BANANAS(EASY, true, "Pick 5 bananas from the plantation located east of the", "volcano."), USE_ROPE_SWING(EASY, true, "Use the rope swing to travel to the small island north-west", "of Karamja, where the moss giants are."), MINE_GOLD(EASY, "Mine some gold from the rocks on the north-west", "peninsula of Karamja."), //done
	...
	EXPLORE_CAIRN_ISLAND(EASY, true, "Explore Cairn Island to the west of Karamja."), USE_FISHING_SPOTS(EASY, KARAMJA_FISHING_AREA::inArea, "Use the Fishing spots north of the banana plantation."), //done
	COLLECT_SEAWEED(EASY, true, "Collect 5 seaweed from anywhere on Karamja."), ATTEMPT_FIGHT_PITS_OR_CAVES(EASY, NO_PREDICATE, "Attempt the TzHaar Fight Pits or Fight Cave."), //done
	KILL_A_JOGRE(EASY, true, "Kill a jogre in the Pothole dungeon."), CLAIM_AN_AGILITY_ARENA_TICKET(MEDIUM, true, "Claim a ticket from the Agility Arena in Brimhaven."), DISCOVER_HIDDEN_WALL(MEDIUM, true, "Discover hidden wall in the dungeon below the volcano."), VISIT_THE_ISLE_OF_CRANDOR(MEDIUM, true, "Visit the Isle of Crandor via the dungeon below the", "volcano."), USE_CART_SERVICE(MEDIUM, true, "Use Vigroy and Hajedy\'s cart service."), EARN_100_PERCENT_FAVOUR(MEDIUM, true, "Earn 100% favour in the village of Tai Bwo Wannai."), COOK_A_SPIDER_ON_A_STICK(MEDIUM, true, "Cook a spider on a stick."), TRAVEL_TO_PORT_KHAZARD(MEDIUM, true, "Charter the Lady of the Waves from Cairn Isle to Port", "Khazard."), CUT_A_TEAK_LOG(MEDIUM, "Cut a log from a teak tree."), //done
	...
	EXCHANGE_GEMS(MEDIUM, true, "Exchange gems for a machete."), USE_GNOME_GLIDER(MEDIUM, p -> !p.inArea("Karamja"), "Use the gnome glider to travel to Karamja."), GROW_A_HEALTHY_FRUIT_TREE(MEDIUM, "Grow a healthy fruit tree in the patch near Brimhaven."), //done
	TRAP_A_HORNED_GRAAHK(MEDIUM, NO_PREDICATE, "Trap a horned graahk."), //done
	CHOP_THE_BRIMHAVEN_DUNGEON_VINES(MEDIUM, NO_PREDICATE, "Chop the vines to gain deeper access to Brimhaven", "Dungeon."), CROSS_THE_LAVA(MEDIUM, NO_PREDICATE, "Cross the lava using the stepping stones within Brimhaven", "Dungeon."), //done
	...
	MINE_A_RED_TOPAZ(MEDIUM, NO_PREDICATE, "Mine a red topaz from a gem rock."), BECOME_FIGHT_PITS_CHAMPION(HARD, true, "Become the Champion of the Fight Pits."), KILL_KET_ZEK(HARD, NO_PREDICATE, "Successfully kill a Ket-Zek in the Fight Caves."), //done
	EAT_AN_OOMLIE_WRAP(HARD, true, "Eat an oomlie wrap."), CRAFT_NATURE_RUNES(HARD, NO_PREDICATE, "Craft some nature runes."), //done
	COOK_A_KARAMBWAN(HARD, "Cook a karambwan thoroughly."), //done
	KILL_A_DEATHWING(HARD, true, "Kill a deathwing in the dungeon under the Kharazi Jungle."), USE_THE_CROSSBOW_SHORTCUT(HARD, true, "Use the crossbow short cut south of the volcano."), COLLECT_PALM_LEAVES(HARD, true, 5, "Collect 5 palm leaves."), SLAYER_TASK_BY_DURADEL(HARD, "Be assigned a Slayer task by Duradel north of Shilo", "Village."), //done
	...
	static {
		for (final com.zenyte.game.content.achievementdiary.Diary.DiaryComplexity type : DiaryComplexity.VALUES) {
			MAP.put(type, new ArrayList<>(15));
		}
		for (final com.zenyte.game.content.achievementdiary.diaries.KaramjaDiary value : VALUES) {
