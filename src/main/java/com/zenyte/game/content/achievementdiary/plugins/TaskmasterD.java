...
public class TaskmasterD extends Dialogue {
    ...
    public TaskmasterD(final Player player, final NPC npc) {
        ...
        final java.util.Map<com.zenyte.game.content.achievementdiary.DiaryArea, java.util.Map<com.zenyte.game.content.achievementdiary.DiaryReward, java.lang.Boolean>> pendingRewards = player.getAchievementDiaries().getPendingRewards();
        if (!pendingRewards.get(diary.area()).isEmpty()) {
            final java.util.List<com.zenyte.game.content.achievementdiary.DiaryReward> unclaimedRewards = pendingRewards.get(diary.area()).entrySet().stream().filter(bool -> bool.getValue().equals(false)).map(Map.Entry::getKey).collect(Collectors.toList());
        ...
        if (pendingReward != null) {
            ...
            player("Yes please!").executeAction(() -> {
                if (!player.getInventory().checkSpace(2)) {
                    ...
                    final com.zenyte.game.item.Item lampItem = new Item(pendingReward.getLamp().getItem());
