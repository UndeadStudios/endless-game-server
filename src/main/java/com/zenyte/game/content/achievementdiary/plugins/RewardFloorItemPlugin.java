...
public class RewardFloorItemPlugin implements FloorItemPlugin {
    ...
    public void handle(Player player, FloorItem item, int optionId, String option) {
        if (option.equals("Take")) {
            ...
            final com.zenyte.game.content.achievementdiary.DiaryReward bestReward = DiaryReward.getBestEligibleReward(player, reward.getArea());
            if (bestReward != null) {
    ...
    public int[] getItems() {
        ...
        for (final com.zenyte.game.content.achievementdiary.DiaryReward diary : DiaryReward.VALUES) {
