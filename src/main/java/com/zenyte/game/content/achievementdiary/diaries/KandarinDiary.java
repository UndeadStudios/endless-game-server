...
public enum KandarinDiary implements Diary {
    ...
    PLAY_THE_ORGAN(EASY, true, "Play the Organ in Seers\' Church."), PLANT_JUTE_SEEDS(EASY, NO_PREDICATE, "Plant some Jute seeds in the patch north of McGrubor\'s", "Wood."), //done
    ...
    CREATE_YEW_LONGBOW(HARD, false, 1 | 2 | 4, SEERS_VILLAGE_CATHERBY_TERRITORY::inArea, "Create a Yew Longbow from scratch around Seers\' Village.") {
        @Override
        public boolean flagging() {
            return true;
        }
    ...
    CONSTRUCT_A_PYRE_SHIP(ELITE, true, "Construct a Pyre ship from Magic Logs.<col=800000>(Requires Chewed", "<col=800000>Bones.)<col=000000>"), TELEPORT_TO_CATHERBY(ELITE, NO_PREDICATE, "Teleport to Catherby.");
    //done
    ...
    static {
        ...
        for (final com.zenyte.game.content.achievementdiary.diaries.KandarinDiary value : VALUES) {
    ...
    public DiaryChunk[] chunks() {
        return new DiaryChunk[] {new DiaryChunk(6307, 10, 4515), new DiaryChunk(6308, 14, 4516), new DiaryChunk(6309, 11, 4517), new DiaryChunk(6310, 7, 4518)};
    }
