...
public interface Diary {
	...
	static void sendDiary(final Player player, final Map<DiaryComplexity, List<Diary>> values) {
		...
		final com.zenyte.game.content.achievementdiary.Diary firstEntry = values.get(EASY).get(0);
		final java.lang.String name = firstEntry.area().getAreaName();
		final java.util.ArrayList<java.lang.String> list = new ArrayList<String>(100);
		final java.lang.String prefix = "<str>";
		...
		for (final com.zenyte.game.content.achievementdiary.Diary.DiaryComplexity difficulty : DiaryComplexity.VALUES) {
			final java.util.List<com.zenyte.game.content.achievementdiary.Diary> diary = values.get(difficulty);
			if (difficulty == EASY) {
			...
			list.add("<col=000000>");
			addSpaces(list, 1);
			for (final com.zenyte.game.content.achievementdiary.Diary task : diary) {
				final java.lang.String pref = (diaries.getProgress(task) == task.objectiveLength()) || task.autoCompleted() ? prefix : "<col=000000>";
				for (final java.lang.String line : task.task()) {
