...
public enum WildernessDiary implements Diary {
	...
	CHOP_YEW_LOGS(MEDIUM, "Chop some yew logs from a fallen Ent."), ENTER_WILDERNESS_GODWARS_DUNGEON(MEDIUM, "Enter the Wilderness Godwars Dungeon."), COMPLETE_AGILITY_COURSE_LAP(MEDIUM, "Complete a lap of the Wilderness Agility course."), //done
	...
	//done
	TAKE_AGILITY_SHORTCUT(HARD, "Take the agility shortcut from Trollheim into the", "Wilderness."), KILL_A_SPIRITUAL_WARRIOR(HARD, p -> p.inArea(WildernessGodwarsDungeon.class), "Kill a Spiritual warrior in the Wilderness Godwars Dungeon."), FISH_RAW_LAVA_EEL(HARD, true, "Fish some Raw Lava Eel in the Wilderness."), KILL_CALLISTO(ELITE, 1 | 2 | 4, "Kill Callisto, Venenatis & Vet\'ion.") {
		@Override
		public boolean flagging() {
			return true;
		}
	...
	SMITH_RUNE_SCIMITAR(ELITE, true, "Smith a rune scimitar from scratch in the Resource Area."), STEAL_FROM_ROGUES_CHEST(ELITE, "Steal from the Rogues\' chest."), SLAY_A_SPIRITRUAL_MAGE(ELITE, p -> p.inArea(WildernessGodwarsDungeon.class), "Slay a spiritual mage inside the wilderness Godwars", "Dungeon."), CUT_AND_BURN_MAGIC_LOGS(ELITE, false, 1 | 2, WILDERNESS_RESOURCE_AREA::inArea, "Cut and burn some magic logs in the Resource Area.") {
		@Override
		public boolean flagging() {
			return true;
		}
	...
	public DiaryChunk[] chunks() {
		return new DiaryChunk[] {new DiaryChunk(6323, 12, 4507), new DiaryChunk(6324, 11, 4508), new DiaryChunk(6325, 10, 4509), new DiaryChunk(6326, 7, 4510)};
	}
