...
public enum ScrollBookTeleport {
    ...
    static {
        for (final com.zenyte.game.content.itemtransportation.masterscrolls.ScrollBookTeleport value : values) {
            map.put(value.scroll, value);
        }
    }

    final int getScrollId() {
        final com.zenyte.game.item.Item[] items = scroll.getItems();
    ...
    private ScrollBookTeleport(final TeleportScroll scroll, final int varbit) {
        this.scroll = scroll;
        this.varbit = varbit;
    }
}
