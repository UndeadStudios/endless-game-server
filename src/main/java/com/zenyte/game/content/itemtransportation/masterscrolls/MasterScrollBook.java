...
public class MasterScrollBook extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handle() {
        ...
        bind("Teleport", (player, item, slotId) -> {
            final java.util.Optional<com.zenyte.game.content.itemtransportation.masterscrolls.ScrollBookTeleport> optionalFavourite = MasterScrollBookInterface.getFavourite(player);
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final com.zenyte.game.item.Item book = from.getId() == ItemId.MASTER_SCROLL_BOOK_EMPTY || from.getId() == ItemId.MASTER_SCROLL_BOOK ? from : to;
        final com.zenyte.game.item.Item scrolls = from == book ? to : from;
