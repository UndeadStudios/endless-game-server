...
public class DyingKnight extends NPCPlugin {
    ...
    private enum KnightLocation {
        BANDOS(new ImmutableLocation(2856, 5364, 2), KillcountNPC.GodType.BANDOS), ZAMORAK(new ImmutableLocation(2936, 5351, 2), KillcountNPC.GodType.ZAMORAK), SARADOMIN(new ImmutableLocation(2923, 5259, 0), KillcountNPC.GodType.SARADOMIN), ARMADYL(new ImmutableLocation(2828, 5292, 2), KillcountNPC.GodType.ARMADYL);
        ...
        private KnightLocation(final Location location, final KillcountNPC.GodType god) {
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> player.getDialogueManager().start(new Dialogue(player, npc) {
            ...
            public void buildDialogue() {
                ...
                npc("Dying Knight", "The instance will cost you " + Utils.format(InstanceConstants.INSTANCE_COST) + " coins to start. Any clan members will be able to join free-of-charge after that.");
                final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.content.godwars.npcs.DyingKnight.KnightLocation> gods = new ObjectArrayList<>(KnightLocation.values);
                gods.sort((k1, k2) -> k1.location.getTileDistance(k2.location));
                final com.zenyte.game.content.godwars.npcs.DyingKnight.KnightLocation closestGod = gods.get(0);
                npc("Dying Knight", "Inside the instance, you\'ll mainly find creatures of " + Utils.formatString(closestGod.god.name()) + ", whom you may devour to gain access to the main chamber. I dread to think of the horrors inside there.");
                ...
                npc("Dying Knight", "I won\'t give them back for free though, I\'ll want " + Utils.format(ItemRetrievalService.RetrievalServiceType.GODWARS.getCost()) + " coins for putting my neck out there. I won\'t be picking up anything you leave on the ground on your own will though.").executeAction(() -> {
                    if (!canUsePortal(player)) {
