...
public class SaradominSteppingStone implements Shortcut {
    ...
    public void startSuccess(Player player, WorldObject object) {
        ...
        if (western) {
            final com.zenyte.game.world.entity.Location tile = object.transform(Direction.WEST, 2);
        ...
        WorldTasksManager.scheduleOrExecute(() -> {
            player.faceObject(object);
            final com.zenyte.game.util.Direction direction = object.getX() > player.getX() ? Direction.EAST : Direction.WEST;
            for (int i = 0; i < 4; i++) {
