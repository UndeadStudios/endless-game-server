...
public class JunaEnterDialogue extends Dialogue {
    ...
    public void buildDialogue() {
        ...
        if (player.getEquipment().getId(EquipmentSlot.WEAPON) != -1) {
            ...
            final java.time.LocalDate lastAttempt = LocalDate.parse(player.getAttributeOrDefault(LAST_ATTEMPT_DATE_ATTR, LocalDate.ofYearDay(1969, 1).toString()));
            final long daysLeft = ENTER_CAVE_INTERVAL_DAY - ChronoUnit.DAYS.between(lastAttempt, LocalDate.now());
    ...
    public static boolean hasTimePassed(@NotNull final Player player) {
        final java.time.LocalDate lastAttempt = LocalDate.parse(player.getAttributeOrDefault(LAST_ATTEMPT_DATE_ATTR, LocalDate.ofYearDay(1969, 1).toString()));
        final java.time.LocalDate today = LocalDate.now();
