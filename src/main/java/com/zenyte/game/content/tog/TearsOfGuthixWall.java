...
public enum TearsOfGuthixWall {
    ...
    public static void onServerLaunch(final ServerLaunchEvent event) {
        for (final com.zenyte.game.content.tog.TearsOfGuthixWall weepingWall : WALLS) {
    ...
    public static TearsOfGuthixWall of(@NotNull final Location location) {
        for (final com.zenyte.game.content.tog.TearsOfGuthixWall wall : WALLS) {
    ...
    private void transform(@NotNull final TearsOfGuthix nextTear) {
        if (nextTear == TearsOfGuthix.ABSENCE) {
            ...
            final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.content.tog.TearsOfGuthixWall> possibleWalls = new ObjectArrayList<>(WALLS);
            possibleWalls.removeIf(wall -> wall.currentTears != TearsOfGuthix.ABSENCE);
            final com.zenyte.game.content.tog.TearsOfGuthixWall targetWall = Utils.getRandomCollectionElement(possibleWalls);
