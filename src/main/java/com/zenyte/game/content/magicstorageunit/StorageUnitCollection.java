...
public class StorageUnitCollection {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(StorageUnitCollection.class);
    ...
    public void register(@NotNull final StorageUnitType type, @NotNull final List<? extends StorageUnitElement> unsortedElements) {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.content.magicstorageunit.StorageUnitElement> elements = new ObjectArrayList<>(unsortedElements);
        elements.sort(Comparator.comparing(c -> ItemDefinitions.nameOf(c.getDisplayItem())));
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<java.util.List<com.zenyte.game.content.magicstorageunit.StorageUnitElement>> listOfLists = new ObjectArrayList<List<StorageUnitElement>>();
        ObjectArrayList<StorageUnitElement> currentList = new ObjectArrayList<StorageUnitElement>(MAXIMUM_ELEMENTS_PER_PAGE);
        final int length = elements.size();
        for (int i = 0; i < length; i++) {
            final com.zenyte.game.content.magicstorageunit.StorageUnitElement piece = elements.get(i);
    ...
    private void pregenerateContainer(@NotNull final StorageUnitType type) {
        final java.util.List<java.util.List<com.zenyte.game.content.magicstorageunit.StorageUnitElement>> elements = this.elements.get(type);
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.player.container.Container> listOfContainers = new ObjectArrayList<Container>();
        displayedContainers.put(type, listOfContainers);
        for (final java.util.List<com.zenyte.game.content.magicstorageunit.StorageUnitElement> list : elements) {
            final com.zenyte.game.world.entity.player.container.Container container = new Container(ContainerPolicy.NORMAL, ContainerType.MAGIC_STORAGE, Optional.empty());
            listOfContainers.add(container);
            for (final com.zenyte.game.content.magicstorageunit.StorageUnitElement element : list) {
