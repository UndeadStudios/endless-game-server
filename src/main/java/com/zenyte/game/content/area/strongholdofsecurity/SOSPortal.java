...
public final class SOSPortal implements ObjectAction {
	...
	private static enum Portal {
		...
		private Portal(final int id, final Location destination, final Predicate<Player> predicate) {
			this.id = id;
	...
	public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
		final com.zenyte.game.content.area.strongholdofsecurity.SOSPortal.Portal portal = Utils.findMatching(Portal.VALUES, p -> p.id == object.getId());
		if (portal == null) {
	...
	public Object[] getObjects() {
		...
		for (final com.zenyte.game.content.area.strongholdofsecurity.SOSPortal.Portal portal : Portal.VALUES) {
			list.add(portal.id);
		}
		return list.toArray();
	}
