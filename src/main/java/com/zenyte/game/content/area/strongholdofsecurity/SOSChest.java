...
public final class SOSChest implements ObjectAction {
	...
	private static enum Chest {
						...
						if (player.getBooleanAttribute("sos fourth claimed")) {
							...
							if (colourful || fighting) {
								...
								player(10, "Yes, I\'d like the other pair instead please!").executeAction(() -> {
									...
									final Container[] containers = new Container[] {player.getInventory().getContainer(), player.getBank().getContainer(), player.getEquipment().getContainer()};
		...
		private Chest(final int id, final Predicate<Player> predicate, final Predicate<Player> spacePredicate) {
			this.id = id;
