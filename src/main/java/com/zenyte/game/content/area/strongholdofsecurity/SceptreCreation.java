...
public class SceptreCreation implements PairedItemOnItemPlugin {
	...
	public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
		...
		final com.zenyte.game.item.ItemOnItemAction.ItemPair pair = getMatchingPair(from, to, concatenatedPairs);
		final int maxCharges = DiaryReward.VARROCK_ARMOUR4.eligibleFor(player) ? 13 : DiaryReward.VARROCK_ARMOUR3.eligibleFor(player) ? 11 : DiaryReward.VARROCK_ARMOUR2.eligibleFor(player) ? 9 : DiaryReward.VARROCK_ARMOUR1.eligibleFor(player) ? 7 : 5;
		if (ArrayUtils.contains(RECHARGE_PAIRS, pair)) {
			final int fromId = from.getId();
			final com.zenyte.game.item.Item sceptre = fromId == IMBUED_SKULL_SCEPTRE ? from : to;
			...
			final com.zenyte.game.item.Item piece = sceptre == from ? to : from;
		...
		if (pair == SCEPTRE_HALVES) {
			final com.zenyte.game.item.Item runedSceptre = new Item(RUNED_SCEPTRE);
			...
			final com.zenyte.game.item.Item strangeSkull = new Item(STRANGE_SKULL);
			...
			final com.zenyte.game.item.Item sceptre = new Item(SKULL_SCEPTRE);
