...
public class GuardianMummyDialogue extends Dialogue {
    ...
    public static void uncharge(final Player player) {
        ...
        final com.zenyte.game.world.entity.player.container.Container container = player.getInventory().containsAnyOf(PHARAOH_SCEPTRE.getAllButFirst()) ? player.getInventory().getContainer() : player.getEquipment().getContainer();
        final com.zenyte.game.item.Item sceptre = container.getAny(PHARAOH_SCEPTRE.getAllButFirst());
        if (sceptre == null) {
            return;
        }
    ...
    private void charge(final Player player, final int key) {
        final com.zenyte.game.world.entity.player.container.Container container = player.getInventory().containsItem(PHARAOH_SCEPTRE.first()) ? player.getInventory().getContainer() : player.getEquipment().getContainer();
        final int slot = container.getSlotOf(PHARAOH_SCEPTRE.first());
        if (slot == -1) {
            return;
        }
        final java.util.ArrayList<com.zenyte.game.item.Item> materials = new ArrayList<Item>();
        if (key == GOLD_KEY) {
        ...
        for (PlunderReward plunderReward : PlunderReward.get(tier)) {
            ...
            for (int rewardId : rewardIds) {
                if (player.carryingItem(rewardId)) {
                    ...
                    final com.zenyte.game.item.Item item = inventory.getAny(rewardId);
                    if (item == null) {
                        continue;
                    }
