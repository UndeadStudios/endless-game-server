...
public class PharaohSceptre extends ItemPlugin {
    ...
    private void handleTeleport(final Player player, final Container container, final int slotId, final PharaohSceptreDestination destination) {
        final com.zenyte.game.item.Item sceptre = container.get(slotId);
        if (checkCharges(player, sceptre)) {
            final com.zenyte.game.content.pyramidplunder.item.PharaohSceptre.PharaohSceptreTeleport teleport = new PharaohSceptreTeleport(destination, container, sceptre, slotId);
    ...
    private static class PharaohSceptreTeleport implements Teleport {
        ...
        private void sendMessage(final Player player, final int newSceptreId) {
            final java.lang.String emptyMessage = "The sceptre has run out of charges. Talk to the Guardian Mummy in the Jalsavrah Pyramid to recharge it.";
            final int charges = CHAIN.getItems().indexOf(newSceptreId);
            final java.lang.String remainingMessage = "The sceptre has " + charges + " charge" + (charges == 1 ? "" : "s") + " left.";
            final java.lang.String messageToSend = newSceptreId == CHAIN.first() ? emptyMessage : remainingMessage;
    ...
    private enum PharaohSceptreDestination {
        ...
        private PharaohSceptreDestination(final ImmutableLocation location) {
