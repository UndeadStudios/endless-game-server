...
public enum SpearTrap {
    ...
    public boolean execute(@NotNull final Player player, final int x, final int y) {
        if (within(x, y)) {
            ...
            final com.zenyte.game.world.object.WorldObject trapObject = findTrap(currentTile);
            final int baseDelay = 1;
            final com.zenyte.game.util.Direction movementDirection = findEmptyMovementTile(currentTile);
            final com.zenyte.game.world.entity.Location destination = currentTile.transform(movementDirection);
            player.lock(baseDelay + 3);
            WorldTasksManager.schedule(() -> {
                if (!player.inArea(PyramidPlunderArea.class)) {
                    return;
                }
                final com.zenyte.game.world.object.WorldObject object = World.getObjectWithType(trapObject, 10);
                ...
                final com.zenyte.game.world.entity.masks.ForceMovement forceMovement = new ForceMovement(player.getLocation(), 10, destination, 30, movementDirection.getDirection());
    ...
    private final Direction findEmptyMovementTile(@NotNull final Location trapActivationTile) {
        for (final com.zenyte.game.util.Direction direction : Direction.mainDirections) {
            final com.zenyte.game.world.entity.Location position = trapActivationTile.transform(direction);
    ...
    private final WorldObject findTrap(@NotNull final Location trapHitTile) {
        ...
        final com.zenyte.game.world.object.WorldObject objectUnderneath = World.getObjectWithType(trapHitTile, 10);
        ...
        for (final com.zenyte.game.util.Direction direction : Direction.mainDirections) {
            final com.zenyte.game.world.entity.Location position = trapHitTile.transform(direction);
            final com.zenyte.game.world.object.WorldObject object = World.getObjectWithType(position, 10);
