...
public class PyramidPlunderArea extends Area implements CycleProcessPlugin, PartialMovementPlugin, TeleportMovementPlugin {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PyramidPlunderArea.class);
    ...
    public void process() {
        try {
            final int tick = getAreaTimer().intValue();
    ...
    public void processMovement(Player player, Location destination) {
        ...
        final com.zenyte.game.content.pyramidplunder.PlunderRoom plunderRoom = PlunderRoom.rooms[currentRoom - 1];
    ...
    private final void updateRoomInfo(@NotNull final Player player, @NotNull final Location destination) {
        for (final com.zenyte.game.content.pyramidplunder.PlunderRoom room : PlunderRoom.rooms) {
