...
public class Dawn extends NPC implements CombatScript {
    ...
    public int attack(Entity target) {
        ...
        if (sphereAttack && sphereAttackDelay <= Utils.currentTimeMillis()) {
            ...
            final java.util.ArrayList<com.zenyte.game.world.entity.Location> sphereLocations = new ArrayList<Location>(3);
            int count = 1000;
            while (--count > 0 && sphereLocations.size() != 3) {
                final com.zenyte.game.world.entity.Location location = instance.getRandomPoint();
            ...
            for (int index = 0; index < sphereLocations.size(); index++) {
                final com.zenyte.game.content.boss.grotesqueguardians.EnergySphere sphere = new EnergySphere(index, sphereLocations.get(index), player, this, instance);
        ...
        if (attacks >= 3 && freezeAttackDelay <= Utils.currentTimeMillis()) {
            final com.zenyte.game.world.entity.Location tile = getFreezeLocation(player);
    ...
    public void setAnimation(final Animation animation) {
        ...
        if (animation == null) {
            ...
            final mgi.types.config.AnimationDefinitions defs = AnimationDefinitions.get(animation.getId());
            if (defs != null) {
    ...
    public void sendDeath() {
        ...
        final com.zenyte.game.world.entity.player.Player source = instance.getPlayer();
        final boolean isUnlocked = source.getSlayer().isUnlocked("Gargoyle smasher");
        final java.lang.Object attr = getTemporaryAttributes().get("used_rock_hammer");
        if ((attr != null && (boolean) attr) || (getHitpoints() <= 9 && isUnlocked && (source.getInventory().containsItem(Gargoyle.ROCK_HAMMER) || source.getInventory().containsItem(Gargoyle.GRANITE_HAMMER) || source.getInventory().containsItem(21754, 1)))) {
            if (isUnlocked) {
                final com.zenyte.game.world.entity.pathfinding.events.player.EntityEvent routeEvent = new EntityEvent(source, new DistancedEntityStrategy(this, 1), () -> deathSequence(source), true);
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                case 1: 
                    ...
                    for (final com.zenyte.game.world.entity.Location corner : getCorners(getMiddleLocation())) {
                        World.sendGraphics(FLAME_CORNER_GFX, corner);
                    }
                    for (final com.zenyte.game.world.entity.Location corner : getCorners(dusk.getMiddleLocation())) {
                        World.sendGraphics(FLAME_CORNER_GFX, corner);
                    }
                    if (!instance.getEnergySpheres().isEmpty()) {
                        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.content.boss.grotesqueguardians.EnergySphere> entry : instance.getEnergySpheres().int2ObjectEntrySet()) {
                            final com.zenyte.game.content.boss.grotesqueguardians.EnergySphere sphere = entry.getValue();
    ...
    private Location getFreezeLocation(final Player player) {
        final com.zenyte.game.world.entity.Location base = player.getLocation();
        for (final byte[] offset : OFFSETS) {
            final com.zenyte.game.world.entity.Location location = base.transform(offset[0], offset[1], 0);
    ...
    private Location[] getCorners(final Location middle) {
        final com.zenyte.game.world.entity.Location[] corners = new Location[4];
    ...
    public void setDusk(final Dusk dusk) {
        this.dusk = dusk;
    }

    public boolean isSphereAttack() {
        return this.sphereAttack;
    }

    public void setSphereAttack(final boolean sphereAttack) {
