...
public class Derwen extends MageArenaBossBase {
    ...
    public void onFinish(final Entity source) {
        super.onFinish(source);
        for (final com.zenyte.game.content.boss.magearenaii.Derwen.EnergyBall ball : new ArrayList<>(energyBalls)) {
            ball.finish();
        }
    }
    ...
    int special(final Entity target) {
        final com.zenyte.game.world.entity.Location tile = target.getLocation().transform(Utils.random(3), Utils.random(3), 0);
        setAnimation(getMagicAnimation());
        World.scheduleProjectile(this, tile, energyBall).schedule(() -> {
            if (isDead() || isFinished()) {
                return;
            }
            final com.zenyte.game.content.boss.magearenaii.Derwen.EnergyBall ball = new EnergyBall(this, tile);
            ball.spawn();
