...
public class Skotizo extends NPC implements CombatScript {
    ...
    private final int countOf(@NotNull final Class<? extends NPC> clazz) {
        final org.apache.commons.lang3.mutable.MutableInt mutableInt = new MutableInt();
        for (final com.zenyte.game.world.entity.npc.NPC minion : minions) {
    ...
    public void processNPC() {
        ...
        if (ankouSpawnDelay <= Utils.currentTimeMillis()) {
            ...
            if (ankouCount <= 0) {
                final com.zenyte.game.world.entity.Location tile = instance.getLocation(new Location(Utils.random(SkotizoInstance.southWesternTile.getX(), SkotizoInstance.northEasternTile.getX()), Utils.random(SkotizoInstance.southWesternTile.getY(), SkotizoInstance.northEasternTile.getY()), getPlane()));
            ...
            final com.zenyte.game.world.entity.Location tile = instance.getLocation(new Location(Utils.random(SkotizoInstance.southWesternTile.getX(), SkotizoInstance.northEasternTile.getX() - 10), Utils.random(SkotizoInstance.southWesternTile.getY(), SkotizoInstance.northEasternTile.getY() - 10), getPlane()));
            for (int i = reanimatedDemonCount; i < 3; i++) {
                final com.zenyte.game.world.entity.Location location = tile.transform(Utils.random(10), Utils.random(10), 0);
    ...
    protected void onFinish(Entity source) {
        ...
        WorldTasksManager.schedule(() -> {
            ...
            final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.npc.NPC> minions = new ObjectArrayList<>(this.minions);
            minions.forEach(NPC::sendDeath);
            final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.content.boss.skotizo.npc.AwakenedAltar> altars = new ObjectArrayList<>(instance.getAltars());
