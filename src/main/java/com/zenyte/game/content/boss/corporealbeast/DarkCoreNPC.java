...
public final class DarkCoreNPC extends NPC {
	...
	public void processNPC() {
		...
		while (!targets.isEmpty()) {
			final com.zenyte.game.world.entity.player.Player player = Utils.getRandomCollectionElement(targets);
			...
			final com.zenyte.game.world.Projectile projectile = new Projectile(319, 0, 0, 0, 15, 30 + ((int) Math.ceil(distance / 3.0F) * 30), 0, 0);
	...
	private final boolean containsSiphonableTarget(@NotNull final Collection<Player> collection) {
		for (final com.zenyte.game.world.entity.player.Player t : collection) {
