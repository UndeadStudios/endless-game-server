 ...
 */
public final class VorkathInstance extends DynamicArea implements CycleProcessPlugin, FullMovementPlugin, LogoutPlugin, DeathPlugin, CannonRestrictionPlugin, DropPlugin, LootBroadcastPlugin {
    ...
    private final boolean containsPool(final int x, final int y) {
        final java.util.List<com.zenyte.game.content.boss.vorkath.AcidPool> pools = vorkath.getAcidPools();
        if (pools.isEmpty()) {
            return false;
        }
        for (int i = pools.size() - 1; i >= 0; i--) {
            final com.zenyte.game.content.boss.vorkath.AcidPool pool = pools.get(i);
            if (pool == null) {
                continue;
            }
    ...
    public void setVorkath(final VorkathNPC vorkath) {
