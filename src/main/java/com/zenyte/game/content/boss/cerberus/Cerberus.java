...
public final class Cerberus extends NPC implements CombatScript, Spawnable {
    ...
    private void processSleepingCerberus() {
        ...
        if (location.matches(respawnTile)) {
            ...
            final java.util.List<com.zenyte.game.world.entity.Entity> possibleTargets = getPossibleTargets(targetType);
            if (!possibleTargets.isEmpty()) {
                setAnimation(leaningFromSleep);
                forcedDelay += 2;
                WorldTasksManager.schedule(() -> {
                    final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.player.Player> possibleRemainingTargets = new ObjectArrayList<>(getLair().getPlayers());
                    ...
                    final com.zenyte.game.world.entity.player.Player target = possibleRemainingTargets.get(Utils.random(possibleRemainingTargets.size() - 1));
    ...
    private void checkMarkerCollisions(@NotNull final ImmutableLocation marker) {
        for (final com.zenyte.game.world.entity.player.Player player : getLair().getPlayers()) {
    ...
    private void spawnFlames() {
        final com.zenyte.game.world.entity.Location westernFire = cerberusRoom.getWesternFireLocation();
        final com.zenyte.game.content.boss.cerberus.area.CerberusLair lair = GlobalAreaManager.getArea(CerberusLair.class);
        for (int i = 0; i < FLAMES_COUNT; i++) {
            final com.zenyte.game.world.entity.Location fireLocation = westernFire.transform(Direction.EAST, i);
            final com.zenyte.game.world.object.WorldObject existingFire = World.getObjectWithType(fireLocation, 10);
            ...
            for (final com.zenyte.game.world.entity.player.Player player : lair.getPlayers()) {
    ...
    public int attack(Entity target) {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.npc.combatdefs.AttackType> possibleStyles = new ObjectArrayList<>(possibleOrderedAttackTypes);
        if (!isWithinMeleeDistance(this, target)) {
            possibleStyles.remove(STAB);
        }
        final com.zenyte.game.world.entity.npc.combatdefs.AttackType style = Utils.getRandomCollectionElement(possibleStyles);
    ...
    private void executeSummonedSoulsAttack(@NotNull final Entity target) {
        ...
        final it.unimi.dsi.fastutil.ints.IntArrayList randomizedSoulIds = new IntArrayList(new int[] {NpcId.SUMMONED_SOUL, NpcId.SUMMONED_SOUL_5868, NpcId.SUMMONED_SOUL_5869});
        for (int i = randomizedSoulIds.size() - 1; i >= 0; i--) {
            ...
            final com.zenyte.game.content.boss.cerberus.SummonedSoul soul = new SummonedSoul(soulId, cerberusRoom.getWesternSoulLocation().transform(Direction.EAST, i), this, target, i);
            soul.spawn();
    ...
    private void executeLavaPoolsAttack(@NotNull final Entity target) {
        ...
        final com.zenyte.game.world.entity.Location leftHeadPosition = getProjectileStartPosition(target, CerberusHead.LEFT);
        final com.zenyte.game.world.entity.Location rightHeadPosition = getProjectileStartPosition(target, CerberusHead.RIGHT);
        final com.zenyte.game.world.entity.Location leftHeadTargetedPosition = getProjectileStartPosition(target, CerberusHead.LEFT, SIZE_OFFSET_LAVA_POOL);
        final com.zenyte.game.world.entity.Location rightHeadTargetedPosition = getProjectileStartPosition(target, CerberusHead.RIGHT, SIZE_OFFSET_LAVA_POOL);
        final com.zenyte.game.world.entity.Location leftHeadDestination = translateRandomReachablePosition(leftHeadTargetedPosition);
        ...
        final com.zenyte.game.world.entity.Location rightHeadDestination = translateRandomReachablePosition(rightHeadTargetedPosition);
    ...
    private Location translateRandomReachablePosition(@NotNull final Location location) {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.Location> possibleLocationsList = new ObjectArrayList<Location>((int) Math.pow(((MARKER_RANDOMIZATION_DISTANCE << 1) + 1), 2));
        for (int x = -MARKER_RANDOMIZATION_DISTANCE; x <= MARKER_RANDOMIZATION_DISTANCE; x++) {
            for (int y = -MARKER_RANDOMIZATION_DISTANCE; y <= MARKER_RANDOMIZATION_DISTANCE; y++) {
                final com.zenyte.game.world.entity.Location transformedLocation = location.transform(x, y, 0);
    ...
    private void sendLavaPool(@NotNull final Location headLocation, @NotNull final Location tile) {
        ...
        final com.zenyte.game.world.entity.ImmutableLocation marker = new ImmutableLocation(tile);
        markers.add(marker);
    ...
    private void executeAttack(@NotNull final Entity target, @NotNull final AttackType style, @NotNull final CerberusHead head) {
        ...
        final com.zenyte.game.world.entity.Location headLocation = getProjectileStartPosition(target, head);
        if (style.isRanged()) {
            ...
            final com.zenyte.game.world.entity.Location destinationLocation = new Location(target.getLocation());
