...
public abstract class MageArenaBossBase extends NPC implements CombatScript {
    ...
    MageArenaBossBase(final int id, @NotNull final Player owner, @NotNull final Location tile) {
        ...
        spawned = true;
        final com.zenyte.game.world.entity.masks.Animation death = combatDefinitions.getSpawnDefinitions().getDeathAnimation();
        if (death != null) {
            deathDelay = Math.max(Math.min((int) Math.ceil(death.getDuration() / 600.0F), 10), 1);
        }
    }
    ...
    public int attack(final Entity target) {
        final it.unimi.dsi.fastutil.ints.IntArrayList specialsList = new IntArrayList();
        ...
        if (!special) {
            ...
            if (melee && Utils.random(meleeFrequency()) == 0) {
                ...
                final com.zenyte.game.world.entity.player.action.combat.magic.CombatSpell spell = validSpell();
                final int delay = 0;
                final int clientDelay = 30;
                final com.zenyte.game.world.entity.SoundEffect sound = spell.getHitSound();
                final com.zenyte.game.world.entity.masks.Graphics gfx = hitGraphics();
                final com.zenyte.game.world.entity.masks.Hit hit = magic(target, 43);
        ...
        if (specialType == 0) {
            ...
            final com.zenyte.game.world.entity.player.action.combat.magic.CombatSpell spell = CombatSpell.TELE_BLOCK;
            ...
            setAnimation(getMagicAnimation());
            final com.zenyte.game.world.entity.player.action.combat.magic.CombatSpell spell = CombatSpell.ICE_BARRAGE;
            final com.zenyte.game.world.Projectile projectile = barrageProjectile;
            final com.zenyte.game.world.entity.SoundEffect sound = spell.getHitSound();
            final com.zenyte.game.world.entity.masks.Graphics gfx = spell.getHitGfx();
            final com.zenyte.game.world.entity.masks.Hit hit = magic(target, 43);
            final boolean splash = hit.getDamage() <= ((target instanceof Player && ((Player) target).getPrayerManager().isActive(Prayer.PROTECT_FROM_MAGIC) ? 1 : 0));
