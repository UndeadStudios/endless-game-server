...
public enum Herb {
    ...
    static {
        for (final com.zenyte.game.content.zahur.Herb herb : ALL) {
    ...
    public static Herb get(final int id, final boolean grimyAllowed) {
        for (final java.util.Map.Entry<java.lang.Integer, com.zenyte.game.content.zahur.Herb> entry : HERBS.entrySet()) {
            final java.lang.Integer key = entry.getKey();
            final com.zenyte.game.content.zahur.Herb herb = entry.getValue();
    ...
    private Herb(final Item clean, final Item grimy, final Item unfinishedPotion) {
        this.clean = clean;
        this.grimy = grimy;
