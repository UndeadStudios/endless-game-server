...
public final class ShotputArea extends WarriorsGuildArea implements PartialMovementPlugin {
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (stage == 0) {
					...
					final NPC ref = World.getNPC(player.getLocation().getPositionHash() == SHOT_18LB.getPositionHash() ? 6073 : 6074, 11319);
					if (ref != null) {
					...
					player.unlock();
					player.getSkills().addXp(Skills.STRENGTH, 11.8d * distance);
					stop();
					return;
				}
				stage++;
	...
	private static final int getRandomDistance(final Player player) {
		...
		final int strengthDistance = player.getSkills().getLevel(Skills.STRENGTH) / 10;
