...
public final class SplatterNPC extends PestNPC {
	...
	public void processNPC() {
		...
		if (trackedObject == null) {
			final java.util.List<com.zenyte.game.world.object.WorldObject> ravagableObjects = instance.getRavagableObjects();
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				if (loop == 0) {
					...
					final java.util.List<com.zenyte.game.world.entity.Entity> targets = getNearbyEntities();
					final com.zenyte.game.world.entity.Location location = SplatterNPC.this.getLocation();
					final java.util.List<com.zenyte.game.world.object.WorldObject> objects = instance.getRavagableObjects();
					if (!objects.isEmpty()) {
					...
					for (final com.zenyte.game.world.entity.Entity e : targets) {
	...
	private List<Entity> getNearbyEntities() {
		...
		possibleTargets.addAll(CharacterLoop.find(getLocation(), getSize() + 1, Entity.class, e -> !e.isDead() && !e.isFinished() && e.isRunning() && (!(e instanceof NPC) || ((NPC) e).isAttackable())));
