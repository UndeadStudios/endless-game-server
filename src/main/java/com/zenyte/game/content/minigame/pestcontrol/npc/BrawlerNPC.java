...
public final class BrawlerNPC extends PestNPC {
	...
	public void unclip() {
		...
		for (int x1 = x; x1 < (x + size); x1++) {
			for (int y1 = y; y1 < (y + size); y1++) {
				...
				assert chunk != null;
				World.getRegion(Location.getRegionId(x1, y1), true).removeFlag(z, x1 & 63, y1 & 63, Flags.OCCUPIED_BLOCK_NPC | Flags.OCCUPIED_BLOCK_PLAYER | Flags.OCCUPIED_PROJECTILE_BLOCK_NPC | Flags.OCCUPIED_PROJECTILE_BLOCK_PLAYER);
			}
		}
	}
	...
	public void clip() {
		...
		for (int x1 = x; x1 < (x + size); x1++) {
			for (int y1 = y; y1 < (y + size); y1++) {
				World.getRegion(Location.getRegionId(x1, y1), true).addFlag(z, x1 & 63, y1 & 63, Flags.OCCUPIED_BLOCK_NPC | Flags.OCCUPIED_BLOCK_PLAYER | Flags.OCCUPIED_PROJECTILE_BLOCK_NPC | Flags.OCCUPIED_PROJECTILE_BLOCK_PLAYER);
			}
		}
	}
}
