...
public class Pyromancer extends NPC {
    ...
    private void randomActions() {
        ...
        final com.zenyte.game.content.minigame.wintertodt.Brazier brazier = Wintertodt.Corner.getFromPyromancer(this).getBrazier();
    ...
    public static final class PotionOnPyromancer implements ItemOnNPCAction {
        ...
        public void handleItemOnNPCAction(final Player player, final Item item, final int slot, final NPC npc) {
            final com.zenyte.game.content.minigame.wintertodt.Pyromancer pyromancer = (Pyromancer) npc;
            ...
            bind("Help", new OptionHandler() {
                ...
                public void handle(Player player, NPC npc) {
                    ...
                    for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> entrySet : player.getInventory().getContainer().getItems().int2ObjectEntrySet()) {
                        final com.zenyte.game.item.Item item = entrySet.getValue();
                        if (RejuvenationPotion.Potion.finishedPotions.contains(item.getId())) {
                            firstItem = item;
