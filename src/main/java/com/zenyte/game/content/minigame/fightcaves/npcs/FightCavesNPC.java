...
public abstract class FightCavesNPC extends NPC {
	...
	public enum TzHaarNPC {
		...
		static {
			for (final com.zenyte.game.content.minigame.fightcaves.npcs.FightCavesNPC.TzHaarNPC tzhaar : allValues) {
		...
		public static boolean isAlternative(final NPC npc) {
			final com.zenyte.game.content.minigame.fightcaves.npcs.FightCavesNPC.TzHaarNPC constant = map.get(npc.getId());
		...
		public static void populate(final List<TzHaarNPC> list, final int wave) {
			...
			while (remaining >= 1) {
				TzHaarNPC npc;
				for (final com.zenyte.game.content.minigame.fightcaves.npcs.FightCavesNPC.TzHaarNPC value : values) {
					npc = value;
		...
		private static void replaceAlternatives(final List<TzHaarNPC> list) {
			...
			final com.zenyte.game.content.minigame.fightcaves.npcs.FightCavesNPC.TzHaarNPC first = list.get(0);
			final com.zenyte.game.content.minigame.fightcaves.npcs.FightCavesNPC.TzHaarNPC second = list.get(1);
			if (first == second) {
				list.set(1, map.get(first.id + 1));
			}
		}

		private TzHaarNPC(final Class<? extends FightCavesNPC> clazz, final int id) {
			this.clazz = clazz;
			this.id = id;
		}
