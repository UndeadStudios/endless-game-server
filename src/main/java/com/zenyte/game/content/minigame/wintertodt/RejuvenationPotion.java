...
public class RejuvenationPotion extends ItemPlugin implements ItemOnItemAction {
    ...
    public void handle() {
        setDefault("Drop", (player, item, slotId) -> {
            final java.lang.String itemName = Potion.finishedPotions.contains(item.getId()) ? "potion" : "vial";
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final com.zenyte.game.item.Item potion = from.getId() == Potion.UNFINISHED.getItemId() ? from : to;
        final com.zenyte.game.item.Item herb = potion == from ? to : from;
    ...
    enum Potion {
        ...
        static {
            for (final com.zenyte.game.content.minigame.wintertodt.RejuvenationPotion.Potion potion : values) {
        ...
        private Potion(final int itemId) {
            this.itemId = itemId;
        }
    ...
    private static final class MakePotionAction extends Action {
        ...
        public MakePotionAction(final int fromSlot, final int toSlot) {
