...
public class Inferno extends DynamicArea implements CycleProcessPlugin, LogoutPlugin, DeathPlugin, CannonRestrictionPlugin, TeleportPlugin, DropPlugin {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(Inferno.class);
    ...
    private WorldObject[] fallingRocks;
    private WorldObject[] wallPatches;
    private Location nibblerSpawnLocation;
    private Location[] tripleJadSpawnLocations;
    private Location[] spawnLocations;
    private Location[] revivalLocations;
    private Location[] dragBlockTiles;
    private List<RockySupport> rockySupports;
    private final List<InfernoNPC> npcs = new ArrayList<>();
    private Entity nibblerTarget;
    private Location middle;
    private Location singleJadSpawnLocation;
    ...
    public Inferno(final Player player, final AllocatedArea area, final boolean practiceMode) {
        ...
        final com.zenyte.game.content.skills.slayer.Assignment task = player.getSlayer().getAssignment();
    ...
    public void constructed() {
        ...
        if (wave.getWave() < InfernoWave.WAVE_67.getWave()) {
            for (final com.zenyte.game.content.minigame.inferno.model.RockySupportLocation location : RockySupportLocation.values) {
                final int hitpoints = player.getNumericAttribute("inferno_rocky_support_" + location).intValue();
                if (hitpoints >= 0) {
                    final com.zenyte.game.content.minigame.inferno.model.RockySupport support = new RockySupport(location, this);
                    rockySupports.add((RockySupport) support.spawn());
                    support.setHitpoints(hitpoints == 0 ? support.getMaxHitpoints() : hitpoints);
                }
            }
        }
        for (final com.zenyte.game.world.entity.Location tile : dragBlockTiles) {
    ...
    public static final void onLogin(final LoginEvent event) {
        ...
        final java.lang.Object attribute = player.getAttributes().get("inferno_progress");
        if (attribute == null) {
            return;
        }
        player.lock();
        try {
            ...
            final com.zenyte.game.content.minigame.inferno.instance.Inferno inferno = new Inferno(player, area, practiceMode);
    ...
    private final int calculateHealthOfAllSlaughteredMonsters() {
        int totalHealth = 0;
        try {
            for (final com.zenyte.game.content.minigame.inferno.model.InfernoWave wave : InfernoWave.getValues()) {
                if (wave.ordinal() >= this.wave.ordinal()) {
                    break;
                }
                for (final com.zenyte.game.content.minigame.inferno.model.WaveEntry entry : wave.getEntries()) {
    ...
    public void leave(final boolean completed) {
        ...
        final com.zenyte.game.item.Item tokkul = new Item(ItemId.TOKKUL, amount);
        ...
        if (completed) {
            ...
            for (final com.zenyte.game.content.minigame.inferno.npc.InfernoNPC npc : npcs) {
    ...
    private void removeSupports() {
        for (final com.zenyte.game.content.minigame.inferno.model.RockySupport support : rockySupports) {
    ...
    private void executeWave(final InfernoWave wave) {
        ...
        setNibblerTarget();
        final com.zenyte.game.content.minigame.inferno.model.WaveEntry[] entries = wave.getEntries();
        final it.unimi.dsi.fastutil.ints.Int2IntOpenHashMap tiles = new Int2IntOpenHashMap(6);
        for (int index = 0; index < entries.length; index++) {
            final com.zenyte.game.content.minigame.inferno.model.WaveEntry entry = entries[index];
            final int amount = entry.getCount();
            for (int count = 0; count < amount; count++) {
                final com.zenyte.game.content.minigame.inferno.model.WaveNPC n = entry.getNpc();
                try {
                    Location location;
                    ...
                    final com.zenyte.game.content.minigame.inferno.npc.InfernoNPC npc = entry.getNpc().getClazz().getDeclaredConstructor(Location.class, Inferno.class).newInstance(location, this);
                    npc.spawn();
    ...
    public void setNibblerTarget() {
        final java.util.ArrayList<com.zenyte.game.content.minigame.inferno.model.RockySupport> supports = new ArrayList<RockySupport>(3);
    ...
    public <T> List<T> getNPCs(final Class<T> npc) {
        ...
        for (final com.zenyte.game.content.minigame.inferno.npc.InfernoNPC n : npcs) {
            if (npc.isInstance(n)) {
                list.add((T) n);
            }
        }
    ...
    public void check(final InfernoNPC npc, final Entity source) {
        ...
        if (npcs.remove(npc) && npcs.isEmpty()) {
            ...
            final java.util.List<com.zenyte.game.content.minigame.inferno.npc.impl.JalZek> zeks = getNPCs(JalZek.class);
            if (zeks.isEmpty() || wave.equals(InfernoWave.WAVE_69)) {
                return;
            }
            for (final com.zenyte.game.content.minigame.inferno.npc.impl.JalZek zek : zeks) {
