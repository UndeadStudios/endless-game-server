...
public class JalMejJak extends InfernoNPC {
    ...
    public int attack(final Entity target) {
        ...
        if (target.equals(zuk)) {
            ...
            final com.zenyte.game.world.entity.Location targetLocation = new Location(target.getLocation());
            final java.util.ArrayList<com.zenyte.game.world.entity.Location> tiles = new ArrayList<Location>(3);
            int count = 1000;
            while (--count > 0) {
                final com.zenyte.game.world.entity.Location tile = getRandomPoint();
                if (!tiles.contains(tile)) {
                    final com.zenyte.game.world.entity.Location location = targetLocation.getDistance(getLocation()) <= 6 && Utils.random(5) == 0 && !tiles.contains(targetLocation) ? targetLocation : tile;
                    tiles.add(location);
                }
                if (tiles.size() == 3) {
                    break;
