...
public class JalImKot extends InfernoNPC {
    ...
    public void processNPC() {
        if (!isDead() && Utils.currentTimeMillis() - getAttackingDelay() >= TimeUnit.TICKS.toMillis(10) && !hasWalkSteps()) {
            ...
            setAttackingDelay(Utils.currentTimeMillis() + TimeUnit.TICKS.toMillis(25));
            final com.zenyte.game.world.entity.Location burrowLocation = getBurrowLocation();
    ...
    private Location getBurrowLocation() {
        final com.zenyte.game.world.entity.player.Player player = inferno.getPlayer();
        final java.util.Optional<com.zenyte.game.world.entity.Location> optionalLocation = Utils.findEmptySquare(player.getLocation(), player.getSize() + getSize(), getSize(), Optional.of(l -> CombatUtilities.isWithinMeleeDistance(l, getSize(), player)));
