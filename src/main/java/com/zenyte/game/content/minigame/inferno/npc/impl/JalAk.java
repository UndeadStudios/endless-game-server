...
public class JalAk extends InfernoNPC {
    ...
    private void switchStyle(@NotNull final Player player) {
        final com.zenyte.game.content.skills.prayer.PrayerManager prayer = player.getPrayerManager();
    ...
    public void onFinish(Entity source) {
        ...
        final int direction = getRoundedDirection(Utils.getFaceDirection(source.getX() - middle.getX(), source.getY() - middle.getY()), 1024);
        ...
        try {
            for (int index = 0; index < bloblings.length; index++) {
                ...
                final com.zenyte.game.content.minigame.inferno.npc.InfernoNPC blobling = bloblings[index].getClazz().getDeclaredConstructor(Location.class, Inferno.class).newInstance(middle.transform(offset[0], offset[1], 0), inferno);
                blobling.freeze(1);
                blobling.spawn();
