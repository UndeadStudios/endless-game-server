...
public final class ShifterNPC extends PestNPC {
	...
	private void teleport() {
		...
		int attempts = 50;
		while (--attempts > 0) {
			final com.zenyte.game.world.entity.Location tile = instance.getLocation(platform ? PestControlUtilities.getRandomPlatformLocation() : PestControlUtilities.getRandomShifterLocation());
			if (tile.withinDistance(getLocation(), 3)) {
				continue;
			}
			if (World.isTileFree(tile, getSize())) {
				final com.zenyte.game.world.entity.Location currentTile = new Location(getX(), getY() - 1, getPlane());
	...
	private final class ShifterCombat extends NPCCombat {
		...
		public boolean checkAll() {
			...
			if (!target.isMultiArea()) {
				...
				if (attackedBy != null && attackedBy != npc && !attackedBy.isDead() && !attackedBy.isFinished() && target.getAttackedByDelay() > Utils.currentTimeMillis()) {
					return false;
				}
			}
