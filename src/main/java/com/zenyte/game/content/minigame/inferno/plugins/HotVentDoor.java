		...
		if (!proven) {
			player.getDialogueManager().start(new Dialogue(player, TzHaar.TZHAAR_KET) {
				...
				public void buildDialogue() {
					final java.util.List<com.zenyte.game.world.entity.npc.NPC> npcs = CharacterLoop.find(player.getLocation(), 10, NPC.class, n -> n.getId() == TzHaar.TZHAAR_KET);
					final com.zenyte.game.world.entity.npc.NPC npc = npcs.get(0);
					if (npc != null) {
					...
					if (player.getInventory().containsItem(TzHaar.FIRE_CAPE, 1) || player.getInventory().containsItem(MaxCape.FIRE.getCape(), 1)) {
						final com.zenyte.game.item.Item item = player.getInventory().containsItem(TzHaar.FIRE_CAPE_ITEM) ? TzHaar.FIRE_CAPE_ITEM : new Item(MaxCape.FIRE.getCape());
