...
public class WintertodtPrisonArea extends WintertodtCampArea implements CannonRestrictionPlugin, LayableTrapRestrictionPlugin, DropPlugin, RandomEventRestrictionPlugin, LogoutPlugin, LoginPlugin, CycleProcessPlugin {
    ...
    private static void startAOEAttack(final Player player) {
        final com.zenyte.game.world.entity.Location aoeMiddleLocation = getAOEAttackMiddleLocation(player);
        if (aoeMiddleLocation == null) {
            return;
        }
        final com.zenyte.game.world.entity.Location middleLocation = new Location(aoeMiddleLocation.getX(), aoeMiddleLocation.getY());
        final java.util.HashSet<com.zenyte.game.world.entity.Location> subLocations = getSubLocations(middleLocation);
        for (Location subLocation : subLocations) {
            aoeAttackLocations.add(subLocation.getPositionHash());
        }
        final java.util.HashSet<com.zenyte.game.world.entity.Location> cornerLocations = new HashSet<Location>(Direction.interCardinalDirections.length);
        ...
        WorldTasksManager.schedule(() -> {
            ...
            for (final com.zenyte.game.world.entity.player.Player p : CharacterLoop.find(middleLocation, 1, Player.class, p -> !Wintertodt.insideSafeArea(p))) {
    ...
    private static HashSet<Location> getSubLocations(final Location middleLocation) {
        final java.util.HashSet<com.zenyte.game.world.entity.Location> subLocations = new HashSet<Location>(9);
    ...
    public void process() {
        ...
        if (!Wintertodt.atFullEnergy()) {
            for (Wintertodt.Corner corner : Wintertodt.Corner.VALUES) {
                final com.zenyte.game.content.minigame.wintertodt.Brazier brazier = corner.getBrazier();
                final com.zenyte.game.content.minigame.wintertodt.Pyromancer pyromancer = corner.getPyromancer();
    ...
    private void spawnBrazierProjectile(final Brazier brazier, final Pyromancer pyromancer) {
        ...
        final com.zenyte.game.util.Direction pointingDirection = pyromancer.getSpawnDirection();
        WorldTasksManager.schedule(() -> {
            final com.zenyte.game.world.entity.Location fromlocation = new Location(brazier.getX(), brazier.getY()).transform(1, 1, 0).transform(pointingDirection, -1);
            final com.zenyte.game.world.entity.Location toLocation = fromlocation.transform(pointingDirection, 9);
            final com.zenyte.game.world.entity.npc.NPC npc = World.spawnNPC(new NPC(7373, fromlocation, pointingDirection, 0));
