...
public class FightCaves extends DynamicArea implements LogoutPlugin, DeathPlugin, CycleProcessPlugin, CannonRestrictionPlugin {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(FightCaves.class);
    ...
    static void login(final Player player) {
        final java.lang.Object attribute = player.getAttributes().get("Fight caves progress");
        if (attribute == null) {
            return;
        }
        player.lock();
    ...
    public static void start(final Player player) {
        try {
            final com.zenyte.game.world.region.dynamicregion.AllocatedArea area = MapBuilder.findEmptyChunk(WIDTH, HEIGHT);
            final com.zenyte.game.content.minigame.fightcaves.FightCaves caves = new FightCaves(area, player);
    ...
    public void skip(final int waveId) {
        ...
        wave = waveId;
        for (final com.zenyte.game.world.entity.npc.NPC npc : monsters) {
            npc.finish();
        }
        monsters.clear();
        informWave();
    ...
    private void spawnNextWave() {
        ...
        for (final com.zenyte.game.content.minigame.fightcaves.npcs.FightCavesNPC.TzHaarNPC npc : npcs) {
            spawnNPC(npc);
        }
    }

    public void spawnNPC(final FightCavesNPC.TzHaarNPC npc) {
        final com.zenyte.game.world.entity.npc.NPC mob = npc.instantiate(getLocation(FightCaveSpawn.getNextLocation(rotation)), this).spawn();
    ...
    public void constructed() {
        ...
        final com.zenyte.game.world.entity.Location tile = getLocation(2400, 5088, 0);
        final java.lang.Number attribute = player.getNumericAttribute("Fight caves progress");
        if (attribute.intValue() != 0) {
            final java.lang.Number duration = player.getNumericAttribute("Fight caves duration");
        ...
        WorldTasksManager.schedule(() -> {
            ...
            for (final com.zenyte.game.world.entity.Location location : path) {
                final com.zenyte.game.world.entity.Location alteredLocation = getLocation(location);
    ...
    public void leave() {
        ...
        if (this.wave == 63) {
            for (final com.zenyte.game.world.entity.npc.NPC monster : monsters) {
        ...
        if (roundsCompleted > 1) {
            ...
            final com.zenyte.game.item.Item tokkul = new Item(6529, amount);
            ...
            if (wave >= 63) {
                final com.zenyte.game.content.follower.impl.BossPet pet = BossPet.TZREK_JAD;
