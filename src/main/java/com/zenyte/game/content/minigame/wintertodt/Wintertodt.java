...
public class Wintertodt {
    ...
    static {
        for (final java.lang.Integer potion : RejuvenationPotion.Potion.items) {
    ...
    static void refreshOverlayForAllPlayers() {
        for (final com.zenyte.game.world.entity.player.Player player : GlobalAreaManager.get("Wintertodt").getPlayers()) {
            refreshOverlay(player);
        }
    }
    ...
    private static void finishRound() {
        for (final com.zenyte.game.world.entity.player.Player player : GlobalAreaManager.get("Wintertodt").getPlayers()) {
            ...
            if (points < 500) {
                ...
                final com.zenyte.game.item.Item crate = generateSupplyCrate(player, points);
    ...
    static Item generateSupplyCrate(final Player player, final int points) {
        ...
        final com.zenyte.game.item.Item crate = new Item(RewardCrate.SUPPLY_CRATE, 1);
    ...
    static int getNumberOfWarmClothingWorn(final Player player) {
        int amount = 0;
        for (final com.zenyte.game.item.Item item : player.getEquipment().getContainer().getItems().values()) {
    ...
    static int getNumberOfBraziersLit() {
        int amount = 0;
        for (final com.zenyte.game.content.minigame.wintertodt.Wintertodt.Corner corner : Corner.VALUES) {
    ...
    enum Corner {
        ...
        static {
            for (Corner corner : VALUES) {
                ...
                final com.zenyte.game.world.entity.Location middle = new Location(corner.brazier.getX(), corner.brazier.getY()).transform(Direction.NORTH_EAST, 1);
        ...
        private Corner(final Pyromancer pyromancer, final Brazier brazier) {
