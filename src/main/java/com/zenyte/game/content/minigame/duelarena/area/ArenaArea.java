 ...
 */
public abstract class ArenaArea extends DuelArenaArea implements DeathPlugin, PlayerCombatPlugin, TradePlugin, TeleportPlugin, EntityAttackPlugin, EquipmentPlugin, PrayerPlugin, DropPlugin, EdiblePlugin, DrinkablePlugin, LogoutPlugin, LoginPlugin, RandomEventRestrictionPlugin {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(ArenaArea.class);
    ...
    public void leave(Player player, boolean logout) {
        ...
        if (!duel.isCompleted()) {
            final com.zenyte.game.world.entity.player.Player opponent = Objects.requireNonNull(duel.getOpponent());
    ...
    public boolean attack(final Player player, final Entity entity) {
        ...
        final com.zenyte.game.world.entity.player.Player target = (Player) entity;
        final com.zenyte.game.world.entity.player.Player opponent = duel.getOpponent();
        if (opponent == null) {
            return false;
        }
    ...
    public void login(Player player) {
        for (final com.zenyte.game.content.minigame.duelarena.area.ArenaArea area : Utils.concatenate(Duel.NORMAL_ARENA, Duel.OBSTACLES_ARENA, Duel.NO_MOVEMENT_ARENA)) {
    ...
    public boolean equip(final Player player, final Item item, final int slot) {
        ...
        if (duel.hasRule(DuelSetting.NO_FUN_WEAPONS)) {
            for (final java.lang.Integer id : Duel.FUN_WEAPONS) {
                if (id == item.getId()) {
                    player.sendMessage("You can\'t equip that during this duel.");
                    return false;
                }
