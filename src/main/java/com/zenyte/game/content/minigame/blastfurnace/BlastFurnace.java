...
public class BlastFurnace {
    ...
    public void processVarbits() {
        for (final BlastFurnaceOre ore : BlastFurnaceOre.VALUES) {
            ...
            final com.zenyte.game.content.minigame.blastfurnace.BlastFurnaceVarbit varbit = BlastFurnaceVarbit.oreVarbits.get(ore);
        ...
        for (final SmeltableBar bar : SmeltableBar.bfValues) {
            ...
            final com.zenyte.game.content.minigame.blastfurnace.BlastFurnaceVarbit varbit = BlastFurnaceVarbit.barVarbits.get(bar);
    ...
    public void processBars() {
        ...
        for (final SmeltableBar bar : SmeltableBar.bfValues) {
            ...
            for (final Item oreItem : bar.getMaterials()) {
                final com.zenyte.game.content.minigame.blastfurnace.BlastFurnaceOre ore = BlastFurnaceOre.getOre(oreItem.getId());
            ...
            if (bar.getMaterials().length > 1) {
                final com.zenyte.game.item.Item primary = bar.getMaterials()[0];
                final com.zenyte.game.item.Item secondary = bar.getMaterials()[1];
                final com.zenyte.game.content.minigame.blastfurnace.BlastFurnaceOre primaryE = BlastFurnaceOre.getOre(primary.getId());
                final com.zenyte.game.content.minigame.blastfurnace.BlastFurnaceOre secondaryE = BlastFurnaceOre.getOre(secondary.getId());
                ...
                final com.zenyte.game.item.Item ore = bar.getMaterials()[0];
                final com.zenyte.game.content.minigame.blastfurnace.BlastFurnaceOre oreE = BlastFurnaceOre.getOre(ore.getId());
                int amount = getOre(oreE);
                amount = (getBar(bar) + amount) > 28 ? 28 - getBar(bar) : amount;
                addBars(bar, amount);
                subOre(oreE, amount);
                final com.zenyte.game.item.Item gloves = player.getEquipment().getItem(EquipmentSlot.HANDS);
                Smelting.handleDaily(player, bar, amount);
    ...
    public static final void onInit(final InitializationEvent event) {
        ...
        final com.zenyte.game.content.minigame.blastfurnace.BlastFurnace parserData = event.getSavedPlayer().getBlastFurnace();
        if (parserData == null) {
            return;
        }
    ...
    public int getOresOnBelt() {
        return this.oresOnBelt;
    }

    public void setOresOnBelt(final int oresOnBelt) {
        this.oresOnBelt = oresOnBelt;
    }

    public boolean isProcessBarsFromDialogue() {
        return this.processBarsFromDialogue;
    }

    public void setProcessBarsFromDialogue(final boolean processBarsFromDialogue) {
        this.processBarsFromDialogue = processBarsFromDialogue;
    }

    public boolean isEarlyCool() {
        return this.earlyCool;
    }

    public void setEarlyCool(final boolean earlyCool) {
