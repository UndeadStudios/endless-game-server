...
public final class PestControlInstance extends DynamicArea implements DeathPlugin, CycleProcessPlugin, LogoutPlugin, CannonRestrictionPlugin {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(PestControlInstance.class);
    ...
    public void addActivity(final Player player, final int amount) {
        final com.zenyte.game.content.minigame.pestcontrol.PestControlStatistic statistic = mappedPlayers.get(player);
        if (statistic == null) {
            return;
        }
    ...
    public void constructed() {
        ...
        for (final com.zenyte.game.content.minigame.pestcontrol.PestPortal portal : PestPortal.VALUES) {
            portals.put(portal, new PestPortalNPC(this, portal));
        }
        for (final java.util.Map.Entry<com.zenyte.game.world.entity.player.Player, com.zenyte.game.content.minigame.pestcontrol.PestControlStatistic> entry : mappedPlayers.entrySet()) {
            final com.zenyte.game.world.entity.player.Player player = entry.getKey();
            if (player.isNulled()) {
                continue;
            }
    ...
    public final List<WorldObject> getRavagableObjects() {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.object.WorldObject> ravagableObjects = new ObjectArrayList<WorldObject>();
        for (final com.zenyte.game.world.entity.Location tile : barricadeTiles) {
        ...
        for (final com.zenyte.game.world.entity.Location tile : gateTiles) {
            final com.zenyte.game.world.object.WorldObject object = World.getObjectWithType(tile, 0);
            if (object != null) {
                if (ArrayUtils.contains(PestControlUtilities.VALID_RAVAGABLE_OBJECTS, object.getId())) {
                    ravagableObjects.add(object);
                }
    ...
    public void finish(final boolean successful) {
        ...
        WorldTasksManager.schedule(new WorldTask() {
            ...
            public void run() {
                ...
                for (final com.zenyte.game.world.entity.npc.NPC npc : monsters.toArray(new NPC[0])) {
                    npc.finish();
                }
                stop();
            }
    ...
    private void exportPlayer(final Player player, final boolean successful) {
        ...
        WorldTasksManager.schedule(() -> {
            if (successful) {
                ...
                final com.zenyte.game.world.entity.player.MemberRank rank = player.getMemberRank();
    ...
    public void process() {
        ...
        if (!flagMap.isEmpty()) {
            for (final it.unimi.dsi.fastutil.objects.Object2ByteMap.Entry<com.zenyte.game.world.entity.player.Player> entry : flagMap.object2ByteEntrySet()) {
                final com.zenyte.game.world.entity.player.Player player = entry.getKey();
                if (player.isNulled()) {
                    continue;
                }
    ...
    private Location getRandomSpawnTile(final int npcId, final PestPortal portal) {
        ...
        final com.zenyte.game.world.entity.Location baseTile = portal.getNpcSpawnTile();
        int attempts = 50;
        while (--attempts > 0) {
    ...
    private void spawn() {
        ...
        final com.zenyte.game.content.minigame.pestcontrol.PestNPCTable type = PestNPCTable.VALUES[Utils.random(PestNPCTable.VALUES.length - 1)];
        ...
        final com.zenyte.game.content.minigame.pestcontrol.PestPortal portal = availablePortals.get(Utils.random(availablePortals.size() - 1));
        final com.zenyte.game.world.entity.Location tile = getRandomSpawnTile(id, portal);
        if (tile == null) {
            return;
        }
        final java.lang.Class<? extends com.zenyte.game.content.minigame.pestcontrol.PestNPC> npcClass = type.getNpcClass();
        try {
    ...
    private void dropShield() {
        ...
        final com.zenyte.game.content.minigame.pestcontrol.PestPortal portal = sequence.getPortals()[index];
        flag(SHIELDS_FLAG);
        final com.zenyte.game.content.minigame.pestcontrol.npc.PestPortalNPC portalNPC = portals.get(portal);
