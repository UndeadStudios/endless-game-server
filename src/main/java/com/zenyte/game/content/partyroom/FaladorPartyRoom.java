...
public class FaladorPartyRoom extends FaladorArea implements CycleProcessPlugin, CannonRestrictionPlugin, LayableObjectPlugin, RandomEventRestrictionPlugin {
    ...
    final Container getPrivateContainer(@NotNull final Player player) {
        final java.util.Optional<com.zenyte.game.world.entity.player.container.Container> optionalContainer = findPrivateContainer(player);
        if (!optionalContainer.isPresent()) {
            final com.zenyte.game.world.entity.player.container.Container container = new Container(ContainerPolicy.ALWAYS_STACK, ContainerType.FALADOR_PARTY_CHEST_PRIVATE_DEPOSIT, Optional.of(player));
    ...
    @NotNull
    private final Optional<Container> findPrivateContainer(@NotNull final Player player) {
        final java.lang.Object attribute = player.getTemporaryAttributes().get(PRIVATE_ATTRIBUTE_KEY);
    ...
    void startKnightsDance() {
        ...
        dancing = true;
        final com.zenyte.game.content.partyroom.PartyRoomDancingKnight[] knights = new PartyRoomDancingKnight[6];
        for (int i = 0; i < 6; i++) {
            final com.zenyte.game.content.partyroom.PartyRoomDancingKnight knight = new PartyRoomDancingKnight(i);
            knight.spawn();
            knights[i] = knight;
        }
        final com.zenyte.game.content.partyroom.PartyRoomDancingKnight singingKnight = knights[2];
        for (int i = 0; i < song.length; i++) {
            final com.zenyte.game.world.entity.ForceTalk lyrics = song[i];
            WorldTasksManager.schedule(() -> singingKnight.setForceTalk(lyrics), 4 + (i * 4));
            if (i == song.length - 1) {
                WorldTasksManager.schedule(() -> {
                    for (final com.zenyte.game.content.partyroom.PartyRoomDancingKnight knight : knights) {
                        knight.finish();
                    }
                    dancing = false;
                }, 8 + (i * 4));
    ...
    public FaladorPartyRoom() {
        super();
        final java.util.LinkedList<com.zenyte.game.world.entity.ImmutableLocation> roomTiles = new LinkedList<ImmutableLocation>();
        final com.zenyte.game.world.region.RSPolygon[] polygons = polygons();
        assert polygons.length == 1;
        final com.zenyte.game.world.region.RSPolygon polygon = polygons[0];
        final java.awt.geom.Rectangle2D rectangle = polygon.getPolygon().getBounds2D();
    ...
    public void roll(@NotNull final PartyBalloon balloon) {
        ...
        final java.util.Set<java.lang.String> usernames = balloon.getUsernames();
        if (usernames.isEmpty()) {
            return;
        }
        final java.lang.String randomUsername = Utils.getRandomCollectionElement(usernames);
    ...
    private final Optional<Item> selectAndRemoveRandomItem() {
        ...
        final com.zenyte.game.item.Item item = new Item(Utils.getRandomCollectionElement(container.getItems().values()));
        ...
        final com.zenyte.game.world.entity.player.container.ContainerResult result = container.remove(item);
        container.shift();
        final com.zenyte.game.item.Item removedItem = new Item(item.getId(), result.getSucceededAmount());
    ...
    void rebuildAnnouncementsList() {
        ...
        final java.lang.String timeRemaining = countdown > 75 ? (minutes + " minute" + (minutes == 1 ? "" : "s")) : (seconds + " second" + (seconds == 1 ? "" : "s"));
        long totalValueOfAllItemsCombined = 0;
        for (final com.zenyte.game.item.Item item : container.getItems().values()) {
            if (item == null) {
                continue;
        ...
        final java.util.OptionalInt optionalFrequency = variables.getAnnouncementSpeed(totalValueOfAllItemsCombined);
    ...
    public void process() {
        ...
        for (final com.zenyte.game.world.entity.ImmutableLocation tile : roomTiles) {
        ...
        for (int i = 0; i < amount; i++) {
            ...
            final com.zenyte.game.world.entity.ImmutableLocation tile = reusableTilesList.get(Utils.random(reusableTilesList.size() - 1));
            final com.zenyte.game.content.partyroom.PartyBalloon balloon = new PartyBalloon(Utils.random(115, 122), tile.getX(), tile.getY(), tile.getPlane());
            balloon.alive();
    ...
    public boolean isPartyPeteCountdown() {
        return this.partyPeteCountdown;
    }

    public void setPartyPeteCountdown(final boolean partyPeteCountdown) {
