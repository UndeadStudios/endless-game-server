...
public class Chompy extends NPC {
    ...
    public void drop(final Location location) {
        final com.zenyte.game.world.entity.npc.NPC deadChompy = new NPC(DeadChompy.DEAD_CHOMPY_ID, location, true);
    ...
    private boolean hasArrow(final Player player) {
        final com.zenyte.game.item.Item ammunition = player.getEquipment().getItem(EquipmentSlot.AMMUNITION);
        if (ammunition == null) {
            return false;
        }
    ...
    public void finish() {
        ...
        World.getPlayer(playerName).ifPresent(p -> {
            final java.lang.Object arrow = p.getTemporaryAttributes().get("last hint arrow");
            if (arrow instanceof HintArrow) {
                final com.zenyte.game.HintArrow hint = (HintArrow) arrow;
