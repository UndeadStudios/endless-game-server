...
public class BloatedToad extends ItemPlugin {
    ...
    public void handle() {
        bind("Drop", (player, item, slotId) -> {
            ...
            final com.zenyte.game.content.chompy.plugins.BloatedToadNPC bloatedToad = new BloatedToadNPC(new Location(player.getLocation()));
            bloatedToad.spawn();
            if (Utils.random(2) != 0) {
    ...
    private static void spawnChompy(final Player player, final Location dropLocation, final BloatedToadNPC toad) {
        ...
        final com.zenyte.game.world.entity.Location chompyBaseSpawn = dropLocation.random(10);
        ...
        for (int i = 0; i < amount; i++) {
            Utils.findEmptySquare(chompyBaseSpawn, 10, size, Optional.empty()).ifPresent(spawn -> {
                final com.zenyte.game.content.chompy.Chompy chompy = new Chompy(player.getName(), Chompy.CHOMPY_ID, spawn, true, toad);
                chompy.spawn();
