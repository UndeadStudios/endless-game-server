...
public class BowItemAction extends ItemPlugin {
    ...
    private void check(final Player player) {
        ...
        final com.zenyte.game.content.chompy.ChompyBirdHat hat = ChompyBirdHat.getBest(kills);
        player.sendMessage("You\'ve scratched up a total of " + kills + " chompy bird kills so far!");
        if (hat != null) {
            final java.lang.String title = hat.getTitle();
            final java.lang.String article = (Utils.startWithVowel(title) ? "an" : "a");
