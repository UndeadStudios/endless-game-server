...
public enum FireWall {
    ...
    public static final FireWall select(@NotNull final Position staticTargetPosition) {
        final com.zenyte.game.world.entity.Location tile = staticTargetPosition.getPosition();
        final int tileX = tile.getX();
        final int tileY = tile.getY();
        for (final com.zenyte.game.content.kebos.alchemicalhydra.model.FireWall frequentWall : frequentWalls) {
            for (final com.zenyte.game.content.kebos.alchemicalhydra.model.FireWall.FireBlockDirection polygonBlock : frequentWall.blocks) {
    ...
    public RSPolygon[] transform(final AlchemicalHydraInstance instance) {
        final com.zenyte.game.world.region.RSPolygon[] transformedWalls = new RSPolygon[blocks.length];
        for (int index = 0; index < blocks.length; index++) {
            final com.zenyte.game.world.region.RSPolygon polygon = blocks[index].polygon;
    ...
    public final List<FireWallBlock> getInnerPoints(@NotNull final AlchemicalHydraInstance instance) {
        final com.zenyte.game.world.region.RSPolygon[] transformedPolygons = transform(instance);
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.content.kebos.alchemicalhydra.model.FireWallBlock> list = new ObjectArrayList<FireWallBlock>(2);
        for (int i = 0; i < 2; i++) {
            final com.zenyte.game.world.region.RSPolygon polygon = transformedPolygons[i];
            final it.unimi.dsi.fastutil.objects.ObjectOpenHashSet<com.zenyte.game.world.entity.Location> tileSet = new ObjectOpenHashSet<Location>(25);
            final com.zenyte.game.util.Direction dir = Direction.valueOf(blocks[i].name());
            final com.zenyte.game.content.kebos.alchemicalhydra.model.FireWallBlock block = new FireWallBlock(tileSet, dir, movingFireLocation);
            list.add(block);
            final Polygon poly = polygon.getPolygon();
            final java.awt.geom.Rectangle2D rectangle = poly.getBounds2D();
    ...
    private enum FireBlockDirection {
        ...
        private FireBlockDirection(final RSPolygon polygon) {
