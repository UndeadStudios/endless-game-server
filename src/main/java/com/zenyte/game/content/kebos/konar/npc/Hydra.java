...
public class Hydra extends NPC implements CombatScript, Spawnable {
    ...
    public int attack(Entity target) {
        if (poisonAttack) {
            final com.zenyte.game.world.entity.Location base = new Location(target.getLocation());
            ...
            final com.zenyte.game.world.entity.Location hydraCenter = getMiddleLocation();
            setAnimation(poisonAttackAnim);
            for (final com.zenyte.game.world.entity.Location tile : poisonTiles) {
                World.sendProjectile(this, tile, poisonAttackProj);
                final int direction = getRoundedDirection(Utils.getFaceDirection(tile.getX() - hydraCenter.getX(), tile.getY() - hydraCenter.getY()), 1024);
