...
public class EnragedPhase implements HydraPhaseSequence {
    ...
    public int autoAttack(final AlchemicalHydra hydra, final Player player) {
        final com.zenyte.game.world.entity.npc.combatdefs.AttackType style = previousStyle.equals(AttackType.RANGED) ? AttackType.MAGIC : AttackType.RANGED;
    ...
    public boolean attack(final AlchemicalHydra hydra, final Player player) {
        if (attacks == 3 && !initialSpecial || attacks == 9) {
            ...
            final com.zenyte.game.world.entity.Location base = new Location(player.getLocation());
            final java.util.ArrayList<com.zenyte.game.world.entity.Location> tiles = new ArrayList<Location>(pools);
            Collections.shuffle(PoisonPhase.offsets);
            poisonTiles.clear();
            tiles.add(base);
            for (final java.lang.Byte[] offset : PoisonPhase.offsets) {
                final com.zenyte.game.world.entity.Location tile = base.transform(offset[0], offset[1], 0);
            ...
            final com.zenyte.game.world.entity.Location hydraCenter = hydra.getMiddleLocation();
            for (final com.zenyte.game.world.entity.Location tile : tiles) {
                World.sendProjectile(hydra, tile, PoisonPhase.poisonAttackProj);
                final int direction = hydra.getRoundedDirection(Utils.getFaceDirection(tile.getX() - hydraCenter.getX(), tile.getY() - hydraCenter.getY()), 1024);
                final com.zenyte.game.util.Direction directionConstant = Utils.findMatching(Direction.values, dir -> dir.getNPCDirection() == direction);
                assert directionConstant != null;
                final com.zenyte.game.world.entity.masks.Graphics graphics = PoisonPhase.directionMap.get(directionConstant);
    ...
    public void process(final AlchemicalHydra hydra, final Player player) {
        for (final com.zenyte.game.world.entity.Location tile : poisonTiles) {
    ...
    public void setPreviousStyle(final AttackType previousStyle) {
