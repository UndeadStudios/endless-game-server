...
public class FlamePhase implements HydraPhaseSequence {
    ...
    private Location getNextLocation(@NotNull final Location player) {
        ...
        int radius = 0;
        final com.zenyte.game.world.entity.Location tile = new Location(player);
        while (++radius <= 15) {
    ...
    private Location getNextFollowLocation(final Location flame, final Location player) {
        ...
        final com.zenyte.game.util.Direction directionConstant = Utils.findMatching(Direction.values, dir -> dir.getNPCDirection() == entityDirection);
    ...
    private Location getOutsideInnerCenterLocation(final AlchemicalHydraInstance instance, final FireWallBlock block) {
        final java.util.List<com.zenyte.game.world.entity.Location> border = instance.getCenterBorder();
        Location location = null;
        double previousDistance = 50;
        for (final com.zenyte.game.world.entity.Location point : border) {
    ...
    public int autoAttack(final AlchemicalHydra hydra, final Player player) {
        final com.zenyte.game.world.entity.npc.combatdefs.AttackType style = hydra.getCombatDefinitions().getAttackStyle();
    ...
    private final List<WorldObject> spawnBlockingObjects(@NotNull final Rectangle outer, final int plane) {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.object.WorldObject> list = new ObjectArrayList<WorldObject>();
        for (int x = minX + 1; x < maxX; x++) {
            for (int y = minY + 1; y < maxY; y++) {
                final com.zenyte.game.world.object.WorldObject object = new WorldObject(26209, 10, 0, x, y, plane);
    ...
    private void constructBlock(@NotNull final AlchemicalHydra hydra, @NotNull final FireWallBlock block) {
        hydra.faceDirection(block.getDirection());
        for (final com.zenyte.game.world.entity.Location tile : block.getTiles()) {
    ...
    private void executeFireTrail(@NotNull final AlchemicalHydra hydra, @NotNull final FireWallBlock block, @NotNull final Player player, @NotNull final List<WorldObject> blockingObjects) {
        final com.zenyte.game.content.kebos.alchemicalhydra.instance.AlchemicalHydraInstance instance = hydra.getInstance();
        final com.zenyte.game.world.entity.Location startLocation = instance.getLocation(block.getMovingFireLocation());
        hydra.setAnimation(flameAttackAnim);
        ...
        WorldTasksManager.schedule(new TickTask() {
            ...
            public void run() {
                if (ticks == 0) {
                    ...
                    if (fireTile.getTileDistance(player.getLocation()) <= 2) {
                        if ((ticks & 1) == 1) {
                            final com.zenyte.game.world.entity.Location nextLocation = getNextFollowLocation(fireTile, new Location(player.getLocation()));
                            if (!fire.contains(nextLocation)) {
                                appendFire(new FireGraphics(43, delayedFlameGfx, new Location(fireTile = nextLocation)));
                            }
                        }
    ...
    public boolean attack(final AlchemicalHydra hydra, final Player player) {
        if (attacks == 3 && !initialSpecial || attacks == 21) {
            ...
            final java.awt.Rectangle outer = instance.getOuterCenter();
            final java.awt.Rectangle inner = instance.getInnerCenter();
            fire.clear();
    ...
    public void process(final AlchemicalHydra hydra, final Player player) {
        for (final com.zenyte.game.content.kebos.alchemicalhydra.npc.combat.phases.FlamePhase.FireGraphics graphics : fireGraphics) {
            graphics.process();
        }
        ...
        for (final com.zenyte.game.world.entity.Location tile : fire) {
            ...
            final com.zenyte.game.world.entity.Location destination = getNextLocation(currentTile);
