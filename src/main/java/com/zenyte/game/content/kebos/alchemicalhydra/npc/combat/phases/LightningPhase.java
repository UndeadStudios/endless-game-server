...
public class LightningPhase implements HydraPhaseSequence {
    ...
    private Location getNextFollowLocation(final Location lightning, final Location target) {
        ...
        final int direction = Utils.getFaceDirection(target.getX() - lightning.getX(), target.getY() - lightning.getY());
    ...
    public boolean attack(final AlchemicalHydra hydra, final Player player) {
        if (attacks == 3 && !initialSpecial || attacks == 9) {
            final com.zenyte.game.world.entity.Location center = hydra.getInstance().getLocation(LIGHTNING_CENTRAL_LOCATION);
                ...
                public void run() {
                    ...
                    if (corner != null && previousTick + delay == ticks) {
                        ...
                        WorldTasksManager.schedule(new TickTask() {
                            ...
                            public void run() {
                                if (ticks <= 11) {
                                    final com.zenyte.game.world.entity.Location nextLocation = getNextFollowLocation(lightning, player.getLocation());
