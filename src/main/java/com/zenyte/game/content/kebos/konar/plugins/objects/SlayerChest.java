 ...
 */
public class SlayerChest implements ObjectAction {
    ...
    private enum ChestReward {
        ...
        static {
            int total = 0;
            for (final SlayerChest.ChestReward entry : values()) {
        ...
        private static final Optional<Item> randomReward(@NotNull final Player player) {
            ...
            int current = 0;
            for (final Map.Entry<SlayerChest.ChestReward, ImmutableItem> reward : rewards.entrySet()) {
                final ImmutableItem item = reward.getValue();
                if ((current += (int) item.getRate()) >= random) {
                    if (item.getId() == -1) {
                        final Optional<SlayerChest.ChestReward.FishReward> fishReward = rollFish(player);
                        if (!fishReward.isPresent()) {
                            return Optional.empty();
                        }
                        final SlayerChest.ChestReward.FishReward fish = fishReward.get();
        ...
        private static final Optional<FishReward> rollFish(@NotNull final Player player) {
            ...
            final ObjectOpenHashSet<SlayerChest.ChestReward.FishReward> fishSet = new ObjectOpenHashSet<FishReward>();
            for (final SlayerChest.ChestReward.FishReward reward : FishReward.values) {
