...
public class Preset {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(Preset.class);
    @NotNull
    ...
    public Preset(@NotNull final String name, @NotNull final Player player) {
        ...
        final com.zenyte.game.world.entity.player.MemberRank memberRank = player.getMemberRank();
    ...
    private Int2ObjectMap<UnmodifiableItem> defineCollection(@NotNull final Container container) {
        final it.unimi.dsi.fastutil.ints.Int2ObjectOpenHashMap<com.zenyte.game.item.UnmodifiableItem> collection = new Int2ObjectOpenHashMap<UnmodifiableItem>();
    ...
    private Int2ObjectMap<UnmodifiableItem> defineCollectionCopy(@NotNull final Map<Integer, UnmodifiableItem> container) {
        final it.unimi.dsi.fastutil.ints.Int2ObjectOpenHashMap<com.zenyte.game.item.UnmodifiableItem> collection = new Int2ObjectOpenHashMap<UnmodifiableItem>();
        for (final java.util.Map.Entry<java.lang.Integer, com.zenyte.game.item.UnmodifiableItem> entry : container.entrySet()) {
    ...
    public void switchInventoryItem(final int fromSlot, final int toSlot) {
        final com.zenyte.game.item.UnmodifiableItem fromItem = inventory.get(fromSlot);
        final com.zenyte.game.item.UnmodifiableItem toItem = inventory.get(toSlot);
        if (fromItem == null) {
            return;
        }
    ...
    public void load(@NotNull final Player player) {
        ...
        final java.lang.StringBuilder builder = new StringBuilder(8192);
        ...
        final com.zenyte.game.content.preset.PresetLoadResponse inventoryResponse = loadInventory(player);
        final com.zenyte.game.content.preset.PresetLoadResponse equipmentResponse = loadEquipment(player);
        final com.zenyte.game.content.preset.PresetLoadResponse runePouchResponse = loadRunePouch(player, this.runePouch != null && player.getInventory().containsItem(ItemId.RUNE_POUCH));
        player.getInventory().refreshAll();
        player.getEquipment().refreshAll();
        player.getAppearance().resetRenderAnimation();
        final com.zenyte.game.world.entity.player.container.Container runePouchContainer = player.getRunePouch().getContainer();
        runePouchContainer.setFullUpdate(true);
        runePouchContainer.refresh(player);
        final com.zenyte.game.world.entity.player.CombatDefinitions combatDefinitions = player.getCombatDefinitions();
    ...
    boolean depositContainer(@NotNull final Player player, @NotNull final Container container) {
        ...
        final com.zenyte.game.world.entity.player.container.impl.bank.Bank bank = player.getBank();
        ...
        for (int slot = 0; slot < length; slot++) {
            ...
            if (isEquipment) {
                ...
                if (container.get(slot) == null) {
                    final com.zenyte.plugins.equipment.equip.EquipPlugin plugin = EquipPluginLoader.PLUGINS.get(item.getId());
                    if (plugin != null) {
                        try {
    ...
    PresetLoadResponse loadInventory(@NotNull final Player player) {
        ...
        final com.zenyte.game.world.entity.player.container.impl.bank.BankContainer bankContainer = bank.getContainer();
        for (int i = 0; i < size; i++) {
            final com.zenyte.game.item.UnmodifiableItem presetItem = ((Int2ObjectMap<UnmodifiableItem>) this.inventory).get(i);
            ...
            final com.zenyte.game.item.Item existingItem = inventory.getItem(i);
            //If the inventory slot is free, let's try to set the item.
            if (existingItem == null) {
                final java.util.OptionalInt slot = findSlotOfBestMatchingItem(bank, presetItem, true);
                if (slot.isPresent()) {
                    final int slotId = slot.getAsInt();
                    final com.zenyte.game.item.Item itemInBank = bank.get(slotId);
    ...
    PresetLoadResponse loadEquipment(@NotNull final Player player) {
        final com.zenyte.game.world.entity.player.container.impl.equipment.Equipment equipment = player.getEquipment();
        final com.zenyte.game.world.entity.player.container.Container equipmentContainer = equipment.getContainer();
        ...
        final com.zenyte.game.world.entity.player.container.impl.bank.BankContainer bankContainer = bank.getContainer();
        for (int i = 0; i < size; i++) {
            final com.zenyte.game.item.UnmodifiableItem presetItem = ((Int2ObjectMap<UnmodifiableItem>) this.equipment).get(i);
            //If no preset item is set in that slot, ignore it entirely, even if it remained filled from failing to offload the items into the bank.
            if (presetItem == null) {
                continue;
            }
            final int presetItemId = presetItem.getId();
            final com.zenyte.game.item.Item existingItem = equipment.getItem(i);
            //If the equipment slot is free, let's try to set the item.
            if (existingItem == null) {
                final java.util.OptionalInt slot = findSlotOfBestMatchingItem(bank, presetItem, false);
                if (slot.isPresent()) {
                    final int slotId = slot.getAsInt();
                    final com.zenyte.game.item.Item itemInBank = bank.get(slotId);
                    final int bankItemId = itemInBank.getId();
                    ...
                    final int succeededCount = bankContainer.removeSpecific(slotId, new Item(itemInBank.getId(), Math.min(itemInBank.getAmount(), presetItem.getAmount()), itemInBank.getAttributes()), alwaysSetPlaceholders, player).getSucceededAmount();
                    if (succeededCount > 0) {
                        final com.zenyte.game.item.Item it = new Item(bankItemId, succeededCount, itemInBank.getAttributesCopy());
                        equipmentContainer.set(i, it);
                        final com.zenyte.plugins.equipment.equip.EquipPlugin plugin = EquipPluginLoader.PLUGINS.get(bankItemId);
                        if (plugin != null) {
                            try {
    ...
    boolean mayEquip(@NotNull final Player player, @NotNull final Item item) {
        final mgi.types.config.items.ItemDefinitions definitions = item.getDefinitions();
        final com.zenyte.game.parser.impl.ItemRequirements.ItemRequirement requirement = definitions.getRequirements();
        final java.util.List<com.zenyte.game.parser.impl.ItemRequirements.ItemRequirement.PrimitiveRequirement> requirements = requirement == null ? null : requirement.getRequirements();
        if (requirements != null) {
            if (!requirements.isEmpty()) {
                final com.zenyte.game.world.entity.player.Skills skills = player.getSkills();
                if (requirements.size() >= 10) {
                    if (!skills.isMaxed()) {
                        return false;
                    }
                }
                final java.util.Iterator<com.zenyte.game.parser.impl.ItemRequirements.ItemRequirement.PrimitiveRequirement> it = requirements.iterator();
    ...
    PresetLoadResponse loadRunePouch(@NotNull final Player player, final boolean containsRunePouch) {
        ...
        final com.zenyte.game.world.entity.player.container.Container container = player.getRunePouch().getContainer();
        final int size = container.getContainerSize();
        final com.zenyte.game.world.entity.player.container.impl.bank.Bank bank = player.getBank();
        final boolean alwaysSetPlaceholders = bank.getSetting(BankSetting.ALWAYS_PLACEHOLDER) == 1;
        PresetLoadResponse loadResponse = FLAWLESS_LOAD;
        for (int i = 0; i < size; i++) {
            final com.zenyte.game.item.UnmodifiableItem presetItem = ((Int2ObjectMap<UnmodifiableItem>) this.runePouch).get(i);
            if (presetItem == null) {
                continue;
            }
            final int presetItemId = presetItem.getId();
            final int presetItemAmount = presetItem.getAmount();
            final com.zenyte.game.world.entity.player.container.Container pouch = player.getRunePouch().getContainer();
            final com.zenyte.game.item.Item existingItem = pouch.get(i);
            ...
            if (existingItem == null || existingItem.getId() == presetItemId && existingItem.getAmount() < presetItemAmount) {
                ...
                final com.zenyte.game.item.Item finalItem = container.get(i);
    ...
    OptionalInt findSlotOfBestMatchingItem(@NotNull final Bank bank, @NotNull final Item item, final boolean checkNotes) {
        ...
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> entry : bank.getContainer().getItems().int2ObjectEntrySet()) {
            final int entrySlot = entry.getIntKey();
            final com.zenyte.game.item.Item entryItem = entry.getValue();
            if (entryItem == null) {
                continue;
            }
