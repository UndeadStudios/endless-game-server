...
public enum PresetSubstitute {
    ...
    static {
        for (final com.zenyte.game.content.preset.PresetSubstitute value : values()) {
    ...
    public static final int iterateToFullId(final int itemId) {
        ...
        final com.zenyte.game.content.preset.PresetSubstitute current = ITEMS.get(itemId);
        int charges = current == null ? 0 : current.charges;
        while (true) {
            final com.zenyte.game.content.preset.PresetSubstitute item = BROKEN_ITEMS.get(id);
    ...
    public static final int getCompletelyDegradedId(final int itemId) {
        ...
        PresetSubstitute last = null;
        while (true) {
            final com.zenyte.game.content.preset.PresetSubstitute item = ITEMS.get(id);
            if (item == last || item == null) {
                return id;
            }
            last = item;
    ...
    public static final int getCharges(final int itemId) {
        final com.zenyte.game.content.preset.PresetSubstitute constant = ITEMS.get(itemId);
    ...
    public static final int[] findSubstitutes(final int id) {
        ...
        while (true) {
            final com.zenyte.game.content.preset.PresetSubstitute nextDeg = ITEMS.get(deg.getNextId());
    ...
    private PresetSubstitute(final int itemId, final int nextId, final int charges) {
        this.itemId = itemId;
        this.nextId = nextId;
