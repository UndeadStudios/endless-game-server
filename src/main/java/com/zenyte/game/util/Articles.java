...
public final class Articles {
    ...
    private static Articles.Result find(final String word) {
        Articles.Node node = NODE;
        int depth = 0;
        Articles.Ratio ratio = node.ratio;
        final int length = word.length();
        while(true) {
            if(depth >= length) {
                return new Articles.Result(ratio, word);
            } else {
    ...
    private static class Node {
        final Articles.Ratio ratio;
        final Articles.Node[] children;
        final char c;

        Node(final Articles.Ratio ratio, final Articles.Node[] children, final char c) {
            this.ratio = ratio;
    ...
    private static final class NodeParser {
        ...
        Articles.Node parse(final String string) {
            return this.parse0(string, '\u0000');
        }

        private Articles.Node parse0(final String string, final char cc) {
            final Articles.Ratio ratio = new Articles.Ratio(this.parseInt(string), this.parseInt(string));

            final int childCount = this.parseInt(string);
            final Articles.Node[] children;
            if(childCount != 0) {
                children = new Articles.Node[childCount];
            ...
            return new Articles.Node(ratio, children, cc);
        }
    ...
    public static final class Result {
        private final Articles.Ratio ratio;
        private final String word;

        Result(final Articles.Ratio ratio, final String word) {
            this.ratio = ratio;
            this.word = word;
        }
