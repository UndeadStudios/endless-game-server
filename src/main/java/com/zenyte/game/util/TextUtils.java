...
public class TextUtils {
	...
	public static String getItemLogDisplay(final Item[] items) {
		String list = "";
		for (int i = 0; i < 28; i++) {
			if (items[i] != null && items[i].getId() != -1) {

				list = list + "" + items[i].getDefinitions().getName();
				if (items[i].getAmount() > 1) {
					list = list + " x " + items[i].getAmount() + ", ";
				} else {
					list = list + ", ";
				}
			}
	...
	/*
	 * public static String getTradeItemLogDisplay(Player player) { Item[] items =
	 * player.getTradeItems().getItems(); String list = ""; for (int i = 0; i < 28;
	 * i++) { if (items[i] != null && items[i].getId() != -1) { list = list + "" +
	 * ItemDefinitionParser.getAreaName(items[i].getId()); if (items[i].getCount() > 1)
	 * { list = list + " x " + items[i].getCount() + ", "; } else { list = list +
	 * ", "; } } } if (list.equals("")) { list = "nothing"; } return list; }
	 */
