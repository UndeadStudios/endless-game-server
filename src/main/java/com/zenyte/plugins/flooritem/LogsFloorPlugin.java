...
public final class LogsFloorPlugin implements FloorItemPlugin {
	...
	public void handle(final Player player, final FloorItem item, final int optionId, final String option) {
		...
		final com.zenyte.game.content.skills.firemaking.Firemaking firemakingType = Firemaking.MAP.get(item.getId());
		if (firemakingType == null) {
			return;
		}
		final java.util.Optional<com.zenyte.game.content.skills.firemaking.FiremakingTool> tool = FiremakingTool.getAvailableTool(player);
		if (!tool.isPresent()) {
