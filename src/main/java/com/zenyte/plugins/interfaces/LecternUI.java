...
import static com.zenyte.game.content.skills.magic.spells.MagicSpell.AIR_RUNE;
import static com.zenyte.game.content.skills.magic.spells.MagicSpell.COSMIC_RUNE;
import static com.zenyte.game.content.skills.magic.spells.MagicSpell.EARTH_RUNE;
import static com.zenyte.game.content.skills.magic.spells.MagicSpell.FIRE_RUNE;
import static com.zenyte.game.content.skills.magic.spells.MagicSpell.LAW_RUNE;
import static com.zenyte.game.content.skills.magic.spells.MagicSpell.NATURE_RUNE;
import static com.zenyte.game.content.skills.magic.spells.MagicSpell.WATER_RUNE;
...
public final class LecternUI implements UserInterface {
	...
	private static enum Tablet {
		...
		private Tablet(final int componentId, final int level, final Item tab, final Item... runes) {
	...
	private static final class TabletCreation extends Action {
		...
		public int processWithDelay() {
			...
			if (player.getSkills().getLevel(Skills.MAGIC) < tab.level) {
		...
		private final boolean check() {
			if (!player.getInventory().containsItem(1761, 1) || !player.getInventory().containsItems(tab.runes)) {
				...
				player.sendMessage("You need " + builder.toString() + " and some soft clay to craft " + ((Utils.startWithVowel(tabName) ? "an " : "a ")) + tabName + " tablet.");
				return false;
			}
			return true;
		}
