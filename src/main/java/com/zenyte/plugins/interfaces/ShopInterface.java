...
public final class ShopInterface implements UserInterface {
	...
	public void handleComponentClick(final Player player, final int interfaceId, final int componentId, int slotId, final int itemId, final int optionId, final String option) {
		final java.lang.Object obj = player.getTemporaryAttributes().get("Shop");
		if (!(obj instanceof GameShop)) {
			return;
		}
		final com.zenyte.game.world.entity.player.container.impl.GameShop shop = (GameShop) obj;
		final com.zenyte.game.item.Item item = interfaceId == INVENTORY_INTERFACE ? player.getInventory().getContainer().get(slotId) : shop.getContainer().get(slotId - 1);
		if (item == null) {
			return;
		}
		final mgi.types.config.items.ItemDefinitions defs = ItemDefinitions.get(item.getId());
		switch (interfaceId) {
		case INTERFACE: 
			if (componentId == 16) {
				slotId--;
				...
				case 1: 
					...
					final java.lang.String currency = shop.getCurrency() == 995 ? "coins" : shop.getCurrency() == 11849 ? "marks of grace" : "";
