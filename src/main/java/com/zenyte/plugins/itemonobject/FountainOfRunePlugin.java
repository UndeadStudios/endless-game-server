		...
		WorldTasksManager.schedule(() -> {
			...
			for (int i = 0; i < Utils.random(1, 2); i++) {
				for (final com.zenyte.game.world.entity.player.Player player : players) {
					...
					final com.zenyte.game.world.entity.Location playerTile = player.getLocation();
					Location tile = null;
					int tryCount = 100;
					while (--tryCount > 0) {
					...
					final com.zenyte.game.world.Projectile projectile = PROJECTILES.get(Utils.random(PROJECTILES.size() - 1));
	...
	public void handleItemOnObjectAction(final Player player, final Item item, int slot, final WorldObject object) {
		...
		WorldTasksManager.schedule(() -> {
			...
			for (int i = container.getContainerSize(); i >= 0; i--) {
				final com.zenyte.game.item.Item containerItem = container.get(i);
