...
public final class CooksGuildEntranceDoor implements ObjectAction {
	...
	public static boolean hasEntranceEquipment(final Player player) {
		...
		final boolean hasVarrockArmour = DiaryReward.VARROCK_ARMOUR3.isEquipped(player) || DiaryReward.VARROCK_ARMOUR4.isEquipped(player);
		return hasHat || hasCape || hasVarrockArmour;
	}

	@Override
	public void handleObjectAction(final Player player, final WorldObject object, final String name, final int optionId, final String option) {
		final int cooking = player.getSkills().getLevelForXp(Skills.COOKING);
