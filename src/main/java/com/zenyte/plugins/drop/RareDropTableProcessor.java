...
public class RareDropTableProcessor extends DropProcessor {
    ...
    public void attach() {
        for (final int id : allIds) {
            //Rare drop table
            final com.zenyte.game.world.entity.npc.drop.StandardRareDropTable.SRDTNPC srdt = StandardRareDropTable.getTable(id);
            if (srdt == null) {
                continue;
            }
        ...
        for (final int id : allIds) {
            //Gem drop table
            final com.zenyte.game.world.entity.npc.drop.GemDropTable.GDTNPC grdt = GemDropTable.getTable(id);
            if (grdt == null) {
                continue;
            }
    ...
    public void onDeath(final NPC npc, final Player killer) {
        ...
        final com.zenyte.game.world.entity.npc.drop.StandardRareDropTable.SRDTNPC srdt = StandardRareDropTable.getTable(npc.getId());
        if (srdt != null) {
    ...
    @Override
    public int[] ids() {
        final it.unimi.dsi.fastutil.ints.IntList list = IntLists.synchronize(new IntArrayList(100));
        ...
        definitionsList.parallelStream().forEach(definition -> {
            ...
            final java.lang.String name = definition.getName();
