...
public class GargoyleProcessor extends DropProcessor {
    ...
    public void attach() {
        //Granite maul
        appendDrop(new DisplayedDrop(4153, 1, 1, 256));
        //Mystic robe top (dark)
        appendDrop(new DisplayedDrop(4101, 1, 1, 512));
        //Brittle key
        appendDrop(new DisplayedDrop(BRITTLE_KEY, 1, 1, 100));
    ...
    public Item drop(final NPC npc, final Player killer, final Drop drop, final Item item) {
        if (!drop.isAlways()) {
            if (killer.getSlayer().isCurrentAssignment(npc) && !killer.getBooleanAttribute("brittle-entrance_unlocked") && !killer.containsItem(BRITTLE_KEY) && random(100) == 0) {
