...
public class CatacombsProcessor extends DropProcessor {
    ...
    public void attach() {
        ...
        appendDrop(new DisplayedDrop(19679, 1, 1) {
            public double getRate(final Player player, final int id) {
                final com.zenyte.game.world.entity.npc.combatdefs.NPCCombatDefinitions definitions = NPCCDLoader.get(id);
                if (definitions == null) {
                    return 500;
                }
        ...
        appendDrop(new DisplayedDrop(19677, 1, 1) {
            public double getRate(final Player player, final int id) {
                final com.zenyte.game.world.entity.npc.combatdefs.NPCCombatDefinitions definitions = NPCCDLoader.get(id);
                return (int) (2.0F / 3.0F * (500 - Math.min(400, definitions.getHitpoints())));
            }
        });
    }

    public void onDeath(final NPC npc, final Player killer) {
        final int rate = 500 - npc.getMaxHitpoints();
        ...
        if (!SuperiorMonster.superiorMonsters.contains(npc.getId()) || CatacombsOfKourend.polygon.contains(npc.getLocation())) {
            final int shardRate = (int) (2.0F / 3.0F * (500 - Math.min(400, npc.getMaxHitpoints())));
    ...
    @Override
    public int[] ids() {
        final it.unimi.dsi.fastutil.ints.IntOpenHashSet set = new IntOpenHashSet(new int[] {7268, 7276, 7256, 7247, 7251, 7248, 7255, 7252, 7272, 7273, 7246, 7277, 7279, 7244, 7242, 7243, 7241, 7270, 7258, 7245, 7278, 7250, 7253, 7254, 7249, 7275, 7274, 7269, 7271, 7265, 7259, 7257, 7260, 7261, 7262});
