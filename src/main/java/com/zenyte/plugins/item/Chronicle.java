...
public class Chronicle extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        final com.zenyte.game.item.Item card = from.getId() == 13658 ? from : to;
    ...
    private static final class ChronicleTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            final int charges = player.getNumericAttribute("Chronicle charges").intValue();
            final java.lang.String message = charges == 1 ? "<col=7F00FF>You use your chronicle\'s last charge.</col>" : charges == 2 ? "<col=7F00FF>Your chronicle has one charge left.</col>" : "<col=7F00FF>Your chronicle has " + Utils.format(charges - 1) + " charges left.</col>";
            player.addAttribute("Chronicle charges", Math.max(0, player.getNumericAttribute("Chronicle charges").intValue() - 1));
            player.sendMessage(message);
        }

        public ChronicleTeleport(final Item item, final Location destination) {
            this.item = item;
