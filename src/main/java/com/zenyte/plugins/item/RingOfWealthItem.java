...
public class RingOfWealthItem extends ItemPlugin implements ItemOnItemAction {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        ...
        final com.zenyte.plugins.item.RingOfWealthItem.RingOfWealth unimbuedRing = RingOfWealth.get(from.getId() == RING_OF_WEALTH_IMBUE_SCROLL ? to : from);
        ...
        for (final com.zenyte.plugins.item.RingOfWealthItem.RingOfWealth ring : RingOfWealth.values.values()) {
    ...
    private enum RingOfWealth {
        ...
        static {
            for (final com.zenyte.plugins.item.RingOfWealthItem.RingOfWealth value : values()) {
        ...
        private RingOfWealth(final Item item, final Item result, final int charges, final boolean imbued) {
            this.item = item;
            this.result = result;
            this.charges = charges;
            this.imbued = imbued;
    ...
    private static final class RingTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            final com.zenyte.plugins.item.RingOfWealthItem.RingOfWealth ring = RingOfWealth.get(item);
            final int charges = ring.getCharges();
            final java.lang.String message = charges == 1 ? "<col=7F00FF>You use your amulet\'s last charge." : charges == 2 ? "<col=7F00FF>Your ring of wealth has one charge left..</col>" : "<col=7F00FF>Your ring of wealth has " + Utils.format(charges - 1) + " charges left.</col>";
            player.sendMessage(message);
            final com.zenyte.game.item.Item result = ring.getResult();
