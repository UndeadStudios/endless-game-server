...
public class RingOfSuffering extends ItemPlugin implements PairedItemOnItemPlugin, ChargeExtension {
    ...
    private void removeRecoil(final Player player, final Item item, final Container container, final int slotId) {
        player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                options("Are you sure you want to uncharge your <col=00080>" + item.getName() + "</col>?", "Yes, I\'m sure.", "No.").onOptionOne(() -> {
    ...
    private void uncharge(final Player player, final Item item, final Container container, final int slotId) {
        ...
        final com.zenyte.game.item.enums.ImbueableItem imbueable = ImbueableItem.get(item.getId());
        if (imbueable == null) {
            return;
        }
        final java.lang.String name = item.getName();
        final com.zenyte.game.item.Item uncharged = new Item(imbueable.getNormal());
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        final com.zenyte.game.item.Item recoil = ArrayUtils.contains(RECOILS, from.getId()) ? from : to;
        final int recoilId = recoil.getId();
        final com.zenyte.game.item.Item suffering = recoil == from ? to : from;
        ...
        player.sendInputInt("How many rings do you wish to use?", value -> {
            ...
            int total = 0;
            final com.zenyte.game.world.entity.player.container.ContainerResult result = inventory.deleteItem(recoilId, amount);
            total += result.getSucceededAmount();
            if (result.getSucceededAmount() != amount) {
                final int remainder = amount - result.getSucceededAmount();
                final com.zenyte.game.world.entity.player.container.ContainerResult additionalResult = inventory.deleteItem(recoilId == RECOIL ? NOTED_RECOIL : RECOIL, remainder);
