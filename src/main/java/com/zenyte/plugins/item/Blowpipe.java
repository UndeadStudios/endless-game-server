...
public class Blowpipe extends ItemPlugin implements ChargeExtension {
    ...
    public void handle() {
        bind("Unload", (player, item, slotId) -> {
            ...
            final com.zenyte.game.world.entity.player.container.ContainerResult result = player.getInventory().addItem(new Item(type, darts));
        ...
        bind("Dismantle", (player, item, slotId) -> {
            ...
            player.getDialogueManager().start(new OptionDialogue(player, "Dismantle the blowpipe for 20,000 Zulrah\'s scales?", new String[] {"Yes, dismantle the blowpipe.", "No, don\'t dismantle it."}, new Runnable[] {() -> {
                if (player.getInventory().getItem(slotId) != item) return;
    ...
    public void checkCharges(final Player player, final Item item) {
        ...
        final java.lang.String percentage = FORMATTER.format(scales / (float) 16383 * 100).replace(".0", "");
