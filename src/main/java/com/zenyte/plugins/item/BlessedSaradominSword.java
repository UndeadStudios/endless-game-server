...
public class BlessedSaradominSword extends ItemPlugin implements PairedItemOnItemPlugin {
                ...
                public void buildDialogue() {
                    ...
                    options("Revert the sword?", new DialogueOption("Revert it.", () -> {
                        player.getInventory().ifDeleteItem(item, () -> {
                            ...
                            final com.zenyte.game.item.Item tear = new Item(12804);
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        player.getDialogueManager().start(new Dialogue(player) {
            @Override
            public void buildDialogue() {
                final com.zenyte.game.item.Item blessedSword = new Item(12808);
                ...
                options("Combine the tear with the sword?", new DialogueOption("Combine them.", () -> {
                    ...
                    final com.zenyte.game.item.Item a = inventory.getItem(fromSlot);
                    final com.zenyte.game.item.Item b = inventory.getItem(toSlot);
                    if (a != from || b != to) {
                        return;
                    }
