...
public class RechargeDragonstoneJewelleryScroll extends ItemPlugin {
    ...
    public void handle() {
        bind("Read", (player, __, ___, slotId) -> {
            final com.zenyte.game.world.entity.player.container.Container[] containers = new Container[] {player.getInventory().getContainer(), player.getEquipment().getContainer()};
            int count = 0;
            int firstSuccessfulItem = -1;
            for (final com.zenyte.game.world.entity.player.container.Container ct : containers) {
                for (int i = ct.getContainerSize(); i >= 0; i--) {
                    final com.zenyte.game.item.Item containerItem = ct.get(i);
