...
public class SpinningPlate extends ItemPlugin {
    ...
    public void handle() {
        bind("Spin", (player, item, slotId) -> {
            ...
            final com.zenyte.game.world.entity.masks.Animation animation = new Animation(1902 + Utils.random(5));
            final int duration = animation.getCeiledDuration();
            final com.zenyte.game.world.entity.player.Emote nextEmote = animation.getId() <= 1903 ? Emote.CHEER : Emote.CRY;
            final int nextDuration = nextEmote.getAnimation().getCeiledDuration();
