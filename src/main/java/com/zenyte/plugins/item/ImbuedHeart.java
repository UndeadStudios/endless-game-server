	...
	public void handle() {
		bind("Invigorate", (player, item, slotId) -> {
			if (player.getVariables().getTime(TickVariable.IMBUED_HEART_COOLDOWN) == 0) {
				final int boost = (int) (player.getSkills().getLevel(Skills.MAGIC) * 0.1 + 1);
				...
				final int totalSeconds = (int) (player.getVariables().getTime(TickVariable.IMBUED_HEART_COOLDOWN) * 0.6F);
