...
public class TransmogrifyingRings implements EquipPlugin {
    ...
    public boolean handle(final Player player, final Item item, final int slotId, final int equipmentSlot) {
        ...
        final com.zenyte.plugins.item.TransmogrifyingRings.TransmogrifyRing ring = TransmogrifyRing.RINGS.get(item.getId());
        if (ring == null) {
            return false;
        }
    ...
    public enum TransmogrifyRing {
        ...
        static {
            for (final com.zenyte.plugins.item.TransmogrifyingRings.TransmogrifyRing ring : VALUES) {
