...
public class SlayerRingItem extends ItemPlugin {
    ...
    private enum SlayerRing {
        ...
        static {
            for (final com.zenyte.plugins.item.SlayerRingItem.SlayerRing value : values()) {
        ...
        private SlayerRing(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class RingTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            final com.zenyte.plugins.item.SlayerRingItem.SlayerRing ring = SlayerRing.get(item);
            final int charges = ring.getCharges();
            final java.lang.String message = charges == 1 ? "<col=7F00FF>Your slayer ring crumbles to dust.</col>" : charges == 2 ? "<col=7F00FF>Your slayer ring has one charge left..</col>" : ring == SlayerRing.ETERNAL ? "<col=7F00FF>No charges were used since the ring holds unlimited charges." : "<col=7F00FF>Your slayer ring has " + Utils.format(charges - 1) + " charges left.</col>";
