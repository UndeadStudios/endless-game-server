...
public class BurningAmulet extends ItemPlugin {
    ...
    public void handle() {
        bind("Rub", (player, item, container, slotId) -> {
            ...
            final com.zenyte.game.world.entity.player.dialogue.DialogueManager dialogueManager = player.getDialogueManager();
    ...
    private class BurningAmuletWarningDialogue extends Dialogue {
        ...
        @Override
        public void buildDialogue() {
            final java.lang.String wildyLevelString = "level " + wildernessLevel + " Wilderness.";
    ...
    private enum BurningAmuletItems {
        ...
        static {
            for (final com.zenyte.plugins.item.BurningAmulet.BurningAmuletItems value : values()) {
        ...
        private BurningAmuletItems(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class BurningAmuletTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            final com.zenyte.plugins.item.BurningAmulet.BurningAmuletItems amulet = BurningAmuletItems.get(item);
            final int charges = amulet.getCharges();
            final java.lang.String message = charges == 1 ? "<col=7F00FF>You use your amulet\'s last charge.</col>" : charges == 2 ? "<col=7F00FF>Your amulet has one charge left.</col>" : "<col=7F00FF>Your amulet has " + Utils.format(charges - 1) + " charges left.</col>";
            player.sendMessage(message);
            final com.zenyte.game.item.Item result = amulet.getResult();
