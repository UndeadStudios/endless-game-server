...
public class DigsitePendantItem extends ItemPlugin {
    ...
    private enum Pendant {
        ...
        static {
            for (final com.zenyte.plugins.item.DigsitePendantItem.Pendant amulet : values()) {
        ...
        private Pendant(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class PendantTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            final com.zenyte.plugins.item.DigsitePendantItem.Pendant pendant = Pendant.get(item);
            final int charges = pendant.getCharges();
            final java.lang.String message = charges == 1 ? "<col=7F00FF>You use your pendant\'s last charge.</col>" : charges == 2 ? "<col=7F00FF>Your pendant has one charge left.</col>" : "<col=7F00FF>Your pendant has " + Utils.format(charges - 1) + " charges left.</col>";
            player.sendMessage(message);
            final com.zenyte.game.item.Item result = pendant.getResult();
