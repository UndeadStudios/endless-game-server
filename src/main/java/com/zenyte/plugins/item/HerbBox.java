...
public class HerbBox extends ItemPlugin {
    ...
    private enum BoxedHerb {
        GUAM(199, 32), MARRENTILL(201, 24), TARROMIN(203, 17), HARRALANDER(205, 14), RANARR(207, 11), IRIT(209, 8), AVANTOE(211, 6), KWUARM(213, 6), CADANTINE(215, 4), LANTADYME(2485, 3), DWARF_WEED(217, 3);
        ...
        private static final Optional<BoxedHerb> getReward() {
            ...
            int current = 0;
            for (final com.zenyte.plugins.item.HerbBox.BoxedHerb reward : values) {
        ...
        private BoxedHerb(final int id, final int weight) {
            this.id = id;
            this.weight = weight;
        }
    }
    ...
    public void handle() {
        bind("Take-one", (player, item, container, slotId) -> {
            ...
            final com.zenyte.game.item.Item herb = new Item(BoxedHerb.getReward().orElseThrow(RuntimeException::new).id);
            if (count >= 9) {
