...
public class MaxCape extends ItemPlugin {
    ...
    public void handle() {
        bind("Features", (player, item, container, slotId) -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                final java.util.ArrayList<com.zenyte.game.world.entity.player.dialogue.Dialogue.DialogueOption> list = new ArrayList<DialogueOption>();
                ...
                list.add(new DialogueOption("Stamina Boost", () -> {
                    final java.lang.Number lastConsumptionDate = player.getNumericAttribute("Stamina Boost Use");
                ...
                options(300, "What would you like to search for?", new DialogueOption("Pestle and Mortar", () -> {
                    ...
                    if (remaining >= 1) {
                        final java.lang.String remainingString = remaining == 2 ? "twice more today" : "one more time today";
        ...
        bind("Teleports", (player, item, slotId) -> {
            player.getDialogueManager().start(new OptionsMenuD(player, "Select a destination", "Warrior\'s Guild", "Fishing Guild", "Crafting Guild", "Farming Guild", "Otto\'s Grotto", "Chinchompas", "POH Portals") {
                ...
                public void handleClick(int slotId) {
                    if (slotId <= 4) {
                        final com.zenyte.game.content.skills.magic.spells.teleports.ItemTeleport teleport = TELEPORTS[slotId];
    ...
    static final void sendSpellbookDialogue(final Player player) {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.player.dialogue.Dialogue.DialogueOption> list = new ObjectArrayList<Dialogue.DialogueOption>();
        final com.zenyte.game.content.skills.magic.Spellbook currentSpellbook = player.getCombatDefinitions().getSpellbook();
    ...
    private enum POHPortal implements Teleport {
        HOME(null), RIMMINGTON(new Location(2954, 3224, 0)), TAVERLEY(new Location(2894, 3465, 0)), POLLNIVNEACH(new Location(3340, 3004, 0)), HOSIDIUS(new Location(1743, 3517, 0)), RELLEKKA(new Location(2670, 3632, 0)), BRIMHAVEN(new Location(2758, 3178, 0)), YANILLE(new Location(2544, 3095, 0));
        ...
        static {
            for (final com.zenyte.plugins.item.MaxCape.POHPortal tele : values) {
    ...
    static final void sendPOHPortalTeleports(final Player player) {
        player.getDialogueManager().start(new OptionsMenuD(player, "House portal teleports", POHPortal.map.keySet().toArray(new String[0])) {
            @Override
            public void handleClick(int slotId) {
                final com.zenyte.plugins.item.MaxCape.POHPortal type = POHPortal.values[slotId];
