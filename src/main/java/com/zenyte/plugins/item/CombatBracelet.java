...
public class CombatBracelet extends ItemPlugin {
    ...
    private enum CombatBraceletItem {
        ...
        static {
            for (final com.zenyte.plugins.item.CombatBracelet.CombatBraceletItem value : values()) {
        ...
        private CombatBraceletItem(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class BraceletTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            final com.zenyte.plugins.item.CombatBracelet.CombatBraceletItem bracelet = CombatBraceletItem.get(item);
            final int charges = bracelet.getCharges();
            final java.lang.String message = charges == 1 ? "<col=7F00FF>You use your combat bracelet\'s last charge.</col>" : charges == 2 ? "<col=7F00FF>Your combat bracelet has one charge left.</col>" : "<col=7F00FF>Your combat bracelet has " + Utils.format(charges - 1) + " charges left.</col>";
            player.sendMessage(message);
            final com.zenyte.game.item.Item result = bracelet.getResult();
