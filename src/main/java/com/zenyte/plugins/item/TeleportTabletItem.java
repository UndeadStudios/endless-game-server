...
public class TeleportTabletItem extends ItemPlugin {
	...
	public void handle() {
			...
			player.getDialogueManager().start(new Dialogue(player) {
				...
				public void buildDialogue() {
					options("Revert these tablets?", new DialogueOption("Revert ALL - you will not get your scrolls back.", () -> {
						...
						if (succeeded > 0) {
							final com.zenyte.game.item.Item houseTabs = new Item(8013, succeeded);
		...
		bind("Toggle", (player, item, slotId) -> {
			final com.zenyte.game.content.itemtransportation.TeleportTablet tablet = TeleportTablet.getTeleportTab(item);
			String current;
			String target;
			Setting setting;
			int currentValue;
			...
			final java.lang.String message = "Your default option is now the " + target + " teleport.";
			player.getDialogueManager().start(new Dialogue(player) {
				@Override
				public void buildDialogue() {
