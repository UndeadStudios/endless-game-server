...
public class NecklaceOfPassageItem extends ItemPlugin {
    ...
    private enum NecklaceOfPassage {
        ...
        static {
            for (final com.zenyte.plugins.item.NecklaceOfPassageItem.NecklaceOfPassage value : values()) {
        ...
        private NecklaceOfPassage(final Item item, final Item result, final int charges) {
            this.item = item;
            this.result = result;
    ...
    private static final class NecklaceTeleport implements Teleport {
        ...
        public void onArrival(final Player player) {
            final com.zenyte.plugins.item.NecklaceOfPassageItem.NecklaceOfPassage necklace = NecklaceOfPassage.get(item);
            final int charges = necklace.getCharges();
            final java.lang.String message = charges == 1 ? "<col=7F00FF>Your necklace of passage crumbles to dust.</col>" : charges == 2 ? "<col=7F00FF>Your necklace of passage has one use left.</col>" : "<col=7F00FF>Your necklace of passage has " + Utils.format(charges - 1) + " uses left.</col>";
            player.sendMessage(message);
            final com.zenyte.game.item.Item result = necklace.getResult();
