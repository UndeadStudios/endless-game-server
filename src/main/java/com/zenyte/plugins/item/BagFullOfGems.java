...
public class BagFullOfGems extends ItemPlugin {
	...
	private enum Gem {
		...
		private Gem(final int notedId, final float percentage) {
	...
	public void handle() {
		bind("Open", (player, item, slotId) -> {
			...
			int spaceRequired = 5;
			for (final com.zenyte.plugins.item.BagFullOfGems.Gem gem : Gem.VALUES) {
			...
			final it.unimi.dsi.fastutil.ints.Int2IntOpenHashMap map = new Int2IntOpenHashMap(6);
			for (int i = 0; i < 40; i++) {
			...
			final it.unimi.dsi.fastutil.objects.ObjectIterator<it.unimi.dsi.fastutil.ints.Int2IntMap.Entry> iterator = map.int2IntEntrySet().fastIterator();
			while (iterator.hasNext()) {
				final it.unimi.dsi.fastutil.ints.Int2IntMap.Entry next = iterator.next();
