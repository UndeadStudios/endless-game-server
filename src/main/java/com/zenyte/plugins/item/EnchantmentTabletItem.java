...
public class EnchantmentTabletItem extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    private enum EnchantmentTab {
        ...
        private EnchantmentTab(final int tabId, final JewelleryEnchantmentType type, final String typeString) {
            this.tabId = tabId;
            this.type = type;
    ...
    public void handle() {
        bind("Info", (player, item, slotId) -> {
            final com.zenyte.plugins.item.EnchantmentTabletItem.EnchantmentTab tab = Objects.requireNonNull(EnchantmentTab.get(item.getId()));
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        final com.zenyte.plugins.item.EnchantmentTabletItem.EnchantmentTab tab = Utils.getOrDefault(EnchantmentTab.get(from.getId()), EnchantmentTab.get(to.getId()));
        final com.zenyte.game.item.Item jewellery = tab.tabId == from.getId() ? to : from;
        final int jewellerySlot = tab.tabId == from.getId() ? toSlot : fromSlot;
        final com.zenyte.game.content.skills.magic.actions.JewelleryEnchantment.JewelleryEnchantmentItem data = Utils.findMatching(JewelleryEnchantment.JewelleryEnchantmentItem.values, item -> item.getBase().getId() == jewellery.getId());
        assert data != null;
    ...
    public int[] getItems() {
        ...
        for (final com.zenyte.plugins.item.EnchantmentTabletItem.EnchantmentTab tab : EnchantmentTab.values) {
            list.add(tab.tabId);
        }
    ...
    public ItemPair[] getMatchingPairs() {
        ...
        for (final com.zenyte.plugins.item.EnchantmentTabletItem.EnchantmentTab tab : EnchantmentTab.values) {
            for (final com.zenyte.game.content.skills.magic.actions.JewelleryEnchantment.JewelleryEnchantmentItem jewellery : JewelleryEnchantment.JewelleryEnchantmentItem.values) {
