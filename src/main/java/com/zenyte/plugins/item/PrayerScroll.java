...
public class PrayerScroll extends ItemPlugin {
	...
	public void handle() {
		bind("Read", (player, item, slotId) -> {
			...
			final java.lang.String name = id == 21079 ? "Augury" : id == 21034 ? "Rigour" : "Preserve";
	...
	private final void readScroll(final Player player, final Item item, final int slotId) {
		...
		final com.zenyte.game.item.Item inSlot = inventory.getItem(slotId);
		if (inSlot != item) {
			return;
		}
		final int id = item.getId();
		final java.lang.String name = id == 21079 ? "Augury" : id == 21034 ? "Rigour" : "Preserve";
		final com.zenyte.game.world.entity.player.Setting setting = id == 21079 ? Setting.AUGURY : id == 21034 ? Setting.RIGOUR : Setting.PRESERVE;
		player.lock(5);
