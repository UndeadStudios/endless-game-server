...
public class TomeOfExperience extends ItemPlugin {
    ...
    public void handle() {
        bind("Read", (player, item, container, slotId) -> player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                item(item, "Reading the tome will grant you five minutes of 50% bonus experience in anything you do.<br>This bonus experience will not stack with the global bonus experience.");
    ...
    private final void read(@NotNull final Player player, @NotNull final Item item, final int slot, final int amount) {
        ...
        player.sendMessage(Colour.RED.wrap("You\'ve read " + usedAmount + " tome" + (usedAmount == 1 ? "" : "s") + " of experience and gained " + (usedAmount * 5) + " minutes of private bonus experience."));
