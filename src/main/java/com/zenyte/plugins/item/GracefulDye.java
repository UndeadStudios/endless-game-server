...
public class GracefulDye extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    private enum RegularGraceful {
        ...
        private RegularGraceful(final int itemId) {
            this.itemId = itemId;
        }
    }
    ...
    private enum RecolouredGraceful {
        ...
        private RecolouredGraceful(final RecolourDye dye, final int recolouredPiece) {
            this.dye = dye;
    ...
    public static final class GracefulCleaning implements PairedItemOnItemPlugin {
        ...
        public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
            final com.zenyte.game.item.Item cloth = from.getId() == ItemId.CLEANING_CLOTH ? from : to;
            final com.zenyte.game.item.Item piece = cloth == to ? from : to;
            final int slot = piece.getDefinitions().getSlot();
            final com.zenyte.plugins.item.GracefulDye.RegularGraceful regularPiece = Utils.findMatching(RegularGraceful.values, p -> ItemDefinitions.getOrThrow(p.itemId).getSlot() == slot);
        ...
        public ItemPair[] getMatchingPairs() {
            final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.item.ItemOnItemAction.ItemPair> pairs = new ObjectArrayList<ItemPair>();
            for (final com.zenyte.plugins.item.GracefulDye.RecolouredGraceful piece : RecolouredGraceful.values) {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.plugins.item.GracefulDye.RecolourDye> colourList = new ObjectArrayList<RecolourDye>();
        final com.zenyte.game.item.Item gracefulPiece = from.getId() == GRACEFUL_DYE ? to : from;
        ...
        for (final com.zenyte.plugins.item.GracefulDye.RecolouredGraceful recolour : RecolouredGraceful.values) {
            final mgi.types.config.items.ItemDefinitions def = ItemDefinitions.getOrThrow(recolour.recolouredPiece);
    ...
    private static final boolean isWhiteUnlocked(@NotNull final Player player) {
        for (final com.zenyte.game.content.achievementdiary.DiaryReward reward : DiaryReward.VALUES) {
    ...
    private static final void recolour(@NotNull final Player player, @NotNull final Item from, @NotNull final Item to, final int fromSlot, final int toSlot, @NotNull final RecolourDye dye) {
        final com.zenyte.game.item.Item dyeItem = from.getId() == GRACEFUL_DYE ? from : to;
        final com.zenyte.game.item.Item gracefulPiece = dyeItem == from ? to : from;
        ...
        final com.zenyte.plugins.item.GracefulDye.RecolouredGraceful recolourConst = Utils.findMatching(RecolouredGraceful.values, value -> value.dye == dye && ItemDefinitions.getOrThrow(value.recolouredPiece).getSlot() == gracefulEquipSlot);
    ...
    public ItemPair[] getMatchingPairs() {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.item.ItemOnItemAction.ItemPair> pairs = new ObjectArrayList<ItemPair>();
        for (final com.zenyte.plugins.item.GracefulDye.RegularGraceful piece : RegularGraceful.values) {
