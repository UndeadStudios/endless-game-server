...
public class BottomlessCompostBucket extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handle() {
        bind("Check", (player, item, container, slotId) -> {
            ...
            final java.lang.String compostName = type == 0 ? "compost" : type == 1 ? "supercompost" : "ultracompost";
            ...
            if (charges == 0) {
                ...
                if (differenceCount == 1) {
                    ...
                    player.getDialogueManager().start(new Dialogue(player) {
                        @Override
                        public void buildDialogue() {
                            final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.world.entity.player.dialogue.Dialogue.DialogueOption> list = new ObjectArrayList<DialogueOption>();
                            if (compost != 0) {
    ...
    private final void fill(final Player player, final Item item, final int type) {
        ...
        final Runnable runnable = () -> {
            ...
            final java.lang.String typeName = type == 0 ? "Compost" : type == 1 ? "Supercompost" : "Ultracompost";
            final com.zenyte.game.world.entity.player.container.impl.Inventory inventory = player.getInventory();
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final com.zenyte.game.item.Item bucket = from.getId() == 22994 || from.getId() == 22997 ? from : to;
        final com.zenyte.game.item.Item compost = bucket == from ? to : from;
