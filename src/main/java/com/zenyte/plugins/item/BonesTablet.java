...
public class BonesTablet extends ItemPlugin {
    ...
    public static final int convertBones(@NotNull final Player player, final int transformedItem) {
        int amount = 0;
        for (final com.zenyte.game.item.Item item : player.getInventory().getContainer().getItems().values()) {
            if (item == null) {
                continue;
            }
    ...
    public void handle() {
        bind("Break", (player, tablet, container, slotId) -> {
            ...
            if (amount > 0) {
                ...
                final java.lang.String typeString = tablet.getId() == 8014 ? "bananas" : "peaches";
