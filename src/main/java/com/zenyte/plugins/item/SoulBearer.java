...
public class SoulBearer extends ItemPlugin implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final com.zenyte.game.world.entity.player.container.impl.Inventory inventory = player.getInventory();
        final com.zenyte.game.item.Item bearer = from.getId() == ItemId.SOUL_BEARER ? from : to;
        final com.zenyte.game.item.Item ensouledHead = bearer == from ? to : from;
    ...
    public ItemPair[] getMatchingPairs() {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<com.zenyte.game.item.ItemOnItemAction.ItemPair> list = new ObjectArrayList<ItemPair>();
        for (final com.zenyte.game.content.skills.magic.spells.arceuus.Reanimation reanimation : Reanimation.values()) {
