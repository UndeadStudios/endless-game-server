...
public class LavaScaleOnAntifireAction implements ItemOnItemAction {
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        ...
        final com.zenyte.game.item.Item potion = from.getId() >= 2452 && from.getId() <= 2458 ? from : to;
        final int slot = player.getInventory().getItem(fromSlot).getId() == potion.getId() ? fromSlot : toSlot;
        final java.lang.Integer dose = Integer.valueOf(potion.getName().substring(16, 17));
