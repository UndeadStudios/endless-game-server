...
public final class CookingComboItemAction implements ItemOnItemAction {
    ...
    public enum CookingCombination {
        ...
        public boolean hasRequirements(final Player player) {
            ...
            if (checkForSpace && player.getInventory().getFreeSlots() < spaceNeeded) {
                player.sendMessage("You need at least " + spaceNeeded + " free slot(s) to cut this!");
                return false;
            }
            return player.getInventory().containsItem(raw[0], 1) && player.getInventory().containsItem(raw[1], 1) && player.getSkills().getLevel(Skills.COOKING) >= level;
        }
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        final java.util.List<com.zenyte.plugins.itemonitem.CookingComboItemAction.CookingCombination> combos = CookingCombination.getCombos(from.getId(), to.getId());
        if (combos.isEmpty()) {
