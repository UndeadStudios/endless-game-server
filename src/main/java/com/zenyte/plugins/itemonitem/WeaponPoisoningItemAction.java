...
public class WeaponPoisoningItemAction implements ItemOnItemAction {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final com.zenyte.game.item.Item poison = from.getName().contains("Weapon poison") ? from : to;
        final com.zenyte.game.item.Item weapon = !from.getName().contains("Weapon poison") ? from : to;
        final com.zenyte.game.content.skills.herblore.PoisonAttachement attachement = PoisonAttachement.get(poison.getId());
        if (attachement == null) {
            return;
        }
        final com.zenyte.game.content.skills.herblore.PoisonableWeapon poisonableWeapon = PoisonableWeapon.get(weapon.getId(), attachement);
        ...
        final java.lang.String name = weapon.isStackable() ? (weapon.getName().endsWith("s") ? weapon.getName() : weapon.getName() + "s") : weapon.getName();
    ...
    public int[] getItems() {
        ...
        for (final com.zenyte.game.content.skills.herblore.PoisonableWeapon weapon : PoisonableWeapon.ALL) {
            if (!list.contains(weapon.getBase())) {
                list.add(weapon.getBase());
            }
        }
        for (final com.zenyte.game.content.skills.herblore.PoisonAttachement attachement : PoisonAttachement.ALL) {
