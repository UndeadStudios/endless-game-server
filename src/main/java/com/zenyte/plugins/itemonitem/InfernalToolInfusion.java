...
public class InfernalToolInfusion implements PairedItemOnItemPlugin {
    ...
    private enum InfernalTool {
        AXE(Skills.WOODCUTTING, Skills.FIREMAKING, 61, 85, new int[] {ItemId.DRAGON_AXE, ItemId.INFERNAL_AXE_UNCHARGED}, ItemId.INFERNAL_AXE), PICKAXE(Skills.MINING, Skills.SMITHING, 61, 85, new int[] {ItemId.DRAGON_PICKAXE, ItemId.INFERNAL_PICKAXE_UNCHARGED}, ItemId.INFERNAL_PICKAXE), HARPOON(Skills.FISHING, Skills.COOKING, 75, 85, new int[] {ItemId.DRAGON_HARPOON, ItemId.INFERNAL_HARPOON_UNCHARGED}, ItemId.INFERNAL_HARPOON);
        ...
        private InfernalTool(final int primarySkill, final int secondarySkill, final int primaryRequirement, final int secondaryRequirement, final int[] usableWeaponry, final int product) {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final com.zenyte.game.item.Item consumedItem = (from.getId() == SMOULDERING_STONE || (Utils.findMatching(InfernalTool.values, value -> (value.usableWeaponry[1] == to.getId() || value.product == to.getId())) != null)) ? from : to;
        final com.zenyte.game.item.Item tool = from == consumedItem ? to : from;
        ...
        final com.zenyte.plugins.itemonitem.InfernalToolInfusion.InfernalTool constant = Objects.requireNonNull(Utils.findMatching(InfernalTool.values, value -> value.product == tool.getId() || ArrayUtils.contains(value.usableWeaponry, tool.getId())));
        ...
        player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                final java.lang.String productName = ItemDefinitions.getOrThrow(constant.product).getName();
                if (isDragon) {
