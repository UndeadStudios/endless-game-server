...
public class PoisonedWeaponCleaningAction implements ItemOnItemAction {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final com.zenyte.game.item.Item weapon = from.getId() != CLEANING_CLOTH.getId() ? from : to;
        final int weaponSlot = from.getId() != CLEANING_CLOTH.getId() ? fromSlot : toSlot;
        final com.zenyte.game.content.skills.herblore.PoisonableWeapon poisonableWeapon = PoisonableWeapon.get(weapon.getId());
        if (poisonableWeapon == null) {
            return;
        }
        final com.zenyte.game.item.Item item = new Item(poisonableWeapon.getBase(), weapon.getAmount());
