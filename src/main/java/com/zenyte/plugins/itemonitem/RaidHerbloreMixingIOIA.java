...
public final class RaidHerbloreMixingIOIA implements PairedItemOnItemPlugin {
	...
	public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
		final com.zenyte.game.content.chambersofxeric.skills.RaidHerblore.RaidHerb fromHerb = Utils.findMatching(RaidHerblore.RaidHerb.values, value -> value.getClean() == from.getId());
		final com.zenyte.game.content.chambersofxeric.skills.RaidHerblore.RaidHerb toHerb = Utils.findMatching(RaidHerblore.RaidHerb.values, value -> value.getClean() == to.getId());
		if (fromHerb == null && toHerb == null) {
			player.sendMessage("You should mix your herb in before adding any other ingredients.");
			return;
		}
		final com.zenyte.game.item.Item[] potions = RaidHerblore.RaidPotion.getPotions(player, from, to);
		if (potions.length == 0) {
	...
	public ItemPair[] getMatchingPairs() {
		...
		for (final com.zenyte.game.content.chambersofxeric.skills.RaidHerblore.RaidPotion potion : RaidHerblore.RaidPotion.values()) {
