...
public class ItemOnMaxCapeAction implements ItemOnItemAction {
	...
	public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
		final com.zenyte.game.item.Item cape = from.getId() == 13342 ? from : to;
		final com.zenyte.game.content.MaxCape upgraded = MaxCape.get(from.getId() != 13342 ? from.getId() : to.getId());
		if (upgraded == null) {
			return;
		}
		final com.zenyte.game.item.Item upgrade = new Item(upgraded.getUpgrade());
		final com.zenyte.game.item.Item upgradedCape = new Item(upgraded.getCape());
		final com.zenyte.game.item.Item upgradedHood = new Item(upgraded.getHood());
	...
	public ItemPair[] getMatchingPairs() {
		final com.zenyte.game.item.ItemOnItemAction.ItemPair[] pairs = new ItemPair[MaxCape.values.length];
		for (int i = 0; i < pairs.length; i++) {
			final com.zenyte.game.content.MaxCape cape = MaxCape.values[i];
