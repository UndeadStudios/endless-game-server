...
public class ItemOnLootingBagAction implements ItemOnItemAction {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        final com.zenyte.game.item.Item bag = from.getId() == 11941 ? from : to;
        final com.zenyte.game.item.Item item = from.getId() != 11941 ? from : to;
        ...
        final mgi.types.config.items.ItemDefinitions def = item.getDefinitions();
        ...
        if (amount > 1 && player.getBooleanAttribute("looting_bag_amount_prompt")) {
            final java.lang.String[] options = getOptions(item);
