...
public class GuthixRestTeaMixing implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(Player player, Item from, Item to, int fromSlot, int toSlot) {
        ...
        final com.zenyte.game.item.Item herb = isHerb(from) ? from : to;
        final com.zenyte.game.item.Item mix = herb == from ? to : from;
        final int[] existingArray = mix.getId() == ItemId.CUP_OF_HOT_WATER ? new int[0] : Objects.requireNonNull(Utils.findMatching(PartialTea.values, tea -> tea.teaId == mix.getId())).ingredients;
        final it.unimi.dsi.fastutil.ints.IntArrayList ingredientsList = new IntArrayList(existingArray);
        ingredientsList.add(herb.getId());
        final int[] ingredientsArray = ingredientsList.toIntArray();
        Arrays.sort(ingredientsArray);
        final com.zenyte.plugins.itemonitem.GuthixRestTeaMixing.PartialTea upgradedTea = Utils.findMatching(PartialTea.values, tea -> Arrays.equals(tea.ingredients, ingredientsArray));
        if (upgradedTea == null) {
    ...
    public ItemPair[] getMatchingPairs() {
        ...
        for (final com.zenyte.plugins.itemonitem.GuthixRestTeaMixing.PartialTea partialTea : PartialTea.values) {
