...
public class DarkBowPaint implements ItemOnItemAction {
	...
	public static enum BowPaint {
		...
		static {
			for (final com.zenyte.plugins.itemonitem.DarkBowPaint.BowPaint value : VALUES) {
		...
		private BowPaint(final int paintId, final int recolouredBowId) {
			this.paintId = paintId;
			this.recolouredBowId = recolouredBowId;
		}

		public int getRecolouredBowId() {
			return this.recolouredBowId;
		}
	}

	@Override
	public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
		final com.zenyte.game.item.Item bow = from.getId() == 11235 ? from : to;
		final com.zenyte.game.item.Item dye = bow == from ? to : from;
		final com.zenyte.plugins.itemonitem.DarkBowPaint.BowPaint mix = BowPaint.MAPPED_VALUES.get(dye.getId());
		if (mix == null) {
			return;
		}
