...
public final class LeatherCraftingItemAction implements ItemOnItemAction {
	...
	public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
		if (LeatherCrafting.isMaterial(player, from, to)) {
			final com.zenyte.game.item.Item[] items = LeatherCrafting.getCategory(player, from, to);
	...
	public int[] getItems() {
		...
		for (final com.zenyte.game.item.Item[] items : CraftingDefinitions.MATERIALS) {
