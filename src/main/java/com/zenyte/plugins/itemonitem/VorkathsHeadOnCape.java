...
public class VorkathsHeadOnCape implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        final com.zenyte.game.item.Item head = from.getId() == ItemId.VORKATHS_HEAD_21907 ? from : to;
        final com.zenyte.game.item.Item cape = from == head ? to : from;
        player.getDialogueManager().start(new Dialogue(player) {
            ...
            public void buildDialogue() {
                ...
                doubleItem(head, cape, "Imbuing the cape with the Vorkath\'s head will consume the head and make the cape unstackable. Losing the cape means losing the effect. Do you wish to continue?");
