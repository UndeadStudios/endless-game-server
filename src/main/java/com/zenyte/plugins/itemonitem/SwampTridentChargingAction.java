...
public final class SwampTridentChargingAction implements PairedItemOnItemPlugin {
    ...
    public void handleItemOnItemAction(final Player player, final Item from, final Item to, final int fromSlot, final int toSlot) {
        final com.zenyte.game.item.Item tridentItem = SwampTrident.allTridentsMap.keySet().contains(from.getId()) ? from : to;
        final com.zenyte.plugins.item.trident.SwampTrident tridentDef = SwampTrident.allTridentsMap.get(tridentItem.getId());
    ...
    public ItemPair[] getMatchingPairs() {
        final it.unimi.dsi.fastutil.objects.ObjectOpenHashSet<com.zenyte.game.item.ItemOnItemAction.ItemPair> pairs = new ObjectOpenHashSet<ItemPair>();
