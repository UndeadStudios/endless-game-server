    ...
    public void handle() {
        bind("Travel", (player, npc) ->
                new FadeScreen(player, () ->
                        player.setLocation(player.getLocation().withinDistance(westernGuide, 15) ? new Location(1401, 3536, 0)
                                : new Location(1277, 3558, 0))).fade(3));
    }

    @Override
    public int[] getNPCs() {
        return new int[] {
                NpcId.MOUNTAIN_GUIDE, NpcId.MOUNTAIN_GUIDE_7600, 9346
        };
    }
}
