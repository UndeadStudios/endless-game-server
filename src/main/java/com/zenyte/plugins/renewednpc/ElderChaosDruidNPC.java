...
public class ElderChaosDruidNPC extends NPCPlugin implements ItemOnNPCAction {
    ...
    public void handle() {
        bind("Talk-to", (player, npc) -> {
            ...
            for (int i = 0; i < 28; i++) {
                ...
                final com.zenyte.game.content.skills.prayer.actions.Bones bone = Bones.getBone(item.getDefinitions().getNotedId());
                ...
                player.getDialogueManager().start(new Dialogue(player, npc) {
                    ...
                    public void buildDialogue() {
                        ...
                        final java.util.ArrayList<com.zenyte.game.world.entity.player.dialogue.Dialogue.DialogueOption> optionsList = new ArrayList<DialogueOption>(5);
                        ...
                        if (realCount > 1) optionsList.add(new DialogueOption("Exchange All: " + costOfAll + " coins", () -> exchange(player, item, realCount)));
    ...
    private static final void exchange(final Player player, final Item item, int count) {
        if (count <= 0) return;
        final com.zenyte.game.world.entity.player.container.ContainerResult coinResult = player.getInventory().deleteItem(new Item(995, count * 50));
        count = coinResult.getSucceededAmount() / 50;
        final com.zenyte.game.world.entity.player.container.ContainerResult result = player.getInventory().deleteItem(item.getId(), count);
    ...
    public Object[] getItems() {
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<java.lang.Integer> list = new ObjectArrayList<Integer>();
        for (final com.zenyte.game.content.skills.prayer.actions.Bones bone : Bones.VALUES) {
            for (final com.zenyte.game.item.Item item : bone.getBones()) {
