...
public class Zahur extends NPCPlugin {
            ...
            public void buildDialogue() {
                ...
                options("Crush the secondaries for " + (free ? "free" : Utils.format(cost)) + "?", new DialogueOption("Yes, crush them.", () -> {
                    ...
                    for (int i = 0; i < 28; i++) {
                        ...
                        final com.zenyte.plugins.renewednpc.Zahur.HerbloreSecondary secondary = Utils.findMatching(HerbloreSecondary.values, sec -> (sec.rawItem == id && sec.processedItem != -1) || (sec.rawNotedItem == id && (sec == HerbloreSecondary.LAVA_SCALE || sec.processedNotedItem != -1)));
                        if (secondary != null) {
    ...
    private enum HerbloreSecondary {
        UNICORN_HORN(237, 235), CHOCOLATE_BAR(1973, 1975), KEBBIT_TEETH(10109, 10111), GORAK_CLAWS(9016, 9018), BIRD_NEST(5075, 6693), DESERT_GOAT_HORN(9735, 9736), SPRING_SQIRK(10844, 10848), SUMMER_SQIRK(10845, 10849), AUTUMN_SQIRK(10846, 10850), WINTER_SQIRK(10847, 10851), CHARCOAL(973, 704), RUNE_SHARDS(6466, 6467), ASHES(592, 8865), RAW_KARAMBWAN(3142, 3152), POISON_KARAMBWAN(3146, 3153), COOKED_KARAMBWAN(3147, 3154), LAVA_SCALE(11992, 11994), BLUE_DRAGON_SCALE(ItemId.BLUE_DRAGON_SCALE, ItemId.DRAGON_SCALE_DUST), SUPERIOR_DRAGON_BONES(22124, 21975);
    ...
    private void makeUnfinishedPotions(final Player player, final NPC npc) {
        ...
        final java.util.HashMap<com.zenyte.game.content.zahur.Herb, com.zenyte.game.item.Item> herbs = new HashMap<Herb, Item>();
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> entry : player.getInventory().getContainer().getItems().int2ObjectEntrySet()) {
            if (entry == null) {
                continue;
            }
            final com.zenyte.game.item.Item item = entry.getValue();
            final com.zenyte.game.content.zahur.Herb herb = Herb.get(item.getId(), DiaryReward.DESERT_AMULET3.eligibleFor(player) || SkillcapePerk.HERBLORE.isEffective(player));
            if (herb == null) {
                continue;
            }
            herbs.put(herb, item);
        ...
        for (final java.util.Map.Entry<com.zenyte.game.content.zahur.Herb, com.zenyte.game.item.Item> entry : herbs.entrySet()) {
            if (entry == null) {
                continue;
            }
            final com.zenyte.game.content.zahur.Herb herb = entry.getKey();
            final com.zenyte.game.item.Item item = entry.getValue();
            final com.zenyte.game.item.Item unfinishedPotion = herb.getUnfinishedPotion();
    ...
    private void cleanHerbs(final Player player, final NPC npc) {
        ...
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> entry : player.getInventory().getContainer().getItems().int2ObjectEntrySet()) {
            ...
            final com.zenyte.game.content.zahur.Herb herb = Herb.get(item.getId(), true);
            if (herb == null) {
                continue;
            }
            herbs.put(herb, item);
    ...
    private boolean decant(final Player player, final NPC npc, final int selectedDose) {
        final java.util.HashMap<com.zenyte.game.content.consumables.drinks.Potion, com.zenyte.game.content.zahur.PotionResult> potions = new HashMap<Potion, PotionResult>();
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> entry : player.getInventory().getContainer().getItems().int2ObjectEntrySet()) {
            if (entry == null) {
                continue;
            }
            final com.zenyte.game.item.Item item = entry.getValue();
            final com.zenyte.game.content.consumables.drinks.Potion potion = Potion.get(item.getId());
            if (potion == null) {
                continue;
            }
            if (potions.containsKey(potion)) {
                ...
                final com.zenyte.game.content.zahur.PotionResult result = new PotionResult(potion, item);
        ...
        for (final java.util.Map.Entry<com.zenyte.game.content.consumables.drinks.Potion, com.zenyte.game.content.zahur.PotionResult> entry : potions.entrySet()) {
            if (entry == null) {
                continue;
            }
            final com.zenyte.game.content.consumables.drinks.Potion potion = entry.getKey();
            final com.zenyte.game.content.zahur.PotionResult result = entry.getValue();
            ...
            final com.zenyte.game.item.Item decanted = new Item(doseFunction.apply(selectedDose - 1), amount);
            if (amount >= 500000000) {
