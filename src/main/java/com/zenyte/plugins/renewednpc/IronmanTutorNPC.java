...
public class IronmanTutorNPC extends NPCPlugin {
    ...
    private void armour(final Player player, final NPC npc) {
        ...
        final com.zenyte.game.item.Item[] armour = ZenyteGuide.STARTER_ITEMS[player.getGameMode().ordinal()];
        final com.zenyte.game.item.Item[] items = Utils.concatenate(player.getInventory().getContainer().getItems().values().toArray(new Item[] {}), player.getEquipment().getContainer().getItems().values().toArray(new Item[] {}), player.getBank().getContainer().getItems().values().toArray(new Item[] {}));
        int added = 0;
        for (final com.zenyte.game.item.Item item : armour) {
        ...
        final java.lang.String message = added == 0 ? "I think you\'ve already got the whole set." : "There you go. Wear it with pride.";
