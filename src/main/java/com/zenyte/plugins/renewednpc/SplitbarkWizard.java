...
public class SplitbarkWizard extends NPCPlugin {
    ...
    private enum SplitbarkArmour {
        ...
        private SplitbarkArmour(final int product, final int materialCount, final int price) {
    ...
    private final void sendMenu(@NotNull final Player player) {
        ...
        for (final com.zenyte.plugins.renewednpc.SplitbarkWizard.SplitbarkArmour value : SplitbarkArmour.values) {
        ...
        player.getDialogueManager().start(new SkillDialogue(player, "How many do you wish to buy?", list.toArray(new Item[0])) {
            ...
            public void run(int slotId, int amount) {
                final com.zenyte.plugins.renewednpc.SplitbarkWizard.SplitbarkArmour constant = Objects.requireNonNull(Utils.findMatching(SplitbarkArmour.values, value -> value.product == list.get(slotId).getId()));
                if (!inventory.containsItem(ItemId.BARK, constant.materialCount) || !inventory.containsItem(ItemId.FINE_CLOTH, constant.materialCount) || !inventory.containsItem(ItemId.COINS_995, constant.price)) {
                    final java.lang.String quantifiedLabel = constant.materialCount == 1 ? "1 piece of" : (constant.materialCount + " pieces of");
