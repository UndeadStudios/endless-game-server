...
public final class FiremakingItemOnFIA implements ItemOnFloorItemAction {
	...
	public void handleItemOnFloorItemAction(final Player player, final Item item, final FloorItem floorItem) {
		final com.zenyte.game.content.skills.firemaking.Firemaking val = Firemaking.MAP.get(floorItem.getId());
		if (val == null) {
			return;
		}
		final java.util.Optional<com.zenyte.game.content.skills.firemaking.FiremakingTool> tool = FiremakingTool.getTool(item.getId());
		if (!tool.isPresent()) {
	...
	public Object[] getItems() {
		...
		for (final com.zenyte.game.content.skills.firemaking.FiremakingTool tool : FiremakingTool.values) {
	...
	public Object[] getFloorItems() {
		final java.util.ArrayList<java.lang.Integer> list = new ArrayList<Integer>(Firemaking.VALUES.length);
