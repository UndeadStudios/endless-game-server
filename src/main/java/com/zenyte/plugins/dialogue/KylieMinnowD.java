...
public class KylieMinnowD extends Dialogue {
    ...
    public void buildDialogue() {
        if(rowboat) {
            ...
            if(player.inArea("Fishing platform") || player.getSkills().getLevel(Skills.FISHING) >= 82/* && checkSuit()*/) {
        ...
        if(player.inArea("Fishing platform")) {
            ...
            if (player.getSkills().getLevel(Skills.FISHING) >= 82/* && checkSuit()*/) {
    ...
    private boolean checkSuit() {
        ...
        if(helm.getId() == 13258 && body.getId() == 13259 && legs.getId() == 13260 && boots.getId() == 13261) {
			return true;
		}
