...
public class DestroyItemDialogue extends Dialogue {
    ...
    public void buildDialogue() {
        destroyItem(item, getMessage(item)).onYes(() -> {
            ...
            final com.zenyte.game.item.Item inSlot = inventory.getItem(slot);
    ...
    private void handle() {
        if (item.getId() == 13639) {
            ...
            if (inWilderness) {
                for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<com.zenyte.game.item.Item> entry : player.getLootingBag().getContainer().getItems().int2ObjectEntrySet()) {
