...
public final class SpiritTreeMenuD extends OptionsMenuD {
	...
	public void handleClick(int slotId) {
		final com.zenyte.game.world.object.SpiritTree tree = SpiritTree.getTree(player);
		final com.zenyte.game.world.object.SpiritTree destinationTree = SpiritTree.get(slotId);
		final com.zenyte.game.world.object.SpiritTree currentTree = SpiritTree.getTree(player);
		if (destinationTree == currentTree) {
			player.getDialogueManager().start(new NPCChat(player, 4982, "You\'re already here."));
			return;
		}
		final java.util.Optional<java.util.function.Predicate<com.zenyte.game.world.entity.player.Player>> optionalPredicate = destinationTree.getOptionalPredicate();
		if (optionalPredicate.isPresent()) {
			final java.util.function.Predicate<com.zenyte.game.world.entity.player.Player> predicate = optionalPredicate.get();
			if (!predicate.test(player)) {
				return;
			}
		}
		final com.zenyte.game.world.entity.Location location = destinationTree.getLocation();
		player.lock(2);
