...
public class ProjectileUtils {
     ...
     */
    public static boolean isProjectileClipped(final Entity sender, final Entity receiver, final Position from, final Position to, final boolean closeProximity) {
    ...
    public static boolean isProjectileClipped(final Entity sender, final Entity receiver, final Position from, final Position to, final boolean closeProximity, final boolean ignoreUnderneath) {
        final int fromSize = sender != null ? sender.getSize() : 1;
        final int toSize = receiver != null ? receiver.getSize() : 1;
        return isProjectileClipped(sender, receiver, from, to, fromSize, toSize, closeProximity, ignoreUnderneath);
    }

    public static boolean isProjectileClipped(final Entity sender, final Entity receiver, final Position from, final Position to, final int fromSize, final int toSize, final boolean closeProximity, final boolean ignoreUnderneath) {
     ...
     * @return whether the projectile is clipped in this path or not.
     */
    private static boolean _isProjectileClipped(final Entity sender, final Entity receiver, final Position from, final Position to, final int fromSize, final int toSize, final boolean closeProximity, boolean ignoreUnderneath) {
        final com.zenyte.game.world.entity.Location sourceLp = from.getPosition();
        final com.zenyte.game.world.entity.Location targetLp = to.getPosition();
        ...
        if (Utils.collides(sourceLp.getX(), sourceLp.getY(), fromSize, targetLp.getX(), targetLp.getY(), toSize)) return false;
        ...
        if (absDistanceX > absDistanceY) {
            ...
            while (currentX != targetX) {
                ...
                if ((getMask(currentX, actualCurrentY, fromPlane) & (currentX == targetX ? (xTileMask & ~entityFlag) : xTileMask)) != 0) {
                ...
                if (postRatioCurrentY != actualCurrentY && (getMask(currentX, postRatioCurrentY, fromPlane) & (currentX == targetX ? (yTileMask & ~entityFlag) : yTileMask)) != 0) {
            ...
            while (currentY != targetY) {
                ...
                if ((getMask(actualCurrentX, currentY, fromPlane) & (currentY == targetY ? (yTileMask & ~entityFlag) : yTileMask)) != 0) {
