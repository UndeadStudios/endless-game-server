...
public class Group {
	...
	public void load(ByteBuffer packedFiles, int[] xtea, boolean unpack) {
		...
		if (unpack) {
			unpack(packedFiles);
			int highest = 0;
			for (final mgi.tools.jagcached.cache.File file : files) {
	...
	private void unpack(ByteBuffer packed) {
		...
		for (int sectorID = 0; sectorID < sectorsCount; sectorID++) for (int i = 0, length = 0; i < fileCount(); i++) lengths[i] += (length += packed.readInt());
	...
	private ByteBuffer pack() {
		...
		for (int i = 0; i < files.length; i++) packed.writeBytes(files[i].getData().getBuffer(), 0, files[i].getData().getBuffer().length);
