...
public class ObjectReader implements TypeReader {
    ...
    public ArrayList<Definitions> read(final Map<String, Object> properties) throws NoSuchFieldException, IllegalAccessException {
        ...
        if (properties.containsKey("inherit")) {
            ...
            if (inherit instanceof ArrayList) {
                ...
                for (final java.lang.Long id : ids) {
                    final mgi.types.config.ObjectDefinitions def = ObjectDefinitions.get(id.intValue());
                    defs.add(TypeParser.KRYO.copy(def));
                }
            } else {
                final mgi.types.config.ObjectDefinitions def = ObjectDefinitions.get(((Long) inherit).intValue());
                defs.add(TypeParser.KRYO.copy(def));
            }
        } else {
        ...
        for (final mgi.types.Definitions definition : defs) {
            final mgi.types.config.ObjectDefinitions object = (ObjectDefinitions) definition;
