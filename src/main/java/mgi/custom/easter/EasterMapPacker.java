...
public class EasterMapPacker {
    ...
    public final void packAll() throws IOException {
        ...
        //testInterfaces();
        final java.util.Collection<com.zenyte.game.world.object.WorldObject> objs = MapUtils.decode(new ByteBuffer(Files.readAllBytes(Paths.get("assets/easter/map/l_9810.dat"))));
        objs.forEach(o -> {
            set.add(o.getId());
            o.setId(transformedObjects.getOrDefault(o.getId(), o.getId()));
        });
        final java.util.Collection<com.zenyte.game.world.object.WorldObject> scourgeObjs = MapUtils.decode(new ByteBuffer(Files.readAllBytes(Paths.get("assets/easter/map/l_9811.dat"))));
    ...
    public void packInstructionsInterface() throws IOException {
        ...
        final mgi.tools.jagcached.cache.Group group = new Group(712);
        for (int i = 0; i < 4; i++) {
            final mgi.types.component.ComponentDefinitions def = new ComponentDefinitions(712 << 16 | i, new ByteBuffer(java.nio.file.Files.readAllBytes(Paths.get("assets/easter/interfaces/712/" + i + ".dat"))));
    ...
    public void testInterfaces() throws IOException {
        ...
        final mgi.tools.jagcached.cache.Cache cache = Game.getCacheMgi();
        final mgi.tools.jagcached.cache.Group group = new Group(711);
        final it.unimi.dsi.fastutil.ints.Int2IntOpenHashMap map = new Int2IntOpenHashMap();
        map.put(32821, 53007);
        map.put(32825, 53008);
        map.put(32826, 53009);
        map.put(32828, 53010);
        ...
        map.put(537, 53016);
        for (int i = 0; i < 31; i++) {
            final mgi.types.component.ComponentDefinitions def = new ComponentDefinitions(711 << 16 | i, new ByteBuffer(java.nio.file.Files.readAllBytes(Paths.get("assets/easter/interfaces/711/" + i + ".dat"))));
    ...
    public void packObjects() throws IOException {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<mgi.types.config.ObjectDefinitions> objSet = new ObjectArrayList<ObjectDefinitions>();
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<java.io.File> entry : sortedMap.int2ObjectEntrySet()) {
            ...
            final mgi.types.config.ObjectDefinitions def = new ObjectDefinitions(ChristmasMapPacker.objectId, new ByteBuffer(FileUtils.readFileToByteArray(file)));
        ...
        final it.unimi.dsi.fastutil.ints.Int2IntOpenHashMap map = new Int2IntOpenHashMap();
        for (final mgi.types.config.ObjectDefinitions obj : objSet) {
        ...
        for (final mgi.types.config.ObjectDefinitions obj : objSet) {
            if (ArrayUtils.contains(objectsToTransform, obj.getId())) {
                if (obj.getTransmogrifiedIds() != null) {
                    final it.unimi.dsi.fastutil.ints.IntAVLTreeSet transforms = new IntAVLTreeSet(obj.getTransformedIds());
                    transforms.remove(-1);
                    for (final java.lang.Integer transform : transforms) {
                        final mgi.types.config.ObjectDefinitions def = Utils.findMatching(objSet, t -> t.getId() == transform);
    ...
    public void packModels() throws IOException {
        ...
        final it.unimi.dsi.fastutil.ints.IntAVLTreeSet list = new IntAVLTreeSet();
        while (it.hasNext()) {
            final java.io.File file = it.next();
            final java.lang.String name = file.getName().replace(".dat", "");
            final int id = Integer.parseInt(name);
            list.add(id);
        }
        for (final java.lang.Integer originalId : list) {
    ...
    public void packVarbits() throws IOException {
        ...
        final it.unimi.dsi.fastutil.ints.Int2IntOpenHashMap varpMap = new Int2IntOpenHashMap();
        int currentVarp = ChristmasMapPacker.varpIndex + 10;
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<java.io.File> entry : sortedMap.int2ObjectEntrySet()) {
            final java.io.File file = entry.getValue();
            final mgi.types.config.VarbitDefinitions defs = new VarbitDefinitions(entry.getIntKey() + varbitOffset, new ByteBuffer(FileUtils.readFileToByteArray(file)));
    ...
    public void packGraphics() throws IOException {
        ...
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<java.io.File> entry : sortedMap.int2ObjectEntrySet()) {
            final java.io.File file = entry.getValue();
            final mgi.types.config.GraphicsDefinitions def = new GraphicsDefinitions(ChristmasMapPacker.graphicsId++, new ByteBuffer(FileUtils.readFileToByteArray(file)));
    ...
    private void packAnimationBase(@NotNull final AnimationBase base, final int[] anims) throws IOException {
        for (int anim : anims) {
            ...
            final mgi.types.config.AnimationDefinitions definitions = new AnimationDefinitions(anim, new ByteBuffer(FileUtils.readFileToByteArray(file)));
    ...
    private void injectObjectAsNPC(final int objectId, final ObjectArrayList<NPCDefinitions> objSet) {
        mgi.types.config.npcs.NPCDefinitions def = new NPCDefinitions(ChristmasMapPacker.npcId++, new ByteBuffer(new byte[1]));
        objSet.add(def);
        mgi.types.config.ObjectDefinitions objectDef = objectMap.get(objectId);
        ...
        final it.unimi.dsi.fastutil.ints.IntArrayList transforms = new IntArrayList(new IntAVLTreeSet(transformedIds));
        transforms.rem(-1);
        final mgi.types.config.npcs.NPCDefinitions[] bases = new NPCDefinitions[transforms.size()];
        bases[0] = def;
        ...
        for (int i = 0; i < bases.length; i++) {
            final mgi.types.config.npcs.NPCDefinitions base = bases[i];
        ...
        for (final java.lang.Integer transformedId : transforms) {
    ...
    public void packNPCs() throws IOException {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectArrayList<mgi.types.config.npcs.NPCDefinitions> objSet = new ObjectArrayList<NPCDefinitions>();
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<java.io.File> entry : sortedMap.int2ObjectEntrySet()) {
            final java.io.File file = entry.getValue();
            final mgi.types.config.npcs.NPCDefinitions def = new NPCDefinitions(ChristmasMapPacker.npcId, new ByteBuffer(FileUtils.readFileToByteArray(file)));
            def.setVarbit(transformedVarbits.getOrDefault(def.getVarbit(), def.getVarbit()));
            objSet.add(def);
            final int originalId = entry.getIntKey();
        ...
        //7427, 7443
        //Lets make an exception for a known problematic npc of id 15197
        final mgi.types.config.npcs.NPCDefinitions matching = Utils.findMatching(objSet, o -> o.getId() == 15197);
        final mgi.types.config.npcs.NPCDefinitions def = Utils.findMatching(objSet, o -> o.getId() == 15213);
        ...
        for (final mgi.types.config.npcs.NPCDefinitions obj : objSet) {
            if (obj.getTransmogrifiedIds() != null) {
        ...
        for (final mgi.types.config.npcs.NPCDefinitions obj : objSet) {
            if (obj.getTransmogrifiedIds() != null) {
                for (int i = 0; i < obj.getTransmogrifiedIds().length; i++) {
                    ...
                    final int index = i;
                    final mgi.types.config.npcs.NPCDefinitions otherDef = Utils.findMatching(objSet, o -> o.getId() == obj.getTransmogrifiedIds()[index]);
                    if (otherDef != null) {
    ...
    public void packItems() throws IOException {
        ...
        for (final it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<java.io.File> entry : sortedMap.int2ObjectEntrySet()) {
            final java.io.File file = entry.getValue();
            final mgi.types.config.items.ItemDefinitions def = new ItemDefinitions(ChristmasMapPacker.itemId++, new ByteBuffer(FileUtils.readFileToByteArray(file)));
            ...
            if (def.getParameters() != null) {
                if (def.getParameters().get(528) != null) {
                    final java.lang.Object op = def.getParameters().get(528);
