...
public class ModeSelectionBackup extends ComponentDefinitions {
    public ArrayList<ComponentDefinitions> assemble() {
        ...
        add(componentId++, component);
        final java.lang.String[] gameModes = new String[] {"Regular", "Standard Iron Man", "Hardcore Iron Man", "Ultimate Iron Man"};
        for (int i = 0; i < 4; i++) {
        ...
        add(componentId++, component);
        final java.lang.String[] xpModes = new String[] {"1x rate", "2x rate"};
        for (int i = 0; i < 2; i++) {
            component = new RectangleComponent((i + 1) % 2 == 0 ? "#574E43" : "#4E453A", true);
            component.setParentId(26);
            component.setPosition(2, 2 + (i * 45));
