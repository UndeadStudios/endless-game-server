...
public class SpriteGroupDefinitions implements Definitions, Cloneable {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(SpriteGroupDefinitions.class);
    ...
    public void load() {
        ...
        final mgi.tools.jagcached.cache.Archive sprites = cache.getArchive(ArchiveType.SPRITES);
        for (int id = 0; id < sprites.getHighestGroupId(); id++) {
            final mgi.tools.jagcached.cache.Group spriteGroup = sprites.findGroupByID(id);
            if (spriteGroup == null) {
                continue;
            }
            for (int index = 0; index < spriteGroup.fileCount(); index++) {
                final mgi.tools.jagcached.cache.File file = spriteGroup.findFileByID(index);
                if (file == null) {
                    continue;
                }
    ...
    public ByteBuffer encode() {
        ...
        try {
            ...
            for (int i = 0; i < images.size(); i++) {
                final java.awt.image.BufferedImage image = images.get(i);
    ...
    public void pack() {
        final mgi.tools.jagcached.cache.Cache cache = Game.getCacheMgi();
        final mgi.tools.jagcached.cache.Archive archive = cache.getArchive(ArchiveType.SPRITES);
