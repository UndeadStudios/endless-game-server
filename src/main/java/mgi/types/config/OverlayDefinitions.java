...
public final class OverlayDefinitions implements Definitions {
	...
	public void load() {
		...
		final mgi.tools.jagcached.cache.Group overlays = configs.findGroupByID(GroupType.OVERLAY);
		definitions = new OverlayDefinitions[overlays.getHighestFileId()];
		for (int id = 0; id < overlays.getHighestFileId(); id++) {
			final mgi.tools.jagcached.cache.File file = overlays.findFileByID(id);
			if (file == null) {
				continue;
			}
