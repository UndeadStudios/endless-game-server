...
public class HitbarDefinitions implements Definitions {
    private static final org.slf4j.Logger log = org.slf4j.LoggerFactory.getLogger(HitbarDefinitions.class);
    ...
    public static final void extractAll() {
        ...
        for (int i = 0; i < Utils.getIndiceSize(Indice.HITBAR); i++) {
            final mgi.types.config.HitbarDefinitions defs = HitbarDefinitions.get(i);
    ...
    public void load() {
        ...
        final mgi.tools.jagcached.cache.Group hitBars = configs.findGroupByID(GroupType.HITBAR);
    ...
    public void extract(final File file, final float fillPercentage) {
        ...
        final java.awt.image.BufferedImage mainImage = primarySprite == -1 ? null : SpriteGroupDefinitions.get(primarySprite).getImage(0);
        final java.awt.image.BufferedImage backgroundImage = secondarySprite == -1 ? null : SpriteGroupDefinitions.get(secondarySprite).getImage(0);
        final java.awt.image.BufferedImage resizedImage = new BufferedImage(size, HEIGHT, 1);
        if (primarySprite == -1) {
