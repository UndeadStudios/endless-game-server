...
public final class VarbitDefinitions implements Definitions {
	...
	public void load() {
		...
		final mgi.tools.jagcached.cache.Group varbits = configs.findGroupByID(GroupType.VARBIT);
		definitions = new VarbitDefinitions[varbits.getHighestFileId()];
		for (int id = 0; id < varbits.getHighestFileId(); id++) {
			final mgi.tools.jagcached.cache.File file = varbits.findFileByID(id);
			if (file == null) {
				continue;
			}
	...
	public void pack() {
		...
		final mgi.tools.jagcached.cache.Group varbits = archive.findGroupByID(GroupType.VARBIT);
	...
	public ByteBuffer encode() {
		final mgi.utilities.ByteBuffer buffer = new ByteBuffer(6);
		buffer.writeByte(1);
