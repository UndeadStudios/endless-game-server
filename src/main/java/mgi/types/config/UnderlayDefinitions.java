...
public final class UnderlayDefinitions implements Definitions {
	...
	public void load() {
		...
		final mgi.tools.jagcached.cache.Group underlays = configs.findGroupByID(GroupType.UNDERLAY);
		definitions = new UnderlayDefinitions[underlays.getHighestFileId()];
		for (int id = 0; id < underlays.getHighestFileId(); id++) {
			final mgi.tools.jagcached.cache.File file = underlays.findFileByID(id);
			if (file == null) {
				continue;
			}
