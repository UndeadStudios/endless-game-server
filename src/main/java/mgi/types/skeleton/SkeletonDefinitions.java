...
public class SkeletonDefinitions implements Definitions {
    ...
    public void load() {
        ...
        final mgi.tools.jagcached.cache.Archive skeletons = cache.getArchive(ArchiveType.SKELETONS);
        for (int i = 0; i < skeletons.getHighestGroupId(); i++) {
            final mgi.tools.jagcached.cache.Group skeletonGroup = skeletons.findGroupByID(i);
            if (skeletonGroup == null) {
                continue;
            }
            for (int id = 0; id < skeletonGroup.getHighestFileId(); id++) {
                final mgi.tools.jagcached.cache.File file = skeletonGroup.findFileByID(id);
                if (file == null) {
                    continue;
                }
    ...
    public static final IntOpenHashSet getLinkedFrames(final int frameMapId) {
        ...
        final it.unimi.dsi.fastutil.objects.ObjectIterator<it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<mgi.types.skeleton.SkeletonDefinitions>> iterator = SkeletonDefinitions.getDefinitions().int2ObjectEntrySet().fastIterator();
    ...
    public static Int2ObjectOpenHashMap<SkeletonDefinitions> getDefinitions() {
        return SkeletonDefinitions.definitions;
    }
